function[] = drawSaveNBNInMesh2Imagefun(figureId,filepath,filetype, edge_a,edge_b,x,y,z,node_fit,numColor,basicNodeSize,linewidth,mesh_x,mesh_y,mesh_z,conT)
...
nbn_plot = plot(nbn_graph,'XData',x,'YData',y,'ZData',z,'NodeColor', black_color , 'LineWidth',linewidth ,'EdgeColor',black_color);
hold on;
%nbn_points=  scatter3(x,y,z,node_size,node_color,'filled');
hold on;
            sampleSize = size(mesh_x);
            div = sqrt(sampleSize(1,2));
            mesh_x= reshape(mesh_x,div,div);
            mesh_y = reshape(mesh_y,div,div);
            mesh_z = reshape(mesh_z,div,div);
         contour3(mesh_x,mesh_y,mesh_z,conT);
            colormap("hsv");
    outputfilename=strcat(filepath,"_fitness_nbn_mesh");   
