...
namespace ofec {
	class NBN_OneMaxSimpleDivison : public NBN_DivisionBase {
	...
	protected:
		std::vector<std::vector<int>> m_sols_edges;
		//std::vector<std::vector<int>> m_sol_vecs;
	...
	public:
		...
		void setSol(const std::vector<std::shared_ptr<SolutionBase>>& sols,
			const std::vector<int>& solId
		) {
			resize(sols.size());
			m_insertId = solId;
			auto pro = m_environment->problem();
			//m_sols_edges.resize(sols.size());
			ofec::Real pos = (pro->optimizeMode(0) == ofec::OptimizeMode::kMaximize) ? 1 : -1;
			for (int idx(0); idx < sols.size(); ++idx) {
				m_sols[idx] = sols[idx];
				m_sols[idx]->setFitness(pos * m_sols[idx]->objective(0));
				m_fitness[idx] = m_sols[idx]->fitness();
				m_sols_edges[idx] = dynamic_cast<solutionType&>(*m_sols[idx]).variable().vect();
			}
		...
		virtual void addSol(const SolutionBase& new_sol, int& belong_id,
		...
		virtual void getSharedNBN(std::vector<std::shared_ptr<SolutionBase>>& sols,
			std::vector<double>& fitness,
			std::vector<int>& belong,
			std::vector<double>& dis2parent,
		...
		void getFitness(std::vector<double>& fitness) {
			fitness = m_fitness;
		}
