#include "nbn_grid_shortest_path_division.h"
#include "../../core/problem/continuous/continuous.h"
#include "../function/custom_function.h"



void ofec::NBN_GridDivision_ShortestPath::initialize_(bool flag_grid_sample )  {
...
void ofec::NBN_GridDivision_ShortestPath::idxToVec(int idx, std::vector<int>& cur) {
...
void ofec::NBN_GridDivision_ShortestPath::vecToIdx(const std::vector<int>& cur, int& idx) {
	//for(int idDim)
	idx = 0;
...
void ofec::NBN_GridDivision_ShortestPath::solToVec(const SolBase& sol, std::vector<int>& vec) {
...
bool ofec::NBN_GridDivision_ShortestPath::judgeFeasible(const std::vector<int>& cur) {
...
void ofec::NBN_GridDivision_ShortestPath::initNodesNormal() {
...
void ofec::NBN_GridDivision_ShortestPath::updateFitness() {
	...
	if (!m_flag_multiThread||m_pro->hasTag(ProTag::kCSIWDN)) {
		...
		for (size_t i = 0; i < num_task; ++i) {
			...
			thrds.push_back(std::thread(
				&NBN_GridDivision_ShortestPath::updateFitnessThreadTask, this,
...
void ofec::NBN_GridDivision_ShortestPath::updateFitnessThreadTask(int from, int to) {
...
//
//void ofec::NBN_GridDivision_ShortestPath::initNodes2D() {
//
...
//
//					double cur_dis = m_pro->norVariableDistance(*m_cur_sols[cur_id], *m_cur_sols[nei_id]);
...
void ofec::NBN_GridDivision_ShortestPath::mergeParent(Node* cur_node) {
...
void ofec::NBN_GridDivision_ShortestPath::mergePeaks(int curId, std::vector<int>& peaksId) {
	if (m_nbn_nodes[curId].m_visited_id == m_cur_visited)return;
	m_nbn_nodes[curId].m_visited_id = m_cur_visited;
	if (m_nbn_nodes[curId].m_peak_range.empty()) peaksId.push_back(curId);
	else {
		for (auto& betterId : m_nbn_nodes[curId].m_peak_range) {
			mergePeaks(betterId, peaksId);
		}
	}
}
void ofec::NBN_GridDivision_ShortestPath::mergePeaks(const std::vector<int>& ranges, std::vector<int>& peaksId) {
	for (auto& betterId : ranges) {
		mergePeaks(betterId, peaksId);
	}
}

void ofec::NBN_GridDivision_ShortestPath::calculate()
{
	...
	std::sort(sorted_idx.begin(), sorted_idx.end(), [&](
		...
		if (m_nbn_nodes[a].m_fitness == m_nbn_nodes[b].m_fitness) {
			return m_random->uniform.next() < 0.5;
		}
	...
	for (auto& sortedId : sorted_idx) {
		...
		for (auto& nei_info : cur_node.m_neighbor_id_dis){
			...
			if (nei_node.m_peak_range.empty()) {
				if (nei_node.m_visited_id != m_cur_visited) {
					nei_node.m_visited_id = m_cur_visited;
					peak_ranges.push_back(nei_node.m_id);
				}
			}
			else {
				for (auto& betterId : nei_node.m_peak_range) {
					auto& betterNode(m_nbn_nodes[betterId]);
					if (betterNode.m_visited_id != m_cur_visited) {
						betterNode.m_visited_id = m_cur_visited;
						peak_ranges.push_back(betterNode.m_id);
					}
				}
			}
		}
		...
		mergePeaks(peak_ranges, cur_node.m_better_range);
		cur_node.m_peak_range = cur_node.m_better_range;
	}

	for (auto it = sorted_idx.rbegin(); it != sorted_idx.rend(); ++it){
		int curId = *it;
		auto& curNode(m_nbn_nodes[curId]);
		int minStep(std::numeric_limits<int>::max());
		double minDis(std::numeric_limits<double>::max());
		double curDis(0);
		int parentId(0);
		for (auto& betterId : curNode.m_better_range) {
			if (betterId == curId)continue;
			auto& parNode(m_nbn_nodes[betterId]);
			curDis = m_pro->norVariableDistance(*parNode.m_sol, *curNode.m_sol);
			if (curDis < minDis) {
				minDis = curDis;
				minStep = parNode.m_step2global;
			}
			else if (curDis == minDis) {
				if (minStep > parNode.m_step2global) {
					minStep = parNode.m_step2global;
					parentId = betterId;
				}
				else if (minStep == parNode.m_step2global && m_random->uniform.next() < 0.5) {
					parentId = betterId;
				}
			}
		...
		curNode.m_step2global = minStep + 1;
		curNode.m_direct_parent= curNode.m_parent = parentId;
		curNode.m_dis2parent = m_pro->norVariableDistance(*curNode.m_sol, *m_nbn_nodes[parentId].m_sol);
	}
}



void ofec::NBN_GridDivision_ShortestPath::updateSol(const SolBase& new_sol,int & belong_id, 
	...
	if (cur_sol->fitness() > m_nbn_nodes[nearest_id].m_fitness) {
		m_nbn_nodes[nearest_id].m_sol = cur_sol;
...
//
//void ofec::NBN_GridDivision_ShortestPath::udpate_network() {
//
...
void ofec::NBN_GridDivision_ShortestPath::getNearestBetterNetworkShareMemory(
...
void  ofec::NBN_GridDivision_ShortestPath::getNearestBetterNetworkShareMemory(
	...
	for (int idx(0); idx < sols.size(); ++idx) {
		sols[idx]= m_nbn_nodes[idx].m_sol;
		representative[idx] = m_nbn_nodes[idx].m_sol;
		fitness[idx] = m_nbn_nodes[idx].m_fitness;
		belong[idx] = m_nbn_nodes[idx].m_direct_parent;
...
void ofec::NBN_GridDivision_ShortestPath::getSharedNBN(
	std::vector<std::shared_ptr<SolBase>>& sols,
	std::vector<double>& fitness,
	std::vector<int>& belong,
...
void ofec::NBN_GridDivision_ShortestPath::getNearestBetterNetworkShareMemoryFilter(
	std::vector<std::shared_ptr<SolBase>>& sols,
	std::vector<double>& fitness,
	std::vector<int>& belong) {
...
//
//
//int ofec::NBN_GridDivision_ShortestPath::addRandomSol(const std::shared_ptr<SolutionType>& new_sol, int to_idx) {
