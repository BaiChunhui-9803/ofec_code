...
namespace rl {
	...
	std::pair<RANDOM_WALK_STATE_TYPE, double> random_walk::next_state(const RANDOM_WALK_STATE_TYPE & state, const RANDOM_WALK_ACTION_TYPE & action) {
		...
		if (cur_state == m_right_end) {
			return std::make_pair(cur_state, 1.);
		}
		else {
			return std::make_pair(cur_state, 0.);
		}
	}
