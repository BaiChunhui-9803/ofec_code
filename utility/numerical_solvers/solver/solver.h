...
struct State {
  ...
  void Update(const function::State<scalar_t, vector_t, hessian_t>
    ...
    if (std::isnan(f_delta) || std::isnan(x_delta) || std::isnan(gradient_norm)) {
        m_nan_flag = true;
        status = Status::NanStop;
        return;
    }
...
class Solver {
 ...
 public:
  explicit Solver(bool flagDisplay,const State<scalar_t>& stopping_state =
      ...
      if (flagDisplay) {
          ...
          step_callback_ = GetEmptyStepCallback<scalar_t, vector_t, hessian_t>();
  ...
  virtual std::tuple<function_state_t, state_t> Minimize(
    ...
    do {
      ...
      function_state = this->OptimizationStep(function, previous_function_state,
                                              solver_state);
      before_state = solver_state;

      // Update current solver state.
      solver_state.Update(previous_function_state, function_state,
                          stopping_state_);
      if (solver_state.m_nan_flag) {
          solver_state = before_state;
          solver_state.status = Status::NanStop;
