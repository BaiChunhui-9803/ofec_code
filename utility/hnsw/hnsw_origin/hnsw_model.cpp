...
HnswModel::HnswModel(const std::string& fname, const bool use_mmap) {
    if(!use_mmap) {
        ...
        model_mmap_.open(fname);
        model_byte_size_ = model_mmap_.size();
        model_ = model_mmap_.data();
    }
...
void HnswModel::LoadConfigFromModel() {
    ...
    if (metric_ != DistanceKind::ANGULAR && metric_ != DistanceKind::L2 && metric_ != DistanceKind::DOT) {
