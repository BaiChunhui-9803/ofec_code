...
namespace ofec {
	namespace hnsw {
		...
		void HnswModel::addDataMutliThread(const std::vector<ofec::SolutionBase*>& sols, std::vector<int>& solId) {
			//std::vector<ofec::Node*> sols;
			int fromId = nodes_.size();
			nodes_.resize(nodes_.size() + sols.size());
			m_solutions.resize(nodes_.size());
			int toId = nodes_.size();
			solId.clear();
			for (int id(fromId); id < toId; ++id) {
				int level = GetRandomNodeLevel(m_rnd.get());
				nodes_[id].set(id, level, max_m_, max_m0_);
				solId.push_back(fromId);
				m_solutions[id] = sols[id - fromId]->getSharedPtr();
			}
			if (fromId == 0) {
				max_level_ = nodes_.front().GetLevel();
				m_enterpoint_id = nodes_.front().GetId();
				++fromId;
			}

			for (auto& it : m_threadInfos) {
				it.resize(nodes_.size());
			}
			m_globalInfo.clear();
			for (int id(fromId); id < toId; ++id) {
				m_globalInfo.m_tasks.push_back(id);
			}
			std::vector<std::thread> thrds;
			for (size_t i = 0; i < m_threadInfos.size(); ++i) {
				thrds.push_back(std::thread(
					&HnswModel::InsertNodeMultiThreadTask, this, std::ref(m_threadInfos[i])));
			}
			for (auto& thrd : thrds)
				thrd.join();
		...
		void HnswModel::InsertNodeMultiThreadTask(
			ThreadLocalInfo& cur) {
			using namespace std;

			auto& threadInfo = cur;
			auto& globalInfo = m_globalInfo;
			...
			while (true) {
				...
				std::vector<int> calculatingNodes;
				{
					unique_lock<mutex> lock(globalInfo.m_info_mtx);
					if (globalInfo.m_tasks.empty()) return;
					threadInfo.m_curId = globalInfo.m_tasks.front();
					globalInfo.m_tasks.pop_front();
					for (auto& it : globalInfo.m_calculatingNodes) {
						calculatingNodes.push_back(it);
					}
					globalInfo.m_calculatingNodes.insert(threadInfo.m_curId);
				}



				cur.swapVisited();
				cur.m_distanceVisited.Reset();
				int qNodeId = threadInfo.m_curId;



				{

					int cur_level = nodes_[qNodeId].GetLevel();
					int max_level_copy = 0;
					int enterpoint_copy = -1;
					{
						std::shared_lock lock(max_level_guard_);
						max_level_copy = max_level_;
						enterpoint_copy = m_enterpoint_id;
					}
					...
					if (cur_level < max_level_copy) {
						std::set<Info> neighbors;
						int cur_node = enterpoint_copy;
						double d = getSaveDistanceId(cur_node, m_env.get(), cur);
						...
						for (auto i = max_level_copy; i > cur_level; --i) {
							...
							while (changed) {
								changed = false;
								nodes_[cur_node].getFriends(i, neighbors);
					...
					Info curEdge;
					//NeighborInfo beforeNeighbors;
					std::vector<NeighborInfo> level_neighbors(cur_level + 1);
					for (auto i = cur_level; i >= 0; --i) {

						auto& beforeNeighbors = level_neighbors[i];
						NeighborInfo result;
						curVisited.Reset();
						if (i <= max_level_copy) {
							SearchAtLayerMultiThread(cur, enterpoints, i, result);
							enterpoints.clear();
							for (auto& it : result.neighbors()) {
								enterpoints.push_back(it.nodeId());
							}
						}
						{
							for (auto& otherNode : calculatingNodes) {
								if (curVisited.NotVisited(otherNode)
									&& nodes_[otherNode].GetLevel() >= i) {
									double d = getSaveDistanceId(otherNode, m_env.get(), cur);

									if (result.size() < ef_construction_ || result.top().value() > d) {
										curEdge.set(otherNode, d);
										result.push(curEdge);
										if (result.size() > ef_construction_)
											result.pop();
									}
									curVisited.MarkAsVisited(otherNode);

								}
							}
						}
						{
							std::unique_lock lock(nodes_[qNodeId].mutex());
							beforeNeighbors = std::move(nodes_[qNodeId].getFriends(i));
						}
						for (auto& it : beforeNeighbors.neighbors()) {
							if (curVisited.NotVisited(it.nodeId())) {
								result.push(it);
								curVisited.MarkAsVisited(it.nodeId());
							}
						...
						{
							std::unique_lock lock(nodes_[qNodeId].mutex());
							for (auto& it : nodes_[qNodeId].getFriends(i).neighbors()) {
								if (curVisited.NotVisited(it.nodeId())) {
									result.push(it);
									curVisited.MarkAsVisited(it.nodeId());
								}
								//result.emplace(it.GetNode(), it.Getvalue());
							}
							selecting_policy_->Select(m_, i == 0, result, m_distanceFun);
							beforeNeighbors = result;
							nodes_[qNodeId].setFriendsSingleThreadMove(i, result);
						}
					...
					// update network
					{
						...
						for (auto& it : calculatingNodes) {
							curVisited.MarkAsVisited(it);
						}
						for (auto i = cur_level; i >= 0; --i) {
							auto& neighbors = level_neighbors[i];
							for (auto& it : neighbors.neighbors()) {
								if (it.nodeId() < qNodeId || curVisited.Visited(it.nodeId())) {
									curEdge.set(qNodeId, it.value());
									LinkMultiThread(it.nodeId(), curEdge, i);
									//Link(it.GetNode(), qnode, it.Getvalue(), i);
								}
							}
						}
					}
				...
				{
					unique_lock<mutex> lock(globalInfo.m_info_mtx);
					globalInfo.m_calculatingNodes.erase(qNodeId);
