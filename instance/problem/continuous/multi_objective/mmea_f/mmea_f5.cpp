...
namespace ofec {
	void MMEA_F5::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real y = (x[0] + x[1]) / 2.;
		Real sum = 0.;
		Real g;
		for (size_t j = 2; j < m_number_variables; ++j) {
			...
			if (j % 2) {
				...
				h = 2 * x[j] - y * std::sin(1. / 3 * (2 * OFEC_PI * y + (j + 1) * OFEC_PI / m_number_variables)) - 1;
			}
			sum = sum + h * h;
		}
		...
		obj[0] = y;
		obj[1] = g - obj[0]+std::sin(2*OFEC_PI*obj[0])/2./OFEC_PI;
	}

	void MMEA_F5::updateOptima() {
	...
	void MMEA_F5::sampleParetoSols(size_t sample_num) {
