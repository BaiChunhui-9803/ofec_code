#include "moeadde_f9.h"

namespace ofec {
	void MOEADDE_F9::evaluateObjective(Real* x, std::vector<Real>& obj) {
		...
		for (size_t i = 0; i < obj1_dim_inx.size(); ++i) {
			beta1 += (std::pow(x[obj1_dim_inx[i]] - std::sin(6 * OFEC_PI * x[0] + (1. + obj1_dim_inx[i]) * OFEC_PI / m_number_variables), 2));
		}
		for (size_t i = 0; i < obj2_dim_inx.size(); ++i) {
			beta2 += (std::pow(x[obj2_dim_inx[i]] - std::sin(6 * OFEC_PI * x[0] + (1. + obj2_dim_inx[i]) * OFEC_PI / m_number_variables), 2));
		}
		...
		obj[1] = 1 - x[0]*x[0] + 2. / obj2_dim_inx.size() * beta2;
	}

	void MOEADDE_F9::updateOptima() {
...
//#endif // 
	...
	void MOEADDE_F9::sampleParetoSols(size_t sample_num) {
