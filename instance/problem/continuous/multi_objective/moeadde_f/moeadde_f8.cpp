#include "moeadde_f8.h"

namespace ofec {
	void MOEADDE_F8::evaluateObjective(Real* x, std::vector<Real>& obj) {
		...
		for (size_t i = 0; i < obj1_dim_inx.size(); ++i) {
			...
			beta1 += (temp*temp);
			gamma1 *= (std::cos(20*OFEC_PI*temp/std::sqrt(1+ obj1_dim_inx[i])));
		}
		for (size_t i = 0; i < obj2_dim_inx.size(); ++i) {
			...
			beta2 += (temp * temp);
			gamma2 *= (std::cos(20 * OFEC_PI * temp / std::sqrt(1 + obj2_dim_inx[i])));
		}
		if (m_number_variables < 3) {
			obj[0] = x[0];
		}
		else {
			obj[0] = x[0] + 2. / obj1_dim_inx.size() * (4.*beta1-2.*gamma1+2.);
		}
		obj[1] = 1 - std::sqrt(x[0]) + 2. / obj2_dim_inx.size() * (4. * beta2 - 2. * gamma2 + 2.);
	}

	void MOEADDE_F8::updateOptima() {
...
//#endif // 
	...
	void MOEADDE_F8::sampleParetoSols(size_t sample_num) {
