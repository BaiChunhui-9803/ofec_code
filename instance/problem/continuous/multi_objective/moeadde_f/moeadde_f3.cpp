#include "moeadde_f3.h"

namespace ofec {
	void MOEADDE_F3::evaluateObjective(Real* x, std::vector<Real>& obj) {
		...
		for (size_t i = 0; i < obj1_dim_inx.size(); ++i) {
			beta1 += (std::pow(x[obj1_dim_inx[i]] - 0.8*x[0]*std::cos(6 * OFEC_PI * x[0] + (1. + obj1_dim_inx[i]) * OFEC_PI / m_number_variables), 2));
		}
...
//#endif // 
	...
	void MOEADDE_F3::sampleParetoSols(size_t sample_num) {
			...
			for (size_t j = 0; j < m_number_variables; j++) {
				if (j == 0)
					...
					if (j % 2 == 1)
						...
						temp_var[j] = 0.8 * temp_var[0] * std::cos(6 * OFEC_PI * temp_var[0] + (j + 1) * OFEC_PI / m_number_variables);
				}
			}
