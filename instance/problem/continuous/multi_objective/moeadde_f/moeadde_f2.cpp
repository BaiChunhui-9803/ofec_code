#include "moeadde_f2.h"

namespace ofec {
	void MOEADDE_F2::evaluateObjective(Real* x, std::vector<Real>& obj) {
		...
		for (size_t i = 0; i < obj1_dim_inx.size(); ++i) {
			beta1 += (std::pow(x[obj1_dim_inx[i]] - std::sin(4 * OFEC_PI * x[0] + (1. + obj1_dim_inx[i]) * OFEC_PI / m_number_variables), 2));
		}
		for (size_t i = 0; i < obj2_dim_inx.size(); ++i) {
			beta2 += (std::pow(x[obj2_dim_inx[i]] - std::sin(4 * OFEC_PI * x[0] + (1. + obj2_dim_inx[i]) * OFEC_PI / m_number_variables), 2));
		}
...
//#endif // 
	...
	void MOEADDE_F2::sampleParetoSols(size_t sample_num) {
		...
		for (size_t i = 0; i < num; i++) {
			...
			for (size_t j = 0; j < m_number_variables; j++) {
				if (j == 0)
					...
					temp_var[j] = std::sin(4 * OFEC_PI * temp_var[0] + (j + 1) * OFEC_PI / m_number_variables);
				}
			}
