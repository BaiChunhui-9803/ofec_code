#include "moeadde_f5.h"

namespace ofec {
	void MOEADDE_F5::evaluateObjective(Real* x, std::vector<Real>& obj) {
		...
		for (size_t i = 0; i < obj1_dim_inx.size(); ++i) {
			beta1 += (std::pow(x[obj1_dim_inx[i]] - (0.3 * x[0]*x[0] * std::cos(24 * OFEC_PI * x[0] + 4.*(1. + obj1_dim_inx[i]) * OFEC_PI / m_number_variables) + 0.6 * x[0]) * std::cos(6 * OFEC_PI * x[0] + (1. + obj1_dim_inx[i]) * OFEC_PI / m_number_variables), 2));
		}
		for (size_t i = 0; i < obj2_dim_inx.size(); ++i) {
			beta2 += (std::pow(x[obj2_dim_inx[i]] - (0.3 * x[0] * x[0] * std::cos(24 * OFEC_PI * x[0] + 4. * (1. + obj2_dim_inx[i]) * OFEC_PI / m_number_variables)+0.6*x[0]) * std::sin(6 * OFEC_PI * x[0] + (1. + obj2_dim_inx[i]) * OFEC_PI / m_number_variables), 2));
		}
...
//#endif // 
	...
	void MOEADDE_F5::sampleParetoSols(size_t sample_num) {
