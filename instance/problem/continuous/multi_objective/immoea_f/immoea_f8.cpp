...
namespace ofec {
	void IMMOEA_F8::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real g = 1.;
		Real sum = 0.;
		for (size_t i = 2; i < m_number_variables; ++i) {
			Real temp = std::pow(std::pow(x[i], 1. / (1. + m_beta * (i + 1.) / m_number_variables)) - x[0], 2);
			sum += temp;
		}
		g = sum;
	...
	void IMMOEA_F8::sampleParetoSols(size_t sample_num) {
			...
			for (size_t j = 0; j < div_num; j++) {
				...
				for (size_t k = 0; k < m_number_variables; k++) {
					if (k == 0) {
						...
						temp_var[k] = std::pow(temp_var[0], 1. + m_beta * (j + 1.) / m_number_variables);
					}
				}
