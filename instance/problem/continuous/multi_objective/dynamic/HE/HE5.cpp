...
namespace ofec {
	...
	int HE5::updateEvaluationTag(SolutionBase& s, Algorithm *alg) {
	...
	void HE5::evaluateObjective(Real* x, std::vector<Real>& obj) {//recommend the number of variables is 20
		Real t = get_time();
			...
			if (m < 1) {
				if (m_number_variables > 2) {
					...
					for (size_t i = 2; i < m_number_variables; i += 2) {
						obj[m] += pow(x[i] - 0.8 * x[0] * cos(6 * OFEC_PI * x[0] + (i + 1) * OFEC_PI / m_number_variables), 2);
					}
