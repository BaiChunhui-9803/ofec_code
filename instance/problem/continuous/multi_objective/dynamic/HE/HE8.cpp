...
namespace ofec {
	...
	int HE8::updateEvaluationTag(SolutionBase& s, Algorithm *alg) {
	...
	void HE8::evaluateObjective(Real* x, std::vector<Real>& obj) {//recommend the number of variables is 20
		Real t = get_time();
			...
			if (m < 1) {
				if (m_number_variables > 2) {
					...
					for (size_t i = 2; i < m_number_variables; i += 2) {
						...
						obj[m] += 4 * pow(y, 2) - cos(8 * y * OFEC_PI) + 1;
					}
				...
				for (size_t i = 1; i < m_number_variables; i += 2) {
					Real y = x[i] - pow(x[0], 0.5 * (1 + 3. * (i - 1) / (m_number_variables - 2)));
					gt += 4 * pow(y, 2) - cos(8 * y * OFEC_PI) + 1;
				}
