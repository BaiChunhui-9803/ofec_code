...
namespace ofec {
	...
	int JY7::updateEvaluationTag(SolutionBase& s, Algorithm *alg) {
	...
	void JY7::evaluateObjective(Real *x, std::vector<Real> &obj) {
		Real t = get_time();
		...
		obj[1] = (1 + gt) * pow(1 - x[0] + At * sin(Wt * OFEC_PI * x[0]), alpha_t);
	}
}