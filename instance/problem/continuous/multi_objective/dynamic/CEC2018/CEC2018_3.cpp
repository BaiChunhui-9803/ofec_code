...
namespace ofec {
	...
	void CEC2018_3::generateAdLoadPF() {
		int num = 2000;
		Real t = get_time();
		Real temp = 1.5 + sin(0.5*OFEC_PI*t);
		Matrix o_point(num, 2);
		for (int i = 0; i < num; i++) {
			o_point[i][0] = 0 + i * 1. / (num - 1);
			o_point[i][1] = 1 - pow(o_point[i][0], temp);
			m_optima->appendObj(o_point[i].vect());
		}
		m_optima->setVariableGiven(true);//given optima
	}

	 int CEC2018_3::updateEvaluationTag(SolutionBase& s, Algorithm *alg) {
	...
	void CEC2018_3::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real t = get_time();
