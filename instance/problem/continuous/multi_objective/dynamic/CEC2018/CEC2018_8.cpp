...
namespace ofec {
	...
	void CEC2018_8::generateAdLoadPF() {
		int num = 2000;
		Real t = get_time();
		Real alpha_t= 2.25 + 2 * cos(2 * OFEC_PI*t);
		std::vector<std::vector<Real>*> point(num);
		Matrix o_point(num, 2);
		Real x;
		for (int i = 0; i < num; i++) {
			x= i * 1. / (num - 1);
			o_point[i][0] = x+0.1*sin(3*OFEC_PI*x);
			o_point[i][1] = pow((1-x + 0.1*sin(3 * OFEC_PI*x)),alpha_t);
	 ...
	 int CEC2018_8::updateEvaluationTag(SolutionBase& s, Algorithm *alg) {
	...
	void CEC2018_8::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real t = get_time();
		...
		Real alpha_t = 2.25 + 2 * cos(2 * OFEC_PI*t);
		Real beta_t = 1;// 100 * Gt*Gt;
		for (size_t i = 1; i < m_number_variables; ++i)
		{
			gt += pow(x[i] - Gt*sin(4*OFEC_PI*pow(x[0],beta_t))/(1+fabs(Gt)), 2);
		}

		obj[0] = gt*(x[0]+0.1*sin(3*OFEC_PI*x[0]));
		obj[1] = gt * pow((1 -x[0]+ 0.1*sin(3*OFEC_PI*x[0])),alpha_t);
	}
}