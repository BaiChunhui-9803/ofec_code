...
namespace ofec {
	...
	int UDF8::updateEvaluationTag(SolutionBase& s, Algorithm *alg) {
	...
	void UDF8::evaluateObjective(Real *x, std::vector<Real> &obj) {//recommend the number of variables is 20
		Real t = get_time();
		...
		for (size_t m = 0; m < m_number_objectives; ++m) {
			if (m < 1) {
				if (m_number_variables < 3) {
					obj[m] = x[0] + fabs(sin(0.5 * OFEC_PI * m_count[2]));
				}
				else {
					obj[m] = 0;
					for (size_t i = 2; i < m_number_variables; i += 2) {
						obj[m] += pow(x[i] - sin(6 * OFEC_PI * x[0] + \
							(i + 1 + ceil(m_number_variables * sin(0.5 * OFEC_PI * m_count[0]))) * OFEC_PI / m_number_variables) - sin(0.5 * OFEC_PI * m_count[2]), 2);
					}
					obj[m] = x[0] + fabs(sin(0.5 * OFEC_PI * m_count[2])) + 2. / (ceil(m_number_variables / 2.) - 1) * obj[m];
				}
			}
			else {
				obj[m] = 0;
				for (size_t i = 1; i < m_number_variables; i += 2) {
					obj[m] += pow(x[i] - sin(6 * OFEC_PI * x[0] + \
						(i + 1 + ceil(m_number_variables * sin(0.5 * OFEC_PI * m_count[0]))) * OFEC_PI / m_number_variables) - sin(0.5 * OFEC_PI * m_count[1]), 2);
				}
				obj[m] = 1 - (0.5 + fabs(sin(0.5 * OFEC_PI * m_count[3]))) * pow(x[0], 0.5 + \
					fabs(sin(0.5 * OFEC_PI * m_count[4]))) + fabs(sin(0.5 * OFEC_PI * m_count[2])) + 2. / (floor(m_number_variables / 2.)) * obj[m];
			}
		}
	}
}