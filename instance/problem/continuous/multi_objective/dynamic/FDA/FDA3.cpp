#include "FDA3.h"
#include "../../../../../../core/algorithm/algorithm.h"

#ifdef OFEC_DEMO
#include <core/global_ui.h>
#endif

namespace ofec {
	void FDA3::initialize_() {
		DMOPs::initialize_();
		for (size_t i = 0; i < m_number_variables; ++i) {//the x1 is in (0,1), others are in (-1,1)
			if (i < 5)//recommend the number of x1 is 5, x2 is 25
				m_domain.setRange(0., 1., i);
			else
				m_domain.setRange(-1., 1., i);
		}
		generateAdLoadPF();
	}

	//f1 =\sum(xi^Ft), f2 = g * h, f2 =(1+Gt)*(1 -sqrt(f1/(1+Gt))), f1 = [0, 5]
	void FDA3::generateAdLoadPF() {
		int num = 2000;
		Real t = get_time();
		Real temp = 1 + std::fabs(std::sin(0.5*OFEC_PI*t));
		Matrix o_point(num, 2);
		for (int i = 0; i < num; i++) {
			o_point[i][0] = 0 + i * ceil(m_number_variables/6.) / (num-1);
			o_point[i][1] =temp*(1-std::sqrt(o_point[i][0]/temp));
			m_optima->appendObj(o_point[i].vect());
		}
	}

	int FDA3::updateEvaluationTag(SolutionBase& s, Algorithm *alg) {
		size_t eval = alg->evaluations();
		set_effective_eval(eval);
		if (alg != nullptr && eval % get_change_fre() == 0 && eval > 0) {
#ifdef OFEC_DEMO
			ofec_demo::g_buffer->appendProBuffer(this);
#endif
			return kChangeNextEval;
	}
		else if (alg != nullptr && (eval + 1) % get_change_fre() == 0 && eval > 0) {
			//m_optima.reset(new Optima<>());
			set_updated_state(true);
			return kNormalEval;
		}
		else {
			return Continuous::updateEvaluationTag(s, alg);
		}
}

	//f1 =\sum(xi^Ft), f2 = g * h, f2 =(1+Gt)*(1 -sqrt(f1/(1+Gt))), f1 = [0, 5]
	void FDA3::evaluateObjective(Real *x, std::vector<Real> &obj) {
		Real t = get_time();
		if (time_changed() && t != 0. && get_updated_state()) {
			m_optima.reset(new Optima<>());
			generateAdLoadPF();
			set_updated_state(false);
#ifdef OFEC_DEMO
			ofec_demo::g_buffer->appendProBuffer(this);
#endif
		}
		Real Gt = std::fabs(std::sin(0.5*OFEC_PI*t));
		Real Ft = std::pow(10, 2 * std::sin(0.5*OFEC_PI*t));
		Real gt = 1 + Gt;
		for (size_t i = ceil(m_number_variables / 6.); i < m_number_variables; ++i) {//variable group, ratio is 1:5
			gt += std::pow(x[i] - Gt, 2);
		}
		for (size_t m = 0; m < m_number_objectives; ++m)
		{
			if (m < 1) {
				obj[m] = 0;
				for (size_t i = 0; i < ceil(m_number_variables / 6.); ++i) {
					obj[m] += std::pow(x[i], Ft);
				}
			}
			else
				obj[m] = gt * (1 - std::pow(obj[0] / gt, 1./2));
		}
	}
}