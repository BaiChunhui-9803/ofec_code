...
namespace ofec::DMOP {
	void F7::initialize_() {
		DMOPs::initialize_();
		for (size_t i = 0; i < m_number_variables; ++i) {//all variable is in [0,5], recommend n=20
	...
	int F7::updateEvaluationTag(SolutionBase& s, Algorithm *alg) {
	...
	//f1=x1, f2=g*h
	void F7::evaluateObjective(Real *x, std::vector<Real> &obj) {
		Real t = get_time();
