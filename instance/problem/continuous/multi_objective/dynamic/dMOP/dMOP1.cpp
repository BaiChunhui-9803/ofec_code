...
namespace ofec {
	void dMOP1::initialize_() {//all variables are in [0,1], recommend the number of x is 10
		DMOPs::initialize_();
		generateAdLoadPF();
	}
	...
	int dMOP1::updateEvaluationTag(SolutionBase& s, Algorithm *alg) {
	...
	void dMOP1::evaluateObjective(Real *x, std::vector<Real> &obj) {
		Real t = get_time();
