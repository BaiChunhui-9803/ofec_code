...
namespace ofec {
	SDCMOP::SDCMOP(const ParameterMap &v) :SDCMOP(v.at("problem name"), v.at("number of variables"), v.at("numObj"), v.at("testItems"), v.at("numPeak")) {
	...
	void SDCMOP::initialize() {
		...
		m_constraint_type.resize(m_number_objectives-1);
	...
	void SDCMOP::evaluate_obj_nd_con(Real *x, std::vector<Real>& obj, std::vector<Real> &con) {
		...
		obj[m_number_objectives - 1] = g + (1 - m_F / std::pow(2, g) - m_offset);
