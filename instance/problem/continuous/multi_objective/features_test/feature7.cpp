...
namespace ofec {
	void Feature7::initialize_() {
		...
		for (size_t i = 0; i < peak_num; ++i) {
			...
			for (size_t j = 0; j < m_number_variables; ++j) {
				temp_p[j] = 1.8 * GET_RND(m_random.get()).uniform.next() - 0.9;//位置随机
			}
		...
		for (size_t i = 0; i < m_number_objectives; ++i) {
			...
			for (size_t j = 0; j < m_peak_num; ++j) {
				Real h = m_max_height * (0.5 + 0.5 * GET_RND(m_random.get()).uniform.next());
				Real w = 0.;
				std::vector<Real> peak_x;
				peak_x = m_total_peaks_pos[j][0];
				
				for (size_t k = 0; k < peak_x.size(); ++k) {
					peak_x[k] = peak_x[k] - (GET_RND(m_random.get()).uniform.next()>0.5?1:-1)*0.1 * i;
				}
	...
	void Feature7::generateAdLoadPF() {
		size_t num = 1000;
	...
	void Feature7::evaluateObjective(Real* x, std::vector<Real>& obj) {
