...
namespace ofec {
	...
	*/
		...
		for (size_t i = 0; i < num; ++i) {
			...
			for (size_t j = 0; j < m_number_variables; ++j) {
				if (j == m_number_variables - 1) {
					...
					temp_p[j] = 1.6 * GET_RND(m_random.get()).uniform.next() - 0.8;//位置随机
				}
			}
			...
			for (size_t j = 0; j < m_peak_num; ++j) {
				...
				if (i > 0) {
					for (size_t k = 0; k < m_number_variables; ++k) {
						if (k == m_number_variables - 1) {
							peak_x[k] = std::fabs(m_total_peaks_pos[j][0][0]);
						}
						else {
							if (m_total_peaks_pos[j][0][0] > 0) {
								peak_x[k] = 1;
							}
							else {
								peak_x[k] = -1;
	...
	void Feature8::generateAdLoadPF() {
		size_t num = 500;
	...
	void Feature8::evaluateObjective(Real* x, std::vector<Real>& obj) {
