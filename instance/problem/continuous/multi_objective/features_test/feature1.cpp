...
namespace ofec {
	...
	*/
			...
			for (size_t j = 0; j < m_peak_num; ++j) {
				...
				if (i > 0) {
					for (size_t k = 0; k < peak_x.size()-1; ++k) {
						peak_x[k] = peak_x[k] - 0.2*GET_RND(m_random.get()).uniform.next() * i;//随机长度
					}
				}
	...
	void Feature1::generateAdLoadPF() {
		size_t num = 500;
	...
	void Feature1::evaluateObjective(Real* x, std::vector<Real>& obj) {
