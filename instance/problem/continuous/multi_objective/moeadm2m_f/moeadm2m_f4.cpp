#include "moeadm2m_f4.h"

namespace ofec {
	void MOEADM2M_F4::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real sum = 0.;
		...
		obj[0] = (1 + g) * x[0];
		obj[1] = (1 + g) * (1-std::pow(x[0],0.5) * std::pow(std::cos(2 * OFEC_PI * x[0]), 2));
	}

	void MOEADM2M_F4::updateOptima() {
	...
	void MOEADM2M_F4::sampleParetoSols(size_t sample_num) {
