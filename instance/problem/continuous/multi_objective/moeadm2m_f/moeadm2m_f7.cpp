#include "moeadm2m_f7.h"

namespace ofec {
	void MOEADM2M_F7::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real sum = 0.;
		for (size_t i = 2; i < m_number_variables; ++i) {
			Real t = x[i] - x[0] * x[1];
			sum = sum + std::pow(std::fabs(t), 0.6) - 0.9 * std::pow(t, 2);
		}
		Real g = 2 * std::sin(OFEC_PI * x[0]) * sum;

		obj[0] = (1 + g) * std::cos(0.5 * OFEC_PI * x[0]) * std::cos(0.5 * OFEC_PI * x[1]);
		obj[1] = (1 + g) * std::cos(0.5 * OFEC_PI * x[0]) * std::sin(0.5 * OFEC_PI * x[1]);
		obj[2] = (1 + g) * std::sin(0.5 * OFEC_PI * x[0]);
	}

	void MOEADM2M_F7::updateOptima() {
	...
	void MOEADM2M_F7::sampleParetoSols(size_t sample_num) {
