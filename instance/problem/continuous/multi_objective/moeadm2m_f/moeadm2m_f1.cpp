#include "moeadm2m_f1.h"

namespace ofec {
	void MOEADM2M_F1::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real sum = 0.;
		for (size_t i = 1; i < m_number_variables; ++i) {
			Real t = x[i] - std::sin(0.5*OFEC_PI*x[0]);
			sum = sum+std::pow(std::fabs(t), 0.6) - 0.9 * std::pow(t, 2);
		}
		Real g = 2*std::sin(0.5*OFEC_PI*x[0]) * sum;

		obj[0] = (1+g) * x[0];
		obj[1] = (1+g) * (1 - std::sqrt(x[0]));
	}

	void MOEADM2M_F1::updateOptima() {
...
//#endif //
	...
	void MOEADM2M_F1::sampleParetoSols(size_t sample_num) {
