#include "moeadm2m_f3.h"

namespace ofec {
	void MOEADM2M_F3::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real sum = 0.;
		for (size_t i = 1; i < m_number_variables; ++i) {
			Real t = x[i] - std::sin(0.5 * OFEC_PI * x[0]);
			sum = sum + std::fabs(t) / (1 + std::exp(5 * std::fabs(t)));
		}
		Real g = 10 * std::sin(0.5*OFEC_PI * x[0]) * sum;

		obj[0] = (1 + g) * std::cos(0.5 * OFEC_PI * x[0]);
		obj[1] = (1 + g) * std::sin(0.5 * OFEC_PI * x[0]);
	}

	void MOEADM2M_F3::updateOptima() {
	...
	void MOEADM2M_F3::sampleParetoSols(size_t sample_num) {
