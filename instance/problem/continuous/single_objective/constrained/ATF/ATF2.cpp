...
namespace ofec {
	namespace ATF {
		...
		void _2::evaluateObjectiveAndConstraint(Real* x, std::vector<Real>& obj, std::vector<Real>& con) {
			
			obj[0] = x[0]*x[0]-10*cos(2*3.14*x[0])+x[1]*x[1]-10*cos(2*3.14*x[1])+20;

			con[0] = 3 * (x[0] + 7) * (x[0] + 7) + x[1] * x[1] - 0.3;
			if(((x[0] + 8) * (x[0] + 8) + (x[1] - 3) * (x[1] - 3) - 2) < con[0])
				con[0] = (x[0] + 8) * (x[0] + 8) + (x[1] - 3) * (x[1] - 3) - 2;
