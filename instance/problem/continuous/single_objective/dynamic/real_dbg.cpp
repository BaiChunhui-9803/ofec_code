...
namespace ofec {
		...
		,  int peakNumChangeMode,  bool flagNoise,  bool flagTimelinkage) :dynamic_continuous(name, rDimNumber, rNumPeaks, numObj)
	{
		...
		for (auto i = 0; i < m_peak.size(); i++) {
			m_rotM[i].resize(m_number_variables, m_number_variables);
		}
		setDomain(-5, 5);
		m_max_height = 100;
		m_min_height = 10;
		...
		std::vector<std::vector<Real>> position(m_peak.size(),std::vector<Real>(m_number_variables));
	...
	void real_DBG::set_period( int p){
		...
		for (int i = 0; i < m_period; i++) {
			...
			for (int j = 0; j < m_peak.size(); j++)
				m_rotP[i][j].resize(m_number_variables);
		}	
	}
	...
	void real_DBG::change_location(Real angle) {
		...
		std::vector<Real> gene (m_number_variables);
		for (int i = 0; i < m_peak.size(); i++) {
			...
			m.set_row(m_peak[i].data(), m_number_variables);
			m *= m_rotM[i];
			gene = m[0].vect();
			repair(gene.data());
			m_peak[i].vect() = gene;
