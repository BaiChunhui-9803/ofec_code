...
namespace ofec {
	namespace cec2005 {
		void RotatedHybridComposition::addInputParameters() {
			m_index_number = "16";
		}

		void RotatedHybridComposition::evaluateObjective(Real *x, std::vector<Real> &obj) const {
			Composition::evaluateObjective(x, obj);
			obj[0] += 120.; // add m_bias
		}

		void RotatedHybridComposition::setFunction(Environment *env) {
			m_num_function = 10;
		...
		void RotatedHybridComposition::updateOptima(Environment *env) {
