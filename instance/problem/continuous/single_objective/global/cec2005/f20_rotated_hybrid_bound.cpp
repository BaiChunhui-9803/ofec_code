...
namespace ofec::cec2005 {
	void RotatedHybridBound::addInputParameters() {
		m_index_number = "20";
	}
	...
	void RotatedHybridBound::evaluateObjective(Real *x, std::vector<Real> &obj) const {
		Composition::evaluateObjective(x, obj);
		obj[0] += 10.;
	}

	void RotatedHybridBound::setFunction(Environment *env) {
		m_num_function = 10;
	...
	void RotatedHybridBound::updateOptima(Environment *env) {
