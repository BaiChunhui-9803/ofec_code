...
namespace ofec::cec2005 {
	void HybridCompositionNoisy::addInputParameters() {
		m_index_number = "17";
	}

	void HybridCompositionNoisy::evaluateObjective(Real *x, std::vector<Real> &obj) const {
		Composition::evaluateObjective(x, obj);
		obj[0] = obj[0] * noise() + 120.;
	}

	Real HybridCompositionNoisy::noise() const {
		return (1 + 0.2 * fabs(m_random->normal.next()));
	}

	void HybridCompositionNoisy::setFunction(Environment *env) {
		m_num_function = 10;
	...
	void HybridCompositionNoisy::updateOptima(Environment *env) {
