...
namespace ofec::cec2005 {
	void RotatedHybridHighConditionNumberMatrix::addInputParameters() {
		m_index_number = "22";
	}

	void RotatedHybridHighConditionNumberMatrix::evaluateObjective(Real *x, std::vector<Real> &obj) const {
		Composition::evaluateObjective(x, obj);
		obj[0] += 360.;
	}

	void RotatedHybridHighConditionNumberMatrix::setFunction(Environment *env) {
		m_num_function = 10;
		...
		m_function[0]->loadRotation("/instance/problem/continuous/single_objective/global/classical/GOP_CEC2005_F22_part1");
		m_function[1]->loadRotation("/instance/problem/continuous/single_objective/global/classical/GOP_CEC2005_F22_part2");
	...
	void RotatedHybridHighConditionNumberMatrix::updateOptima(Environment *env) {
