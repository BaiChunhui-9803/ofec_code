...
namespace ofec {
	...
	void SphereNoisy::updateOptima(Environment *env) {
		m_optima.reset(new Optima<>(m_original_optima));
		m_objective_accuracy = 1e-8;
	}

	void SphereNoisy::evaluateOriginalObj(Real *x, std::vector<Real> &obj) const {
		Real fit = 0;
		Real noise;
		for (int i = 0; i < m_number_variables; i++) {
			if (initialized()) {
				noise = 0.01 * m_random->uniform.next();
