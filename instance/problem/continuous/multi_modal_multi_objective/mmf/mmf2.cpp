...
namespace ofec {
	...
	void MMF2::loadParetoFront(size_t sample_num) {
		...
		if (!infile.is_open())
		{
			throw MyExcept("open PF file of MMF2 problem is fail");
		}
		std::string str;
		size_t line = 0;
	...
	void MMF2::sampleParetoFront(size_t sample_num) {
	...
	std::vector<Real> MMF2::createVar(const std::vector<Real>& s) const {
		//根据不同的问题，构造一个最优解
		...
		if (new_var[1] >= 0 && new_var[1] <= 1) {
			...
			new_var[0] = std::pow(new_var[1]-1, 2);
		}

		return new_var;
	}

	void MMF2::sampleParetoSets(size_t sample_num) {
		size_t num = sample_num;
		...
		for (size_t i = 0; i < num; i++) {
			...
			for (size_t j = 0; j < m_number_variables; j++) {
				if (j == 1) {
					temp_var[j] = 2.*(Real)i / (num - 1.);
				}
				else {
					if (temp_var[1] >= 0 && temp_var[1] <= 1) {
						temp_var[j] = std::pow(temp_var[1],2);
					}
					else {
						temp_var[j] = std::pow(temp_var[1]-1, 2);
					}
				}
			}
	...
	void MMF2::evaluateObjective(Real* x, std::vector<Real>& obj) {
		...
		if (x[1] >= 0 && x[1] <= 1) {
			...
			obj[1] = 1 - std::sqrt(x[0]) + 2 * (4 * std::pow(x[1] - 1 - std::sqrt(x[0]), 2) - std::cos(20 * (x[1] - 1 - std::sqrt(x[0])) * OFEC_PI / std::sqrt(2)) + 2);
		}
	}
}