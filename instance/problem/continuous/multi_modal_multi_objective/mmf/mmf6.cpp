...
namespace ofec {
	void MMF6::initialize_() {
		Continuous::initialize_();
		auto& v = *m_param;
		resizeVariable(v.get<int>("number of variables"));
		m_domain.setRange(1., 3., 0);
		m_domain.setRange(1., 2., 1);
	...
	void MMF6::loadParetoFront(size_t sample_num) {
		...
		if (!infile.is_open())
		{
			throw MyExcept("open PF file of MMF6 problem is fail");
		}
		std::string str;
		size_t line = 0;
	...
	void MMF6::sampleParetoFront(size_t sample_num) {
	...
	std::vector<Real> MMF6::createVar(const std::vector<Real>& s) const {
		//根据不同的问题，构造一个最优解
		...
		if (new_var[1] >= -1 && new_var[1] <= 1) {
			new_var[1] = std::sin(6 * OFEC_PI * std::fabs(new_var[0] - 2) + OFEC_PI);
		}
		else {
			new_var[1] = 1 + std::sin(6 * OFEC_PI * std::fabs(new_var[0] - 2) + OFEC_PI);
		}

		return new_var;
	}

	void MMF6::sampleParetoSets(size_t sample_num) {
		...
		for (size_t i = 0; i < num; i++) {
			...
			for (size_t j = 0; j < m_number_variables; j++) {
				if (j == 0) {
					temp_var[j] = 1+2 * (Real)i / (num - 1.);//x1=-1,1
				}
				else {
					temp_var[j] = 1 + std::sin(6 * OFEC_PI * std::fabs(temp_var[0] - 2) + OFEC_PI);
				}
			}
			all_vars.emplace_back(temp_var);
		}
	...
	void MMF6::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real g = std::fabs(x[0] - 2);

		obj[0] = g;
		if (x[1] >= -1 && x[1] < 1) {
			obj[1] = 1 - std::pow(g, 0.5) + 2 * std::pow(x[1] - std::sin(6 * OFEC_PI * std::fabs(x[0] - 2) + OFEC_PI), 2);
		}
		else {
			obj[1] = 1 - std::pow(g, 0.5) + 2 * std::pow(x[1] - 1 - std::sin(6 * OFEC_PI * std::fabs(x[0] - 2) + OFEC_PI), 2);
		}
