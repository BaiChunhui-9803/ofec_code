...
namespace ofec {
	...
	void MMF12::loadParetoFront(size_t sample_num) {
		...
		if (!infile.is_open())
		{
			throw MyExcept("open PF file of MMF12 problem is fail");
		}
		std::string str;
		size_t line = 0;
			...
			for (size_t i = 0; i < num; i++) {
				...
				for (size_t j = 0; j < m_number_objectives; j++) {
					if (j == 0)
						...
						temp_obj[j] = g- temp_obj[0]*temp_obj[0]/g- temp_obj[0]*std::sin(2*m_num_pf*OFEC_PI*temp_obj[0]);
				}
				one_layer_objs.emplace_back(temp_obj);
			}
			std::vector<std::vector<Real>*> objs;
			for (size_t i = 0; i < all_objs.size(); ++i) {
				objs.emplace_back(&one_layer_objs[i]);
			}
			std::vector<int> rank;
			ofec::nd_sort::fastSort<Real>(objs, rank, m_optimize_mode);
			for (size_t i = 0; i < rank.size(); ++i) {
				if (rank[i] == 0) {
					all_objs.emplace_back(one_layer_objs[i]);
				}
			}
		}
	...
	std::vector<Real> MMF12::createVar(const std::vector<Real>& s) const {
		//根据不同的问题，构造一个最优解
	...
	void MMF12::sampleParetoSets(size_t sample_num) {
		size_t num = sample_num / m_num_ps;
	...
	void MMF12::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real g = 2 - std::exp(-2 * std::log(2) * std::pow((x[1] - 0.1) / 0.8, 2)) * std::pow(std::sin(m_num_ps * OFEC_PI * x[1]), 6);

		obj[0] = x[0];
		obj[1] = g - obj[0] * obj[0] / g - obj[0] * std::sin(2 * m_num_pf * OFEC_PI * obj[0]);
