...
namespace ofec {
	...
	void MMF9::loadParetoFront(size_t sample_num) {
		...
		if (!infile.is_open())
		{
			throw MyExcept("open PF file of MMF9 problem is fail");
		}
		std::string str;
		size_t line = 0;
	...
	void MMF9::sampleParetoFront(size_t sample_num) {
		size_t num = sample_num;
		std::vector<std::vector<Real>> all_objs;
		Real g = 2 - std::pow(std::sin(OFEC_PI/2), 6);
	...
	std::vector<Real> MMF9::createVar(const std::vector<Real>& s) const {
		//根据不同的问题，构造一个最优解
		...
		new_var[1] = 0.5 / m_num_global_ps + 1. / m_num_global_ps;

		return new_var;
	}

	void MMF9::sampleParetoSets(size_t sample_num) {
		size_t num = sample_num / m_num_global_ps;
		std::vector<VariableVector<Real>> all_vars;
		Real pi = OFEC_PI;
		for (size_t k = 0; k < m_num_global_ps; ++k) {
			for (size_t i = 0; i < num; i++) {
				...
				for (size_t j = 0; j < m_number_variables; j++) {
					if (j == 0) {
						...
						temp_var[j] = 0.5 / m_num_global_ps + 1. / m_num_global_ps * k;
					}
				}
	...
	void MMF9::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real g = 2 - std::pow(std::sin(m_num_global_ps*OFEC_PI*x[1]), 6);
