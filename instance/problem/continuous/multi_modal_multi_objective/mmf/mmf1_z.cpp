...
namespace ofec {
	...
	void MMF1_z::loadParetoFront(size_t sample_num) {
		...
		if (!infile.is_open())
		{
			throw MyExcept("open PF file of MMF1_z problem is fail");
		}
		std::string str;
		size_t line = 0;
	...
	void MMF1_z::sampleParetoFront(size_t sample_num) {
	...
	std::vector<Real> MMF1_z::createVar(const std::vector<Real>& s) const {
		//根据不同的问题，构造一个最优解
		...
		if (new_var[0] >= 1 && new_var[0] < 2) {
			new_var[1] = std::sin(2 * k * OFEC_PI * std::fabs(new_var[0] - 2.) + OFEC_PI);
		}
		else {
			new_var[1] = std::sin(2 * OFEC_PI * std::fabs(new_var[0] - 2.) + OFEC_PI);
		}

		return new_var;
	}

	void MMF1_z::sampleParetoSets(size_t sample_num) {
		size_t num = sample_num;
			...
			for (size_t j = 0; j < m_number_variables; j++) {
				if (j == 0) {
					...
					if (temp_var[0] >= 1 && temp_var[0] < 2) {
						temp_var[j] = std::sin(2 * k * OFEC_PI * std::fabs(temp_var[0] - 2.) + OFEC_PI);
					}
					else {
						temp_var[j] = std::sin(2 * OFEC_PI * std::fabs(temp_var[0] - 2.) + OFEC_PI);
					}
				}	
			}
	...
	void MMF1_z::evaluateObjective(Real* x, std::vector<Real>& obj) {
		Real g = std::fabs(x[0] - 2.);
		size_t k = 3;
		obj[0] = g;
		if (x[0] >= 1 && x[0] < 2) {
			obj[1] = 1 - std::sqrt(g) + 2 * std::pow(x[1] - std::sin(2 * k * OFEC_PI * g + OFEC_PI), 2);
		}
		else {
			obj[1] = 1 - std::sqrt(g) + 2 * std::pow(x[1] - std::sin(2 * OFEC_PI * g + OFEC_PI), 2);
	    }
}