...
namespace ofec {
	...
	void MMF1::loadParetoFront(size_t sample_num) {
		...
		if (!infile.is_open())
		{
			throw MyExcept("open PF file of MMF1 problem is fail");
		}
		std::string str;
		size_t line = 0;
	...
	void MMF1::sampleParetoFront(size_t sample_num) {
	...
	std::vector<Real> MMF1::createVar(const std::vector<Real>& s) const {
		//根据不同的问题，构造一个最优解
	...
	void MMF1::sampleParetoSets(size_t sample_num) {
		size_t num = sample_num;
	...
	void MMF1::evaluateObjective(Real* x, std::vector<Real>& obj) {
