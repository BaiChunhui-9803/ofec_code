...
*
*********************************************************************************/
#ifndef OFEC_DISTANCE_CALCULATOR_MAT_WEIGHT_H
#define OFEC_DISTANCE_CALCULATOR_MAT_WEIGHT_H
...
namespace ofec {
	...
	class DistanceCalculatorSeqWeight :public DistanceCalculatorBase<TSolution> {
	public:
	...
	public:
		...
		virtual int updateMemory(
			...
			m_Emax_radius = m_num_var * (max_val+m_fitness_weight.getMaxWeight());
			m_Emin_radius = m_num_var * m_fitness_weight.getMinWeight();
		...
		double disBetweenInds(const SolutionType& sol1,const SolutionType& sol2)const {
			
			static std::vector<std::vector<bool>> temp_mat;

			double fitness1(sol1.fitness()), fitness2(sol2.fitness());
			fitness1 = m_fitness_weight.getPopFitness(fitness1)/2.0;
			fitness2 = m_fitness_weight.getPopFitness(fitness2)/2.0;
			...
			totalFit += (fitness1 + fitness2);
			return  m_Emax_radius - totalFit;
		}
		virtual double disToPop(const SolutionType& sol) const override {
			double popFit(sol.fitness());
			popFit = m_fitness_weight.getPopFitness(popFit);
			double dis(0);
