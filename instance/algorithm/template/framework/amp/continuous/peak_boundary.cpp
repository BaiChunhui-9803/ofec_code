...
namespace ofec {
	PeakBoundary::PeakBoundary(int num_vars, int number_objectives) :
	...
	PeakBoundary::PeakBoundary(const Solution& s, Problem *pro, Algorithm *alg, Random *rnd, Real startRadi, Real step) :
		Solution<>(s), 
		m_radius(0), 
	...
	Real PeakBoundary::getDeratingObj(Problem *pro, Algorithm *alg) {
	...
	Real PeakBoundary::getRefRadi(const Solution<>& s, Problem *pro, Algorithm *alg, Random *rnd) {
		if (!m_ready) return 0; 
	...
	void PeakBoundary::generateSample(Problem *pro, Algorithm *alg, Random *rnd) {
	...
	void PeakBoundary::creatOneSample(const Vector& vnor, Problem *pro, Algorithm *alg, Random *rnd) {
		Vector vtr(vnor);
	...
	void PeakBoundary::binarySearch(Solution<>& s0, Solution<>& s1, Problem *pro, Algorithm *alg) {
		Solution<> s(s0);
