...
namespace LKH {
	...
	Start:
		...
		if (MaxCandidates > 0) {
			...
			MaxAlpha = std::numeric_limits<GainType>::max();
			if (Optimum != std::numeric_limits<GainType>::min()
			...
			for (P = 1; T && P <= Period && Norm != 0; P++) {
				...
				do {
					if (t->V != 0) {
						...
						if (t->Pi > std::numeric_limits<int>::max() / 10)
							t->Pi = std::numeric_limits<int>::max() / 10;
						else if (t->Pi < std::numeric_limits<int>::min() / 10)
							t->Pi = std::numeric_limits<int>::min() / 10;
					}
					t->LastV = t->V;
				} while ((t = t->Suc) != FirstNode);
