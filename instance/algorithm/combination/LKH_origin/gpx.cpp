...
thread_local LKHAlg::Node **Map2Node=0;
...
{
    LKHAlg::Node *a = Map2Node[i], *b = Map2Node[j];
...
static thread_local int **M_neigh2;          // neighbourhood matrix: the collumns indicate the
                       ...
                       // in collumns 2 and 3
thread_local int *test;                      // test of the candidtes: 1 - true component; 0 - otherwise
...
void new_candidates(int *vector_comp, int n_new)
{
    ...
    // label 0
    n_cand++;
    size_ = new_int(n_cand);     // size of each component
    id = new_int(n_);
...
void findInputs(int *sol_blue, int *sol_red)
{
    ...
    for (i = 0; i < n_cand; i++) {
        comp_size = (int) ceil(size_[i] / 2);    // the inputs/outputs are created
        // with maximum size
    ...
    sol_blue_reduc = alloc_vectori(n_);
    sol_red_reduc = alloc_vectori(n_);
    sol_blue_reduc_t = alloc_vectori(n_);
    sol_red_reduc_t = alloc_vectori(n_);
    j = k = 0;
    for (i = 0; i < n_; i++) {
        aux = sol_blue[i];
        if (size_[id[aux]] > 1) {
...
int testUnfeasibleComp(int *sol_blue)
{
    ...
    comp_seq = alloc_vectori(n_);        // sequence of all entries in unfeasible
    ...
    j = 0;      // j is the effective size of comp_seq
    aux = id[sol_blue[0]];
    if (test[aux] == 0 && aux != id[sol_blue[n_ - 1]])
        comp_seq[j++] = aux;
    inp_comp_seq[aux] = 0;
    for (i = 1; i < n_; i++) {
        aux = id[sol_blue[i]];
...
void fusion(int *sol_blue, int *sol_red)
{
    ...
    for (cand = 0; cand < n_cand; cand++)
        neigh_vec_cond[cand] = test[cand] == 1 || size_[cand] <= 1 ? -1 : 0;
...
void fusionB(int *sol_blue, int *sol_red)
{
    ...
    // Memory allocation
    cand_seq = alloc_vectori(n_);        // list of entries and exits of
    // unfeasible candidates
    cand_seq_cut = alloc_vectori(n_);
    ...
    previous_cand = id[sol_blue[n_ - 1]];
    for (i = 0; i < n_; i++) {
        ...
        if (test[cand] == 0 && n_inputs[cand] > 0 && size_[cand] > 1) {
                 ...
                 int *label_list, LKHAlg *Alg)
{
    ...
    sol_blue_index = new_int(n_);
    sol_red_index = new_int(n_);
    for (i = 0; i < n_; i++) {
        sol_blue_index[sol_blue[i]] = i;
        sol_red_index[sol_red[i]] = i;
    }
    // Offspring: Graph
    Graph *G_offspring = new_Graph(n_);  // graph for offspring
    for (i = 0; i < n_; i++) {
        if (select_cand[id[i]] == 0) {
            if (sol_blue_index[i] == 0) {
                i_l = sol_blue[n_ - 1];
                i_h = sol_blue[sol_blue_index[i] + 1];
            } else if (sol_blue_index[i] == (n_ - 1)) {
