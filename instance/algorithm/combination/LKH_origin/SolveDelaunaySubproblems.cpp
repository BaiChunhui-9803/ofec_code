		...
		for (i = 0; i < Alg->Dimension; i++) {
			...
			do {
				...
				if (u->id < v->id) {
					...
					EdgeSet[j++].Cost = (LKH::LKHAlg::Fixed(From, To) || Alg->IsCommonEdge(From, To)) ? std::numeric_limits<int>::min() :
				...
				for (i = 0; i < Alg->Dimension; i++) {
					...
					do {
						...
						(&Key, EdgeSet, Count1, sizeof(Edge),
							...
							EdgeSet[Count].To = To;
							EdgeSet[Count].Cost =
								(LKH::LKHAlg::Fixed(From, To) || Alg->IsCommonEdge(From, To)) ? std::numeric_limits<int>::min() :
