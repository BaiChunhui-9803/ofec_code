...
namespace LKH {
	...
	static thread_local std::unique_ptr<int> CurrentCycle, Patchwork, RecLevel;
	...
	static GainType PatchCyclesRec(int k, int m, int M, GainType G0, LKHAlg *Alg)
	{
		...
		for (Ns2 = s2->CandidateSet; (s3 = Ns2->To); Ns2++) {
			...
			for (X4 = 1; X4 <= 2; X4++) {
				...
				if ((LKH::LKHAlg::Fixed(s3, s4) || Alg->IsCommonEdge(s3, s4)) || Deleted(s3, s4))
					continue;
				MarkDeleted(s3, s4);
				t[2 * (k + m)] = s4;
					...
					for (Ns4 = s4->CandidateSet; (s5 = Ns4->To); Ns4++) {
						...
						for (X6 = 1; X6 <= 2; X6++) {
							...
							if (s6 == s1 || Alg->Forbidden(s6, s1)
								|| (LKH::LKHAlg::Fixed(s5, s6) || Alg->IsCommonEdge(s5, s6))
								|| Deleted(s5, s6)
								|| Added(s6, s1))
								continue;
