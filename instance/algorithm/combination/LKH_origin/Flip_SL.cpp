...
#define SPLIT_CUTOFF 0.75
	...
	void LKHAlg::Flip_SL(Node * t1, Node * t2, Node * t3)
	{
		...
		if (m_flag_divideBySol) {
			updateBestSolInside(t1->Id, t2->Id,t3->Id, t4->Id);
	...
	void LKHAlg::updateBestSolInside(int t1, int t2, int t3, int t4) {
		using namespace ofec;
		GainType gain = 0;
		//t2->C[t3->Id] + t4->C[t1->Id] - (t1->C[t2->Id] + t3->C[t4->Id]);
		/*	CAST_TSP(m_problem.get())->cost()[t2- 1][t3- 1] + CAST_TSP(m_problem.get())->cost()[t4 - 1][t1 - 1]
			- CAST_TSP(m_problem.get())->cost()[t2- 1][t1- 1] - CAST_TSP(m_problem.get())->cost()[t4- 1][t3- 1];
	*/		//(this->*Distance)(t2, t3)+ (this->*Distance)(t4, t1) - ((this->*Distance)(t1, t2) + (this->*Distance)(t3, t4));
	//	Gain
		//GainType  gain = in
		bool is_update = false;
		m_curCost += gain;
		//update CurDis
		updateDiffDis(t1, t2, t3, t4);
		updateCurSolRecord(t1, t2, t3, t4);
		//
		//if(m_curSameEdge-m_max_radius>=0){
		//	if (m_curCost < m_bestCostInside) is_update = true;
		//	else if (m_curCost == m_bestCostInside && m_curSameEdge > m_bestSolSameEdge) is_update = true;
		//	
		//}
		//if (is_update&& ofec::TravellingSalesman::judgeCurSolLoop(m_curSolRecord)) {
		//	m_bestSolInsideRecord = m_curSolRecord;
		////	RecordCurTour(m_bestSolInside);
		//	m_bestCostInside = m_curCost;
		//	m_bestSolSameEdge = m_curSameEdge;


		//	if (ofec::TravellingSalesman::judgeCurSolLoop(m_curSolRecord)) {
		//		++m_swap_step;
		//		std::vector<int> initSol(m_test_initSol);
		//		std::vector<int> optSol(initSol);
		//		//getCurInsideSol(m_test_initSol, optSol);

		//		ofec::TravellingSalesman::transferEdgeSol(m_curSolRecord, optSol);
		//		for (auto& it : optSol)++it;

		//		// = m_bestSolInside;
		//		auto optCost = m_curCost;

		//		/*		{
		//					auto initCost = generateSol(initSol);
		//					auto cost = generateSol(optSol);
		//					if (cost != optCost) {
		//						std::cout << "differ at cost \t" << m_swap_step << std::endl;

		//					}
		//				}*/
		//		{
		//			for (auto& it : initSol) --it;
		//			for (auto& it : optSol) --it;
		//			//for(auto& it:sampe)
		//			std::vector<std::array<int, 2>> edgex;
		//			std::vector<std::array<int, 2>> edgey;
		//			ofec::TravellingSalesman::transferEdgeSol(initSol, edgex);
		//			ofec::TravellingSalesman::transferEdgeSol(optSol, edgey);


		//			if (int(ofec::TravellingSalesman::tspDis(edgex, edgey)) + m_curSameEdge != DimensionSaved) {
		//				std::cout << "error at tspDis \t" << m_swap_step << std::endl;
		//			}
		//		}

		//		//	//initDiffMatrix(m_test_initSol);
		//	}
		//}
		}
