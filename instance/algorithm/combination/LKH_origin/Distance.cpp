 ...
 */
namespace LKH {
	int LKHAlg::Distance_1(Node * Na, Node * Nb)
	{
		return 1;
	}

	int LKHAlg::Distance_LARGE(Node * Na, Node * Nb)
	{
		return 10000000;
	}

	int LKHAlg::Distance_Asymmetric(Node * Na, Node * Nb)
	{
		...
		if (abs(Na->Id - Nb->Id) == n)
			return 0;
		return Na->Id <= n ? (this->*(this->OldDistance))(Na, Nb - n) : (this->*(this->OldDistance))(Nb, Na - n);
	}

	int LKHAlg::Distance_ATSP(Node * Na, Node * Nb)
	{
		int n = DimensionSaved;
		if ((Na->Id <= n) == (Nb->Id <= n))
			return M;
...
#define RRR 6378.388
	...
	int LKHAlg::Distance_GEO(Node * Na, Node * Nb)
	{
		int deg;
...
#define M_RRR 6378388.0
	...
	int LKHAlg::Distance_GEOM(Node * Na, Node * Nb)
	{
	...
	int LKHAlg::Distance_MAN_2D(Node * Na, Node * Nb)
	{
		return (int)(Scale * (fabs(Na->X - Nb->X) + fabs(Na->Y - Nb->Y)) +
			0.5);
	}

	int LKHAlg::Distance_MAN_3D(Node * Na, Node * Nb)
	{
	...
	int LKHAlg::Distance_MAX_2D(Node * Na, Node * Nb)
	{
	...
	int LKHAlg::Distance_MAX_3D(Node * Na, Node * Nb)
	{
		int dx = (int)(Scale * fabs(Na->X - Nb->X) + 0.5),
			dy = (int)(Scale * fabs(Na->Y - Nb->Y) + 0.5),
	...
	*/
	...
	int LKHAlg::Distance_GEO_MEEUS(Node * Na, Node * Nb)
	{
		double lat1 =
	...
	int LKHAlg::Distance_GEOM_MEEUS(Node * Na, Node * Nb)
	{
...
#undef max
	...
	int LKHAlg::Distance_TOR_2D(Node * Na, Node * Nb)
	{
	...
	int LKHAlg::Distance_TOR_3D(Node * Na, Node * Nb)
	{
		double dx = fabs(Na->X - Nb->X);
		double dy = fabs(Na->Y - Nb->Y);
	...
	int LKHAlg::Distance_XRAY1(Node * Na, Node * Nb)
	{
		double distp =
	...
	int LKHAlg::Distance_XRAY2(Node * Na, Node * Nb)
	{
		double distp =
			min(fabs(Na->X - Nb->X), fabs(fabs(Na->X - Nb->X) - 360));
		double distc = fabs(Na->Y - Nb->Y);
