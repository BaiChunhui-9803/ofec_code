...
#undef max
	...
	void LKHAlg::OrderCandidateSet(int MaxCandidates, GainType MaxAlpha, int Symmetric)
	{
		...
		if (MaxAlpha < 0 || MaxAlpha > std::numeric_limits<int>::max())
			MaxAlpha = std::numeric_limits<int>::max();
		...
		From->Beta = std::numeric_limits<int>::min();
		From = From->Suc;
		do {
			From->Ancestor = To = From->Dad;
			From->Beta = !FixedOrCommon(From, To) ? From->Cost : std::numeric_limits<int>::min();
		...
		do {
			for (NFrom = From->CandidateSet; NFrom && (To = NFrom->To);
				...
				if (FixedOrCommon(From, To))
					NFrom->Alpha = std::numeric_limits<int>::min();
				else if (From->FixedTo2 || To->FixedTo2 || Forbidden(From, To))
					NFrom->Alpha = std::numeric_limits<int>::max();
				else if (To->AlphaComputed && (NN = FindCandidate(To, From)))
					NFrom->Alpha = NN->Alpha;
				else {
					Beta = BetaValue(From, To, this);
					NFrom->Alpha =
						Beta != std::numeric_limits<int>::min() ? max(NFrom->Cost - Beta, 0) : std::numeric_limits<int>::max();
