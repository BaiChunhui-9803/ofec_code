	...
	void LKHAlg::updateCurSolRecord(int t1, int t2, int t3, int t4) {
		...
		for (auto& it : fnode_discard_edges) {
			...
			if (m_curSolRecord[fromId].front() == toId) {
				std::swap(m_curSolRecord[fromId].front(), m_curSolRecord[fromId].back());
			}
		}
...
* are exchanged with edges (t2,t3) and (t4,t1).*/
	...
	void LKHAlg::set2optLocalSearchParamenters() {
	...
	//	auto id_param = ADD_PARAM(params);
	//	m_problem = ofec::Problem::generateByFactory(id_param, 0.1);
	...
	/*				std::cout << "solFit\t" << curSolCost << "\thash\t" << curhash<< "\tcurId\t" << curId << std::endl;
					...
					if (curInfo.beforeFit > curSolCost) {
						j = 0;
						{
							std::cout << curInfo.runId << "\t" << curInfo.beforeId << "\t" << curInfo.beforeFit << "\t" << curInfo.afterId << "\t" << curInfo.afterFit << std::endl;
						}
					}
					// output info 
	...
	GainType LKHAlg::setCurrentSolution(const std::vector<int>& sol) {
	...
	void LKHAlg::setCurrentSolutionFromBest() {
		Node* N = 0, * NextN = 0, * FirstAlternative = 0, * Last = 0;
		for (Last = FirstNode; (N = Last->BestSuc) != FirstNode; Last = N)
			Follow(N, Last);
	}
...
//			}
	...
	GainType LKHAlg::LinKernighanLocalSearchVer2(void) {
	...
	void LKHAlg::setDefaultGA_Parameters() {
		MoveType = 5;
		PatchingC = 3;
		PatchingA = 2;
		Runs = 10;
	...
	int LKHAlg::run(const std::string &fileName, std::vector<size_t> &tour)
	{
		lkh_ptr = this;
		GainType Cost, OldOptimum;
		double Time, LastTime = GetTime();
	...
	//	strcpy(ParameterFileName, fileName.c_str());
		...
		std::string filepath = static_cast<std::string>(ofec::g_working_directory) + "/instance/algorithm/realworld/DVRP/LKH/temp file/" + ProblemFileName;
		ReadProblem(filepath);
		if (SubproblemSize > 0) {
			if (DelaunayPartitioning)
				SolveDelaunaySubproblems();
	...
	int LKHAlg::run(const std::string& dirname, const std::string& fileName,
		std::vector<std::vector<std::vector<int>>>& sols, ofec::Random* rnd) {
		...
		int i;
		setDefaultParaments();
		setDefaultGA_Parameters();
		ReadProblem(dirname + fileName);
		setDefaultGA_Parameters();
		...
		for (Run = 1; Run <= Runs; Run++) {
			...
			if (m_recordedGrails) {
				...
				sols.push_back(m_trails);
				m_trails.clear();
			}
		//	costs.push_back(Cost);
			SRandom(rnd->uniform.nextNonStd<unsigned>(0,std::numeric_limits<unsigned>::max()-1));
		}
		//get tour
	...
	//		//	break;
	//		//}
	//		Cost = FindTourVer2();      /* using the Lin-Kernighan heuristic */
	//		if (MaxPopulationSize > 1 && !TSPTW_Makespan) {
	//			/* Genetic algorithm */
	//			int i;
	//			for (i = 0; i < PopulationSize; i++) {
	//				GainType OldPenalty = CurrentPenalty;
	//				GainType OldCost = Cost;
	//				Cost = MergeTourWithIndividual(i,lkh_ptr);

	//			}
	//			if (!HasFitness(CurrentPenalty, Cost)) {
	//				if (PopulationSize < MaxPopulationSize) {
	//					AddToPopulation(CurrentPenalty, Cost, lkh_ptr);
	//					//if (TraceLevel >= 1)
	//					//	PrintPopulation();
	//				}
	//				else if (SmallerFitness(CurrentPenalty, Cost,
	//					PopulationSize - 1)) {
	//					i = ReplacementIndividual(CurrentPenalty, Cost, lkh_ptr);
	//					ReplaceIndividualWithTour(i, CurrentPenalty, Cost, lkh_ptr);
	//					//if (TraceLevel >= 1)
	//					//	PrintPopulation();
	//				}
	//			}
	...
	//			Node* N;
	//			int Parent1, Parent2;
	//			Parent1 = LinearSelection(PopulationSize, 1.25, lkh_ptr);
	//			do
	//				Parent2 = LinearSelection(PopulationSize, 1.25, lkh_ptr);
	//			while (Parent2 == Parent1);
	//			ApplyCrossover(Parent1, Parent2,lkh_ptr);
	//			N = FirstNode;
	//			do {
	//				if (ProblemType != HCP && ProblemType != HPP) {
	//					int d = (this->*C)(N, N->Suc);
