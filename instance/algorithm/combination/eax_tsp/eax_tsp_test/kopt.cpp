...
//    for( int i = 0; i < fN; ++i ) fLink[ i ] = new int [ 2 ];
 ...
 //   fOrdSeg = new int [ fN ];
  ...
  //  fSegCity = new int [ fN ];
   ...
   // fOrient = new int [ fN ];
    fLinkSeg.resize(fN);
    for (auto& it : fLinkSeg) it.resize(2);
  //  fLinkSeg = new int* [ fN ];
  //  for( int i = 0; i < fN; ++i ) fLinkSeg[ i ] = new int [ 2 ];
   ...
   // fSizeSeg = new int [ fN ];
    fCitySeg.resize(fN);
    for (auto& it : fCitySeg) it.resize(2);
  //  fCitySeg = new int* [ fN ];
  //  for( int i = 0; i < fN; ++i ) fCitySeg[ i ] = new int [ 2 ];
   ...
   // fT = new int [ 5 ];
    fActiveV.resize(fN);
   // fActiveV = new int [ fN ];
    fInvNearList.resize(fN);
    for (auto& it : fInvNearList) it.resize(500);
...
void TKopt::doIt( TIndi& tIndi, Random* rnd){
    this->transIndiToTree( tIndi );
    this->sub(rnd);
    this->transTreeToIndi( tIndi );
}

void TKopt::sub(Random* rnd){
    int t1_st;
    double dis1, dis2;
...
BEGIN:
    //t1_st = rand()%fN;
    t1_st = rnd->uniform.nextNonStd<int>(0, fN);

    fT[1] = t1_st;
    //t1's loop
    while(1){
        ++totalLoop;
        if (totalLoop >= 1e5)break;
...
void TKopt::makeRandSol( TIndi& indi, Random* rnd){
    for( int j = 0; j < fN; ++j ) fB[j] = j;
    int r;
    for( int i = 0; i < fN; ++i ){
     //   r = rand() % (fN-i);
        r = rnd->uniform.nextNonStd<int>(0, fN - i);
        fGene[i] = fB[r];
        fB[r] = fB[fN-i-1];
    }
...
void ofec::eax_tsp2::TKopt::setSol(TIndi& indi, const std::vector<int>& sol)
{
    fGene = fB = sol;
