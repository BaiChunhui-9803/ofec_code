...
void ofec::eax_tsp2::TEnvironment::setPopulation()
{
    int N = fEvaluator->Ncity;
...
void TEnvironment::define(ofec::Environment *env, Random* rnd)
{
...
void TEnvironment::doItOrigin(Random* rnd)
{
...
void TEnvironment::doIt(Random* rnd) {
...
void TEnvironment::initPop(Random *rnd){
    for ( int i = 0; i < Npop; ++i )
    {
        tKopt->makeRandSol(tCurPop[i], rnd); /* Make a random tour */
        tKopt->doIt(tCurPop[i], rnd);        /* Apply the local search with the 2-opt neighborhood */
    }
}

void TEnvironment::selectForMating(Random *rnd)
{
    
    /* fIndexForMating[] <-- a random permutation of 0, ..., fNumOfPop-1 */
    tCross->tRand->permutation( fIndexForMating, Npop, Npop , rnd);
    fIndexForMating[ Npop ] = fIndexForMating[ 0 ];
    testRandom(rnd, "error after tCross permutation ");
}

void TEnvironment::generateKids( int s, Random* rnd)
{
    /* Note: tCurPop[fIndexForMating[s]] is replaced with a best offspring solutions in tCorss->DoIt(). 
     fEegeFreq[][] is also updated there. */
    tCross->setParents( tCurPop[fIndexForMating[s]], tCurPop[fIndexForMating[s+1]], fFlagC, Nch , rnd);


    testRandom(rnd, "error after tCross setParents ");

    tCross->doIt( tCurPop[fIndexForMating[s]], tCurPop[fIndexForMating[s+1]], Nch, 1, fFlagC, fEdgeFreq, rnd);

    testRandom(rnd, "error after tCross doIt ");
