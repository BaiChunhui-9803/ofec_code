...
BEGIN:
    //t1_st = rand()%fN;
    t1_st = tRand->m_random->uniform.nextNonStd<int>(0, fN);
...
void TKopt::checkValid(){
  ...
  //  t_st = rand() % fN;
    t_st = tRand->m_random->uniform.nextNonStd<int>(0, fN);
...
void TKopt::makeRandSol( TIndi& indi ){
    ...
    for( int i = 0; i < fN; ++i ){
     //   r = rand() % (fN-i);
        r = tRand->m_random->uniform.nextNonStd<int>(0, fN - i);
        fGene[i] = fB[r];
        fB[r] = fB[fN-i-1];
    }
...
void ofec::eax_tsp::TKopt::setSol(TIndi& indi, const std::vector<int>& sol)
{
    fGene = fB = sol;
