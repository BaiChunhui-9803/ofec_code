...
TCross::TCross( int N, const std::shared_ptr<Random> &rnd){

//    Random *rnd = ofec::ADD_RND(0.5);

    tRand.reset(new TRandom(rnd));
    tSort.reset(new TSort(tRand->m_random));
...
void TCross::setParents( const TIndi& tPa1, const TIndi& tPa2, const std::vector<int>& flagC, int numOfKids )
{
...
void TCross::doIt( TIndi& tKid, TIndi& tPa2, int numOfKids, int flagP, const std::vector<int>& flagC, std::vector<std::vector<int>>& fEdgeFreq )
{
    int Num;
    int jnum, centerAB;
    double gain;
        ...
        else if (fEsetType == 2) /* Block2 */
        {
            ...
            for( int s = 0; s < fNumOfABcycle; ++s ){
                if( s == centerAB ) fABcycleInEset[ fNumOfABcycleInEset++ ] = s;
                    ...
                    if(tRand->Double(0,1) <=0.5 ) fABcycleInEset[ fNumOfABcycleInEset++ ] = s;
                }
            }
...
void TCross::doItWithoutParent(TIndi& tKid, int numOfKids, int flagP, const std::vector<int>& flagC, std::vector<std::vector<int>>& fEdgeFreq) {
    int Num;
    int jnum, centerAB;
    double gain;
    double BestGain;
...
void TCross::setABcycle( const TIndi& tPa1, const TIndi& tPa2, const std::vector<int>& flagC, int numOfKids )
{
    ...
    while(koritsuMany!=0){
        if(flagSt==1){
            fPosiCurr=0;
            //r=rand()%koritsuMany;
            r = tRand->m_random->uniform.nextNonStd<int>(0, koritsuMany);
        ...
        while(flagCycle==0){
            ...
            case 2:
                //r=rand()%2;
             //   r = tRand->Integer(0, 3);
                r = tRand->m_random->uniform.nextNonStd<int>(0, 2);
    ...
    while(bunkiMany!=0){
        fPosiCurr=0;
        //r=rand()%bunkiMany;
        r = tRand->m_random->uniform.nextNonStd<int>(0, bunkiMany);
   ...
   // m_opt2_circles.clear();
    ...
    RESTART:
        ...
        if (a1 == -1 && nearMax == 10) /* This value must also be changed if nearMax is changed above */
        {
            ...
            int r = tRand->m_random->uniform.nextNonStd<int>(0, fNumOfElementInCU - 1);
...
void TCross::makeUnit(){
    ...
    tSort->sort( fSegPosiList, fSegPosiList.size());
