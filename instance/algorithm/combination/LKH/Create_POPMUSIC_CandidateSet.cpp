...
#define d(a, b) (((a) == (b) ? 0 : D(a, b)\
	...
	/*
	 * The Create_POPMUSIC_CandidateSet function creates for each info.node
	...
	void LKHAlg::Create_POPMUSIC_CandidateSet(int K)
	{
		...
		info.node = (Node **)malloc((n + 1) * sizeof(Node *));
		info.node_path = (Node **)malloc((n + 1) * sizeof(Node *));
		...
		for (no_res = 1; no_res <= NB_RES; no_res++) {
			...
			if (deleted > 0) {
				...
				} while ((N = N->Suc) != info.node[solution[0]]);
				...
				do {
					info.node[solution[n] = n] = N;
					n++;
				} while ((N = N->Next) != info.node[solution[0]]);
			}
			solution[n] = solution[0];
			info.node[n] = info.node[solution[0]];
			...
			for (i = 0; i < n; i++) {
				Node *a = info.node[solution[i]];
				Node *b = info.node[solution[i + 1]];
				d = (this->*D)(a, b);
	...
	static GainType length_path(int n, int *path, LKHAlg *Alg)
	{
		...
		auto& info = Alg->m_popmusic_info;
		LKHAlg::Node *a, *b;
		GainType length = 0;
		int i;

		for (i = 1, a = info.node[path[0]]; i <= n; i++, a = b) {
			b = info.node[path[i]];
	...
	static void optimize_path(int n, int *path, LKHAlg *Alg)
	{
		...
		for (i = 0; i < n; i++)
			length += d[i][i + 1];
		for (i = 0; i <= n; i++)
			info.node_path[i] = info.node[path[i]];
	...
	static void build_path(int n, int *path, int nb_clust, LKHAlg *Alg)
	{
		...
		dmin = (((info.node[tmp_path[1]]) == (info.node[path[0]]) ? 0 : (Alg->*(Alg->D))(info.node[tmp_path[1]], info.node[path[0]])\
			- (info.node[tmp_path[1]])->Pi - (info.node[path[0]])->Pi) / Alg->Precision);
		closest = 1;
		for (i = 2; i < n; i++) {
			d = (((info.node[tmp_path[i]]) == (info.node[path[0]]) ? 0 : (Alg->*(Alg->D))(info.node[tmp_path[i]], info.node[path[0]])\
				- (info.node[tmp_path[i]])->Pi - (info.node[path[0]])->Pi) / Alg->Precision);
			if (d < dmin) {
				dmin = d;
				closest = i;
			}
		...
		dmin = (((info.node[tmp_path[2]]) == (info.node[path[n]]) ? 0 : (Alg->*(Alg->D))(info.node[tmp_path[2]], info.node[path[n]])\
			- (info.node[tmp_path[2]])->Pi - (info.node[path[n]])->Pi) / Alg->Precision);
		closest = 2;
		for (i = 3; i < n; i++) {
			d = (((info.node[tmp_path[i]]) == (info.node[path[n]]) ? 0 : (Alg->*(Alg->D))(info.node[tmp_path[i]], info.node[path[n]])\
				- (info.node[tmp_path[i]])->Pi - (info.node[path[n]])->Pi) / Alg->Precision);
			if (d < dmin) {
				dmin = d;
				closest = i;
			}
		...
		for (i = 1; i < n; i++) {
			...
			for (j = 1; j <= nb_clust; j++) {
				...
				d = (((info.node[path[i]]) == (info.node[sample[j]]) ? 0 : (Alg->*(Alg->D))(info.node[path[i]], info.node[sample[j]])\
					- (info.node[path[i]])->Pi - (info.node[sample[j]])->Pi) / Alg->Precision);
				if (d < dmin) {
					dmin = d;
					closest = j;
				}
	...
	static int fixed(int a, int b, LKHAlg *Alg)
	{
		...
		return (a == 0 && b == n__ - 1) || (a == n__ - 1 && b == 0) ||
			//FixedOrCommon(info.node_path[a], info.node_path[b]);
			(Alg->Fixed(info.node_path[a], info.node_path[b]) || Alg->IsCommonEdge(info.node_path[a], info.node_path[b]));
	}
