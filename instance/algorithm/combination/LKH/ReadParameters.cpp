...
namespace LKH {
	...
	static char *GetFileName(LKHAlg*,char *Line);
	static char *ReadYesOrNo(LKHAlg*, int *V);
#undef max
	static size_t max(size_t a, size_t b);


	void LKHAlg::setDefaultParaments() {
		...
		MaxCandidates = 5;
		m_geneticInfo.MaxPopulationSize = 0;
		...
		Optimum = std::numeric_limits<GainType>::min();
		PatchingA = 1;
		PatchingC = 0;
		PatchingAExtended = 0;
	...
	void LKHAlg::ReadParameters()
	{
		...
		if (ParameterFileName) {
			...
			while (1) {
				...
				if (!(ParameterFileName = GetFileName(this,ReadLine(stdin)))) {
					do {
						printff("PROBLEM_FILE = ");
						ProblemFileName = GetFileName(this,ReadLine(stdin));
		...
		while ((Line = ReadLine(ParameterFile))) {
			...
			if (!strcmp(Keyword, "ASCENT_CANDIDATES")) {
				...
				if (!ReadYesOrNo(this,&Backtracking))
				...
				if (!(InitialTourFileName = GetFileName(this,0)))
				...
				if (!(InputTourFileName = GetFileName(this,0)))
				...
				if (!ReadYesOrNo(this,&TSPTW_Makespan))
				...
				if (!(MTSPSolutionFileName = GetFileName(this,0)))
				...
				if (!(OutputTourFileName = GetFileName(this,0)))
				...
				if (!(Token = lkh_strtok(0, Delimiters, &m_filestream_p)) ||
					!sscanf(Token, "%d", &m_geneticInfo.MaxPopulationSize))
				...
				if (!(ProblemFileName = GetFileName(this,0)))
				...
				if (!ReadYesOrNo(this,&RestrictedSearch))
				...
				if ((Token = lkh_strtok(0, Delimiters, &m_filestream_p))) {
					...
					while ((Token = lkh_strtok(0, Delimiters, &m_filestream_p))) {
						for (i = 0; i < strlen(Token); i++)
							Token[i] = (char)toupper(Token[i]);
				...
				if (!ReadYesOrNo(this,&SubsequentPatching))
				...
				if (!(TourFileName = GetFileName(this,0)))
			...
			if ((Token = lkh_strtok(0, Delimiters, &m_filestream_p)) && Token[0] != '#')
	...
	static char *GetFileName(LKHAlg*alg, char *Line)
	{
		char *Rest = alg->lkh_strtok(Line, "\n\t\r\f", &alg->m_filestream_p), *t;
	...
	static char *ReadYesOrNo(LKHAlg* alg, int *V)
	{
