		...
		const std::string& parFile, std::vector<int>& tour)
	{
	   ...
	   /* if (argc >= 2)
		...
		for (Run = 1; Run <= Runs; Run++) {
			...
			if (m_geneticInfo.MaxPopulationSize > 1 && !TSPTW_Makespan) {
				/* Genetic algorithm */
				int i;
				for (i = 0; i < m_geneticInfo.PopulationSize; i++) {
				...
				if (!HasFitness(CurrentPenalty, Cost,this)) {
					if (m_geneticInfo.PopulationSize < m_geneticInfo.MaxPopulationSize) {
						AddToPopulation(CurrentPenalty, Cost, this);
						if (TraceLevel >= 1)
							PrintPopulation(this);
					}
					else if (SmallerFitness(CurrentPenalty, Cost,
						m_geneticInfo.PopulationSize - 1, m_geneticInfo)) {
			...
			if (StopAtOptimum && m_geneticInfo.MaxPopulationSize >= 1) {
				if (ProblemType != CCVRP && ProblemType != TRP &&
					ProblemType != MLP &&
					MTSPObjective != MINMAX &&
					MTSPObjective != MINMAX_SIZE ?
			...
			if (m_geneticInfo.PopulationSize >= 2 &&
				(m_geneticInfo.PopulationSize == m_geneticInfo.MaxPopulationSize ||
					Run >= 2 * m_geneticInfo.MaxPopulationSize) && Run < Runs) {
				Node* N;
				int Parent1, Parent2;
				Parent1 = LinearSelection(m_geneticInfo.PopulationSize, 1.25, this);
				do
					Parent2 = LinearSelection(m_geneticInfo.PopulationSize, 1.25, this);
