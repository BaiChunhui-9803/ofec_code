...
// GPX2
namespace LKH {
//thread_local int info.n_cities, n_cand;
...
//thread_local LKHAlg::Node **info.Map2Node=0;



    //struct gpxTour {
    //    int* curTour.id;                 // vector with the curTour.id(candidate component) of each node
    //    int* curTour.size_;               // vector
    //    int* curTour.n_inputs;
    //    int* curTour.n_outputs;
    //    tour* curTour.blue, * curTour.red;
    //    int curTour.n_;                   // size of the tours 
    //    int** curTour.M_neigh;           // neighbourhood matrix: first collumn indicates the
    //    // number of neighbours, and the collumns 2 and 3
    //    // indicate the index of the neighbours
    //    int** curTour.M_neigh2;          // neighbourhood matrix: the collumns indicate the
    //    // number of i conections to the neighbours indicated
    //    // in collumns 2 and 3

    //    int* curTour.test;                      // curTour.test of the candidtes: 1 - true component; 0 - otherwise
...
GainType gpx(int *solution_blue, int *solution_red, int *offspring, LKHAlg *Alg)
{
    auto& info = Alg->m_gpxInfo;
    ...
    d4_vertices = alloc_vectori(info.n_cities);
    common_edges_blue = alloc_vectori(info.n_cities);
    common_edges_red = alloc_vectori(info.n_cities);
    n_d4_vertices =
        d4_vertices_id(solution_blue, solution_red, d4_vertices,
                       common_edges_blue, common_edges_red,info);

    // Step 2: Insert ghost nodes
    n_new = info.n_cities + n_d4_vertices;
    // size of the new solutions: info.n_cities + number of ghost nodes
    label_list = alloc_vectori(n_new);
    // label_list: label for each node (including the ghost nodes)
    label_list_inv = alloc_vectori(info.n_cities);
    // label_list_inv: inverse of label_list
    j = 0;      // counter for the vertices with degree 4 (ghost nodes)
    for (i = 0; i < info.n_cities; i++) {
        label_list_inv[i] = -1;
        if (d4_vertices[i] == 1) {
            label_list[info.n_cities + j] = i;
            label_list_inv[i] = info.n_cities + j;
            j++;
        }
        label_list[i] = i;
    }
    // inserting the ghost nodes in solutions curTour.blue and curTour.red
    solution_blue_p2 = alloc_vectori(n_new);
    // solution curTour.blue with the ghost nodes
    solution_red_p2 = alloc_vectori(n_new);
    // solution curTour.red with the ghost nodes
    ...
    for (i = 0; i < info.n_cities; i++) {
        ...
        if (d4_vertices[i] == 1) {
            ...
            common_edges_p2_blue[info.n_cities + j] = common_edges_blue[i];
            common_edges_p2_red[info.n_cities + j] = common_edges_red[i];
            j++;
        }
    }
    ...
    tourTable(solution_blue_p2, solution_red_p2, solution_red, label_list,
              ...
              common_edges_p2_red, info);


    gpxTour curTour;

    // Step 4: Creating the candidate components
    new_candidates(vector_comp, n_new, curTour, Alg->m_gpxInfo); // object candidate recombination
    // component
    free(vector_comp);

    // Step 5: Finding the inputs and outputs of each candidate component
    findInputs(solution_blue_p2, solution_red_p2, info.n_cand,curTour);

    // Step 6: testing the candidate components
    // Step 6.a: curTour.test components using simplified internal graphs
    for (i = 0; i < info.n_cand; i++)
        testComp(i, curTour);    // curTour.test component i

    // Step 6.b: curTour.test unfeasible components using simplified external graphs
    testUnfeasibleComp(solution_blue_p2, info.n_cand, curTour);

    // Step 7.a: fusions of the candidate components that are neighbours
    //           (with more than to cutting points)
    fusion(solution_blue_p2, solution_red_p2, info.n_cand, curTour);
    // if candidate i did not pass the curTour.test and has conditions, apply
    ...
    fusionB(solution_blue_p2, solution_red_p2, info.n_cand, curTour);
    // if candidate i did not pass the curTour.test and has conditions, apply fusionB
    // to find fusions of partitions in order to have partitions with 2 cutting
    // points

    // Selecting the best between the curTour.blue and curTour.red path in each component
    fitness_offspring = off_gen(solution_blue_p2, solution_red_p2,
                                offspring, label_list,curTour, Alg);
    free_candidates(curTour, Alg->m_gpxInfo);
    free(label_list);
    free(label_list_inv);
    free(d4_vertices);
...
// Compute the weights
int weight(int i, int j, LKHAlg *Alg)
{
    auto& info = Alg->m_gpxInfo;
    LKHAlg::Node *a = info.Map2Node[i], *b = info.Map2Node[j];
    return ((Alg->*(Alg->C))(a, b) - a->Pi - b->Pi) / Alg->Precision;
}
...
int d4_vertices_id(int *solution_blue, int *solution_red, int *d4_vertices,
                   int *common_edges_blue, int *common_edges_red,LKHAlg::gpxInfo& info)
{
    int i, aux, aux2, **M_aux, n_d4_vertices;

    //create a matrix (info.n_cities x 4) with all edges;
    M_aux = alloc_matrixi(info.n_cities, 4);

    for (i = 1; i < info.n_cities - 1; i++) {
        aux = solution_blue[i];
    ...
    M_aux[aux][1] = solution_blue[info.n_cities - 1];
    aux = solution_red[0];
    M_aux[aux][2] = solution_red[1];
    M_aux[aux][3] = solution_red[info.n_cities - 1];
    aux = solution_blue[info.n_cities - 1];
    M_aux[aux][0] = solution_blue[0];
    M_aux[aux][1] = solution_blue[info.n_cities - 2];
    aux = solution_red[info.n_cities - 1];
    M_aux[aux][2] = solution_red[0];
    M_aux[aux][3] = solution_red[info.n_cities - 2];
    ...
    dealloc_matrixi(M_aux, info.n_cities);
...
int ghostPair(int *label_list, int *label_list_inv, int entry, LKHAlg::gpxInfo& info)
{
    return entry >
        info.n_cities - 1 ? label_list[entry] : label_list_inv[entry];
}
...
// curTour.test if simplified graphs outside unfesible candidate component are equal
...
void tourTable_fill(int **Tour_table, int *d2_vertices,
                    ...
                    int *common_edges_red_p2, int n_new,LKHAlg::gpxInfo& info)
{
    int i, sol1, sol2, ghost_a, ghost_b, ghost_c, common_a, common_b,
        common_c, a, b, c;

    // Inserting in the table the curTour.blue and curTour.red tours (col. 0-1)
    for (i = 0; i < n_new - 1; i++) {
        // Inserting in the table the tour for the curTour.blue tour
        ...
        // Inserting in the table the tour for the direct curTour.red tour
    ...
    // Inserting in the table the reverse curTour.red tours (col. 2)        
    a = solution_red[info.n_cities - 1];
    ghost_a = ghostPair(label_list, label_list_inv, a, info);
    if (ghost_a == -1)
        common_a = common_edges_red_p2[a];
    else
        common_a = common_edges_red_p2[ghost_a];
    b = solution_red[0];
    ghost_b = ghostPair(label_list, label_list_inv, b, info);
    if (ghost_b == -1)
        common_b = common_edges_red_p2[b];
    else
        common_b = common_edges_red_p2[ghost_b];
    c = solution_red[1];
    ghost_c = ghostPair(label_list, label_list_inv, c, info);
    if (ghost_c == -1)
               ...
               int *common_edges_red_p2, LKHAlg::gpxInfo& info)
{
    ...
    // from dir (0) or rev (1) curTour.red
    ...
    // lines: vertices; 
    // columns: 
    //     0 - next single vertex in curTour.blue tour
    //     1 - next single vertex in direct curTour.red tour
    //     2 - next single vertex in reverse curTour.red tour
    //     3 - next common vertex 
    //     4 - candidate to connected component following the direct curTour.red tour
    //     5 - candidate to connected component following the reverse curTour.red tour
    ...
    tourTable_fill(Tour_table, d2_vertices, solution_blue_p2,
                   ...
                   common_edges_red_p2, n_new, info);
    ...
    do {
        ...
        // folowing direct curTour.red tour
        // AB Cycles: direct curTour.red tour
        // all assigned become visited                          
        cand_dir = 0;
        for (i = 0; i < n_new; i++) {
            if (visited[i] == 0) {
                start = i;
                edge_tour = 0;  // 0 for curTour.blue edge and 1 for curTour.red edge
                do {
                    Tour_table[i][4] = cand_dir;
                    vector_cand_dir[i] = cand_dir;
                    visited[i] = 1;
                    if (edge_tour == 0) {
                        i = Tour_table[i][0];   // get curTour.blue edge
                        edge_tour = 1;
                    } else {
                        i = Tour_table[i][1];   // get direct curTour.red edge
                        edge_tour = 0;
                    }
                } while (i != start);
                cand_dir++;
        ...
        // following reverse curTour.red tour
        // AB Cycles: reverse curTour.red tour          
        // all assigned become visited
        cand_rev = 0;
        for (i = 0; i < n_new; i++) {
            if (vector_comp[i] == -1)
            ...
            // alredy assigned for direct curTour.red tour
        ...
        for (i = 0; i < n_new; i++) {
            if (visited[i] == 0) {
                ...
                do {
                    ...
                    ghost_pair = ghostPair(label_list, label_list_inv, i, info);
                    if (ghost_pair != -1) {
                        ...
                        // cand_rev was alredy assigned for direct curTour.red tour
                    }
                    visited[i] = 1;
                    if (edge_tour == 0) {
                        i = Tour_table[i][0];   // get curTour.blue edge
                        ...
                        i = Tour_table[i][2];   // get reverse curTour.red edge
                        edge_tour = 0;
                    }
                } while (i != start);
                cand_rev++;
        ...
        // new labels for direct curTour.red tour
        ...
        // new labels for reverse curTour.red tour
                ...
                for (i = 0; i < n_new; i++) {
                    if (vector_comp[i] == -1) {
                        if (red_chosen == 0
                            ...
                            // assigning component
                            // recording dir. curTour.red tour                      
                            if (vector_comp_red[i] == -1) {
                                ...
                                if (ghost_pair != -1) {
                                    ...
                                    // direct curTour.red tour
                                    vector_comp_red[ghost_pair] = 0;
                                    // zero means that it comes from the
                                    // direct curTour.red tour
                                    // reversing the ghost nodes (changingi
                                    // direction in  Table) of the curTour.red tours
                                    // exchanging the reverse curTour.red edge for i
                                    // and ghost node
                                    sol1 = i;
                                    sol2 = ghost_pair;
    ...
    // change ghost nodes in curTour.red tour
    for (i = 0; i < n_new; i++) {
        ghost_pair =
            ghostPair(label_list, label_list_inv, solution_red_p2[i], info);
...
static int isequal(Graph * G1, Graph * G2);     // curTour.test if two graphs are equal

void new_candidates(int *vector_comp, int n_new, gpxTour& curTour, LKHAlg::gpxInfo& info)
{
    int i, j;
    auto& n_cand = info.n_cand;

    curTour.n_ = n_new;
    n_cand = 0; // number of candidate components
    for (i = 0; i < curTour.n_; i++)
        if (vector_comp[i] > n_cand)
            n_cand = vector_comp[i];    // remember that the first component has
    // label 0
    n_cand++;
    curTour.size_ = new_int(n_cand);     // size of each component
    curTour.id = new_int(curTour.n_);
    curTour.n_inputs = new_int(n_cand);
    curTour.n_outputs = new_int(n_cand);
    curTour.M_neigh = alloc_matrixi(n_cand, 3);
    curTour.M_neigh2 = alloc_matrixi(n_cand, 2);
    for (i = 0; i < n_cand; i++)
        curTour.size_[i] = 0;
    for (i = 0; i < curTour.n_; i++) {
        curTour.id[i] = vector_comp[i];
        j = curTour.id[i];
		curTour.size_[j] = curTour.size_[j] + 1;
    }
    curTour.test = new_int(n_cand);
    curTour.blue = new_tour(n_cand);
    curTour.red = new_tour(n_cand);
}
...
void free_candidates(gpxTour& curTour, LKHAlg::gpxInfo& info)
{
    ...
    free(curTour.size_);
    free(curTour.id);
    free(curTour.n_inputs);
    free(curTour.n_outputs);
    dealloc_matrixi(curTour.M_neigh, n_cand);
    dealloc_matrixi(curTour.M_neigh2, n_cand);
    for (i = 0; i < n_cand; i++) {
        free(curTour.blue[i].inputs);
        free(curTour.blue[i].outputs);
        free(curTour.red[i].inputs);
        free(curTour.red[i].outputs);
    }
    free(curTour.test);
    free(curTour.blue);
    free(curTour.red);
...
void findInputs(int *sol_blue, int *sol_red, int n_cand, gpxTour& curTour)
{
   // auto& n_cand = cand;
    ...
    for (i = 0; i < n_cand; i++) {
        comp_size = (int) ceil(curTour.size_[i] / 2);    // the inputs/outputs are created
        // with maximum size
        curTour.blue[i].inputs = new_gate_structure(comp_size);
        curTour.blue[i].outputs = new_gate_structure(comp_size);
        curTour.blue[i].first_entry.num = -1;
        curTour.red[i].inputs = new_gate_structure(comp_size);
        curTour.red[i].outputs = new_gate_structure(comp_size);
        curTour.red[i].first_entry.num = -1;
    }

    // Solutions without common edges
    sol_blue_reduc = alloc_vectori(curTour.n_);
    sol_red_reduc = alloc_vectori(curTour.n_);
    sol_blue_reduc_t = alloc_vectori(curTour.n_);
    sol_red_reduc_t = alloc_vectori(curTour.n_);
    j = k = 0;
    for (i = 0; i < curTour.n_; i++) {
        aux = sol_blue[i];
        if (curTour.size_[curTour.id[aux]] > 1) {
    ...
    // Blue Tour
    for (i = 0; i < n_cand; i++) {
        curTour.n_inputs[i] = 0;
        curTour.n_outputs[i] = 0;
        curTour.M_neigh[i][0] = 0;      // the first column indicates the number
        // of neighbours       
    }
    for (i = 0; i < n_reduc; i++) {
        ...
        if (i == (n_reduc - 1))
            ...
            i_h = i + 1;
        aux = curTour.id[sol_blue_reduc[i]];
        aux2 = curTour.id[sol_blue_reduc[i_l]];
        if (aux != aux2) {
            curTour.blue[aux].inputs[curTour.n_inputs[aux]] = gate;
            curTour.n_inputs[aux] = curTour.n_inputs[aux] + 1;
            if (curTour.blue[aux].first_entry.num == -1)
                curTour.blue[aux].first_entry = gate;
            // records the first entry in the candidate for the curTour.blue tour
            // Updating the neighbourhood relations
            if (curTour.M_neigh[aux][0] == 0) {
                curTour.M_neigh[aux][0] = 1;
                curTour.M_neigh[aux][1] = aux2;
                curTour.M_neigh2[aux][0] = 1;
            } else if (curTour.M_neigh[aux][0] == 1) {
                if (curTour.M_neigh[aux][1] == aux2)
                    curTour.M_neigh2[aux][0] = curTour.M_neigh2[aux][0] + 1;
                else {
                    curTour.M_neigh[aux][0] = 2;
                    curTour.M_neigh[aux][2] = aux2;
                    curTour.M_neigh2[aux][1] = 1;
                }
            } else {
                if (curTour.M_neigh[aux][1] == aux2)
                    curTour.M_neigh2[aux][0] = curTour.M_neigh2[aux][0] + 1;
                else if (curTour.M_neigh[aux][2] == aux2)
                    curTour.M_neigh2[aux][1] = curTour.M_neigh2[aux][1] + 1;
                else
                    curTour.M_neigh[aux][0] = curTour.M_neigh[aux][0] + 1;
            }
        }
        aux2 = curTour.id[sol_blue_reduc[i_h]];
        if (aux != aux2) {
            curTour.blue[aux].outputs[curTour.n_outputs[aux]] = gate;
            curTour.n_outputs[aux] = curTour.n_outputs[aux] + 1;
            curTour.blue[aux].last_exit = gate;
            // records the last exit in the candidate for the curTour.blue tour
            // Updating the neighbourhood relations
            if (curTour.M_neigh[aux][0] == 0) {
                curTour.M_neigh[aux][0] = 1;
                curTour.M_neigh[aux][1] = aux2;
    ...
    for (i = 0; i < n_reduc; i++) {
        ...
        if (i == (n_reduc - 1))
            ...
            i_h = i + 1;
        aux = curTour.id[sol_red_reduc[i]];
        if (aux != curTour.id[sol_red_reduc[i_l]]) {
            curTour.red[aux].inputs[curTour.n_inputs[aux]] = gate;
            curTour.n_inputs[aux] = curTour.n_inputs[aux] + 1;
            if (curTour.red[aux].first_entry.num == -1)
                curTour.red[aux].first_entry = gate;
            // records the first entry in the candidate for the curTour.red tour 
        }
        if (aux != curTour.id[sol_red_reduc[i_h]]) {
            curTour.red[aux].outputs[curTour.n_outputs[aux]] = gate;
            curTour.n_outputs[aux] = curTour.n_outputs[aux] + 1;
            curTour.red[aux].last_exit = gate;
            // records the last exit in the candidate for the curTour.red tour      
        }
    }
...
// curTour.test if graphs are equal (obs.: remember that each vertex has 0 or 1 edge)
...
void testComp(int cand, gpxTour& curTour)
{
    ...
    if (curTour.size_[cand] <= 1)
        curTour.test[cand] = -1;
    else {
        if (curTour.n_inputs[cand] < 1)
            curTour.test[cand] = 0;
        else {
            if (curTour.n_inputs[cand] == 1)
                curTour.test[cand] = 1;
            else {
                // Graphs for curTour.blue and curTour.red tours                        
                inp_out_blue_inv = new_int(curTour.n_);
                inp_out_red = new_int(2 * curTour.n_inputs[cand]);
                ...
                if (curTour.blue[cand].inputs[0].time < curTour.blue[cand].outputs[0].time) {
                    for (i = 0; i < curTour.n_inputs[cand]; i++) {
                        inp_out_blue_inv[curTour.blue[cand].inputs[i].num] = 2 * i;
                        inp_out_blue_inv[curTour.blue[cand].outputs[i].num] =
                            2 * i + 1;
                    }
                } else {
                    for (i = 0; i < curTour.n_inputs[cand]; i++) {
                        inp_out_blue_inv[curTour.blue[cand].outputs[i].num] =
                            2 * i;
                        inp_out_blue_inv[curTour.blue[cand].inputs[i].num] =
                            2 * i + 1;
                    }
                }
                if (curTour.red[cand].inputs[0].time < curTour.red[cand].outputs[0].time) {
                    for (i = 0; i < curTour.n_inputs[cand]; i++) {
                        inp_out_red[2 * i] = curTour.red[cand].inputs[i].num;
                        inp_out_red[2 * i + 1] = curTour.red[cand].outputs[i].num;
                    }
                } else {
                    for (i = 0; i < curTour.n_inputs[cand]; i++) {
                        inp_out_red[2 * i] = curTour.red[cand].outputs[i].num;
                        inp_out_red[2 * i + 1] = curTour.red[cand].inputs[i].num;
                    }
                }

                Graph *Gs_blue = new_Graph(2 * curTour.n_inputs[cand]);
                // simplified graph for the curTour.blue path inside the component cand
                Graph *Gs_red = new_Graph(2 * curTour.n_inputs[cand]);
                // simplified graph for the curTour.red path inside the component cand
                // Two cases: Case 1 - first input before first output;
                //            Case 2 - otherwise
                // Obs.: remember that the inputs and outputs are inserted
                // according to the flow
                if (curTour.blue[cand].inputs[0].time < curTour.blue[cand].outputs[0].time) {
                    for (i = 0; i < (2 * curTour.n_inputs[cand] - 1); i = i + 2) {
                        insertEdge(Gs_blue, i, i + 1);
                        insertEdge(Gs_blue, i + 1, i);
                    }
                } else {
                    for (i = 1; i < (2 * curTour.n_inputs[cand] - 2); i = i + 2) {
                        insertEdge(Gs_blue, i, i + 1);
                        insertEdge(Gs_blue, i + 1, i);
                    }
                    insertEdge(Gs_blue, 2 * curTour.n_inputs[cand] - 1, 0);
                    insertEdge(Gs_blue, 0, 2 * curTour.n_inputs[cand] - 1);
                }
                if (curTour.red[cand].inputs[0].time < curTour.red[cand].outputs[0].time) {
                    for (i = 0; i < (2 * curTour.n_inputs[cand] - 1); i = i + 2) {
                        insertEdge(Gs_red,
                ...
                curTour.test[cand] = isequal(Gs_blue, Gs_red);
                free(Gs_red);
                free(Gs_blue);
...
int testUnfeasibleComp(int *sol_blue, int n_cand, gpxTour& curTour)
{
    int i, j, aux, *comp_seq, *inp_comp_seq, n_newpart = 0;

    comp_seq = alloc_vectori(curTour.n_);        // sequence of all entries in unfeasible
    ...
    // creating comp_seq
    j = 0;      // j is the effective size of comp_seq
    aux = curTour.id[sol_blue[0]];
    if (curTour.test[aux] == 0 && aux != curTour.id[sol_blue[curTour.n_ - 1]])
        comp_seq[j++] = aux;
    inp_comp_seq[aux] = 0;
    for (i = 1; i < curTour.n_; i++) {
        aux = curTour.id[sol_blue[i]];
        if (curTour.test[aux] == 0 && aux != curTour.id[sol_blue[i - 1]])
            comp_seq[j++] = aux;
        inp_comp_seq[aux] = 0;
    }
    ...
    if (j > 0) {
        ...
        for (i = 0; i < n_cand; i++)
            if (curTour.test[i] == 0 && inp_comp_seq[i] == 1) {
                curTour.test[i] = 1;
                n_newpart++;
            }
    }
...
// if candidate cand did not pass the curTour.test and has conditions,
// apply fusion with the neighbour with more connections
// (for more than 2 cutting points)
void fusion(int *sol_blue, int *sol_red, int n_cand, gpxTour& curTour)
{
    ...
    //                 = 1 already chosen, and curTour.id will be changed;
    //                 = 2 already chosen but curTour.id will not be changed 
    ...
    for (cand = 0; cand < n_cand; cand++)
        neigh_vec_cond[cand] = curTour.test[cand] == 1 || curTour.size_[cand] <= 1 ? -1 : 0;

    for (cand = 0; cand < n_cand; cand++) {
        if (neigh_vec_cond[cand] == 0) {
            if (curTour.M_neigh[cand][0] == 1) {
                aux = curTour.M_neigh[cand][1];
            ...
            } else if (curTour.M_neigh[cand][0] == 2) {
                if (curTour.M_neigh2[cand][0] > curTour.M_neigh2[cand][1]) {
                    aux = curTour.M_neigh[cand][1];
                    if (neigh_vec_cond[aux] == 0) {
                        neigh_vec_ind[cand] = aux;
                        neigh_vec_cond[cand] = 1;
    ...
    if (n_fusions > 0) {
        // Reseting tour structures curTour.blue and curTour.red 
        for (cand = 0; cand < n_cand; cand++) {
            free(curTour.blue[cand].inputs);
            free(curTour.blue[cand].outputs);
            free(curTour.red[cand].inputs);
            free(curTour.red[cand].outputs);
        }
        free(curTour.blue);
        free(curTour.red);
        curTour.blue = new_tour(n_cand);
        ...
        for (i = 0; i < curTour.n_; i++) {
            ...
            if (neigh_vec_cond[aux] == 1) {
				curTour.size_[aux] = curTour.size_[aux] - 1;
                curTour.id[i] = neigh_vec_ind[aux];
                aux = curTour.id[i];
				curTour.size_[aux] = curTour.size_[aux] + 1;
            }
        }
        ...
        // candidate component
        findInputs(sol_blue, sol_red, cand, curTour);
        // Repeating Step 6: testing the candidate components
        for (cand = 0; cand < n_cand; cand++)
            if (neigh_vec_cond[cand] == 2)
                testComp(cand, curTour); // curTour.test component cand                  
    }
...
// cutting points
void fusionB(int *sol_blue, int *sol_red, int n_cand, gpxTour& curTour)
{
    ...
    // Memory allocation
    cand_seq = alloc_vectori(curTour.n_);        // list of entries and exits of
    // unfeasible candidates
    cand_seq_cut = alloc_vectori(curTour.n_);
    new_label = alloc_vectori(n_cand);
    new_component = alloc_vectori(n_cand);
    Graph *G_cand = new_Graph(n_cand);

    // Walking in the curTour.blue tour and finding the high level cuts
    previous_cand = curTour.id[sol_blue[curTour.n_ - 1]];
    for (i = 0; i < curTour.n_; i++) {
        cand = curTour.id[sol_blue[i]]; // candidate for vertex i of the curTour.blue tour
        if (i == (curTour.n_ - 1))
            next_cand = curTour.id[sol_blue[0]];
        else
            next_cand = curTour.id[sol_blue[i + 1]];
        // curTour.test if it is an unfeasible partition
        if (curTour.test[cand] == 0 && curTour.n_inputs[cand] > 0 && curTour.size_[cand] > 1) {
            if (cand != previous_cand || cand != next_cand) {
                cand_seq[n_cand_seq] = cand;
                // checking if it is a first common entry or last common exit
                if (sol_blue[i] == curTour.blue[cand].first_entry.num &&
                    (curTour.blue[cand].first_entry.num ==
                     curTour.red[cand].first_entry.num
                     || curTour.blue[cand].first_entry.num ==
                     curTour.red[cand].last_exit.num))
                    cand_seq_cut[n_cand_seq] = 1;       // it is a first common entry
                else if (sol_blue[i] == curTour.blue[cand].last_exit.num &&
                         (curTour.blue[cand].last_exit.num ==
                          curTour.red[cand].last_exit.num ||
                          curTour.blue[cand].last_exit.num ==
                          curTour.red[cand].first_entry.num))
    ...
    // Walking in the curTour.red tour and finding the high level cuts
    n_cand_seq = 0;
    previous_cand = curTour.id[sol_red[curTour.n_ - 1]];
    for (i = 0; i < curTour.n_; i++) {
        cand = curTour.id[sol_red[i]];  // candidate for vertex i of the curTour.red tour
        if (i == (curTour.n_ - 1))
            next_cand = curTour.id[sol_red[0]];
        else
            next_cand = curTour.id[sol_red[i + 1]];
        // curTour.test if it is an unfeasible partition
        if (curTour.test[cand] == 0 && curTour.n_inputs[cand] > 0 && curTour.size_[cand] > 1) {
            if (cand != previous_cand || cand != next_cand) {
                cand_seq[n_cand_seq] = cand;
                // checking if it is a first common entry or last common exit
                if (sol_red[i] == curTour.red[cand].first_entry.num &&
                    (curTour.red[cand].first_entry.num ==
                     curTour.blue[cand].first_entry.num
                     || curTour.red[cand].first_entry.num ==
                     curTour.blue[cand].last_exit.num))
                    cand_seq_cut[n_cand_seq] = 1;       // it is a first common entry
                else if (sol_red[i] == curTour.red[cand].last_exit.num &&
                         (curTour.red[cand].last_exit.num ==
                          curTour.blue[cand].last_exit.num ||
                          curTour.red[cand].last_exit.num ==
                          curTour.blue[cand].first_entry.num))
    ...
    if (n_cand_seq > 0) {
        ...
        if (n_cand_new > -1) {
            ...
            for (cand = 0; cand < n_cand; cand++) {
                if (curTour.test[cand] == 0 && curTour.n_inputs[cand] > 0
                    && curTour.size_[cand] > 1) {
            ...
            for (i = 0; i < curTour.n_; i++) {
                ...
                if (new_label[cand] != cand) {
					curTour.size_[cand] = curTour.size_[cand] - 1;
                    curTour.id[i] = new_label[cand];
                    cand = curTour.id[i];
					curTour.size_[cand] = curTour.size_[cand] + 1;
                }
            }
            ...
            for (cand = 0; cand < n_cand; cand++)
                if (new_component[cand] == 1)
                    testComp(cand, curTour);

            // Testing unfeasible partitions 
            do {
                n_rounds++;
                n_newpart = testUnfeasibleComp(sol_blue,cand,curTour);
...
// select between the curTour.blue and curTour.red paths for each component
GainType off_gen(int *sol_blue, int *sol_red, int *offspring,
                 int *label_list, gpxTour& curTour, LKHAlg *Alg)
{
    auto& info = Alg->m_gpxInfo;
    auto& n_cand = info.n_cand;
    ...
    for (i = 0; i < curTour.n_; i++) {
        ...
        aux = sol_blue[i];
        aux2 = sol_blue[i_h];
        if (curTour.test[curTour.id[aux]] > 0 && curTour.id[aux] == curTour.id[aux2]) {
            if (label_list[aux] != label_list[aux2])
                curTour.blue[curTour.id[aux]].fitness = curTour.blue[curTour.id[aux]].fitness +
                    weight(label_list[aux], label_list[aux2], Alg);
        } else {
            if (label_list[aux] != label_list[aux2])
        ...
        if (curTour.test[curTour.id[aux]] > 0 && curTour.id[aux] == curTour.id[aux2]) {
            if (label_list[aux] != label_list[aux2])
                curTour.red[curTour.id[aux]].fitness = curTour.red[curTour.id[aux]].fitness +
                    weight(label_list[aux], label_list[aux2], Alg);
    ...
    if (blue_fitness_rest > red_fitness_rest)
        select_rest = 1;        // 1 for curTour.red and 0 for curTour.blue
    else
        select_rest = 0;
    select_cand = new_int(curTour.n_);

    for (i = 0; i < n_cand; i++) {
        if (curTour.test[i] < 1)
            select_cand[i] = select_rest;
        else {
            if (curTour.blue[i].fitness > curTour.red[i].fitness)
                select_cand[i] = 1;
            else
                select_cand[i] = 0;
        }
    ...
    sol_blue_index = new_int(curTour.n_);
    sol_red_index = new_int(curTour.n_);
    for (i = 0; i < curTour.n_; i++) {
        sol_blue_index[sol_blue[i]] = i;
        sol_red_index[sol_red[i]] = i;
    }
    // Offspring: Graph
    Graph *G_offspring = new_Graph(curTour.n_);  // graph for offspring
    for (i = 0; i < curTour.n_; i++) {
        if (select_cand[curTour.id[i]] == 0) {
            if (sol_blue_index[i] == 0) {
                i_l = sol_blue[curTour.n_ - 1];
                i_h = sol_blue[sol_blue_index[i] + 1];
            } else if (sol_blue_index[i] == (curTour.n_ - 1)) {
            ...
            if (sol_red_index[i] == 0) {
                i_l = sol_red[curTour.n_ - 1];
                i_h = sol_red[sol_red_index[i] + 1];
            } else if (sol_red_index[i] == (curTour.n_ - 1)) {
    ...
    // Offspring: vector
    offspring_p2 = new_int(curTour.n_);
    i = 0;
    offspring_p2[i] = 0;
    v_aux = 0;
    ...
    for (i = 0; i < n_cand; i++) {
        if (curTour.test[i] == 1) {
            if (select_cand[i] == 0)
                fitness = fitness + curTour.blue[i].fitness;
            else
                fitness = fitness + curTour.red[i].fitness;
        }
    }
    if (select_rest == 0)
    ...
    k = 0;
    for (i = 0; i < curTour.n_; i++)
        if (offspring_p2[i] < info.n_cities)
...
const thread_local int white = 0, grey = 1, black = 2;
