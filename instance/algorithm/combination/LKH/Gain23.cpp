...
//static LKHAlg::Node *s1;
//static short OldReversed;
namespace LKH {


	void LKHAlg::freeGain23()
	{
		/*s1.release();
		s1.reset();
		OldReversed.reset();*/
		//free(s1);
		m_gain32Info.s1 = 0;
		m_gain32Info.OldReversed = 0;
	}
	GainType LKHAlg::Gain23()
	{

		auto& s1 = m_gain32Info.s1;
		auto& OldReversed = m_gain32Info.OldReversed;
				...
				for (Ns2 = s2->CandidateSet; (s3 = Ns2->To); Ns2++) {
					...
					for (X4 = 1; X4 <= 2; X4++) {
						...
						if (X4 == 2 &&
							!Forbidden(s4, s1) &&
							(CurrentPenalty > 0 ||
								TSPTW_Makespan || !c || G2 - (this->*c)(s4, s1) > 0)) {
							Gain = G2 - (this->*C)(s4, s1);
							if (CurrentPenalty > 0 ||
