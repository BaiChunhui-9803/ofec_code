...
void LKH::LKHAlg::MTSP2TSP()
{
    ...
    if (ProblemType == TSP || ProblemType == ATSP) {
        if (MTSPObjective == MINSUM)
            Penalty = (&LKH::LKHAlg::Penalty_MTSP_MINSUM);
        else if (MTSPObjective == MINMAX)
            Penalty = (&LKH::LKHAlg::Penalty_MTSP_MINMAX);
        else if (MTSPObjective == MINMAX_SIZE)
            Penalty = (&LKH::LKHAlg::Penalty_MTSP_MINMAX_SIZE);
    } else if (ProblemType == CVRP)
        Penalty = (&LKH::LKHAlg::Penalty_CVRP);
    if (ProblemType == CVRP || ProblemType == CTSP || ProblemType == TSP) {
        ...
        if (ProblemType != CTSP && Salesmen <= Dim && MTSPMinSize > 0 &&
            ...
            for (i = 1; i <= Dim; i++) {
                ...
                if (N == Depot)
                    continue;
                N->Rank = (this->*Distance)(N, Depot);
                HeapLazyInsert(N,this);
            }
            Heapify(this);
            for (i = 1; i <= Salesmen; i++)
                HeapDeleteMin(this)->Special = i;
            HeapClear(this);
            free(Heap);
            Heap = 0;
        }
    ...
    Distance = &LKH::LKHAlg::Distance_MTSP;
    WeightType = SPECIAL;
}
