...
namespace LKH {
	GainType LKHAlg::MergeWithTourGPX2()
	{
		...
		m_gpxInfo.n_cities = NewDimension;
		red = (int *)malloc(m_gpxInfo.n_cities * sizeof(int));
		blue = (int *)malloc(m_gpxInfo.n_cities * sizeof(int));
		offspring = (int *)malloc((m_gpxInfo.n_cities + 1) * sizeof(int));
		m_gpxInfo.Map2Node = (Node **)malloc(m_gpxInfo.n_cities * sizeof(Node *));

		N = First;
		i = 0;
		do {
			m_gpxInfo.Map2Node[i] = N;
			red[i] = N->Rank = i;
			i++;
		...
		if (NewCost >= ShrunkCost1 || NewCost >= ShrunkCost2) {
			free(offspring);
			free(m_gpxInfo.Map2Node);
			CurrentPenalty = Penalty1;
			return Cost1;
		}
		offspring[m_gpxInfo.n_cities] = offspring[0];
		for (i = 0; i < m_gpxInfo.n_cities; i++) {
			N = m_gpxInfo.Map2Node[offspring[i]];
			Node *NextN = m_gpxInfo.Map2Node[offspring[i + 1]];
			N->OldSuc = NextN;
			NextN->OldPred = N;
		}
		free(offspring);
