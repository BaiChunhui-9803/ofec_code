                ...
                GainType * GlobalBestCost)
{
    ...
    for (Run = 1; Run <= Runs; Run++) {
        ...
        if (m_geneticInfo.MaxPopulationSize > 1) {
            /* Genetic algorithm */
            ...
            if (!HasFitness(CurrentPenalty, Cost,this)) {
                if (m_geneticInfo.PopulationSize < m_geneticInfo.MaxPopulationSize) {
                    ...
                    ReplaceIndividualWithTour(m_geneticInfo.PopulationSize - 1,
        ...
        if (Cost < Optimum || (Cost != Optimum && OutputTourFileName)) {
            ...
            if (TraceLevel >= 1) {
                ...
                if (OptimumSaved != std::numeric_limits<GainType>::min() && OptimumSaved != 0)
                    printff(", Gap = %0.4f%%",
                            100.0 * (GlobalCost -
