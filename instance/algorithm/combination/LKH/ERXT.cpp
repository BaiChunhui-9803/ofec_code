...
namespace LKH {
	...
	{
		free(m_erxtInfo.FirstFree);
		m_erxtInfo.FirstFree = nullptr;
	}
	void ERXT(LKHAlg *Alg)
	{
		auto& info = Alg->m_erxtInfo;
		...
		N = Alg->FirstNode;
		N->V = 1;
		info.FirstFree = N->Suc;
		N->Pred->Suc = N->Suc;
		N->Suc->Pred = N->Pred;
		for (i = 1; i < Alg->Dimension; i++) {
			Next = SelectNext(N, Alg);
			if (Next == info.FirstFree)
				info.FirstFree = Next->Suc;
