...
namespace ofec::OneMax_alg {
    ...
    static void sampleNFromM(std::vector<size_t>& sampled_number,
        size_t n, size_t m, ofec::Random* rnd) {
        using namespace std;
        ...
        if (n > m / 2) { /// If n is larger than m/2, we sample random indexes by reordering a permutation.
            ...
            for (size_t i = m - 1; i > 0; --i) {
                randPos = static_cast<size_t>(floor(rnd->uniform.next() * (i + 1)));
                temp = population[i];
            ...
            for (size_t i = 0; i != n; ++i) {
                do {
                    resample = false;
                    randPos = static_cast<size_t>(floor(rnd->uniform.next() * m));
