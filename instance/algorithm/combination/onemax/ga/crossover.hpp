...
namespace ofec::modularGA {
    ...
    public:
      ...
      void DoCrossover(std::vector<int>& y, 
          const std::vector<int>& x1, 
          const std::vector<int>& x2, Random*rnd) {
        switch (this->crossover_operator_) {
        case 1:
          this->UniformCrossover(y, x1, x2,rnd);
          break;
        case 2:
          this->OnePointCrossover(y, x1, x2, rnd);
          break;
        case 3:
          this->TwoPointCrossover(y, x1, x2, rnd);
          break;
        default:
          std::cerr << "unknown crossover operator" << std::endl;
          assert(false);
        }
      }

      void UniformCrossover(std::vector<int>& y, 
          const std::vector<int>& x1, const std::vector<int>& x2,Random* rnd) {
      ...
      void OnePointCrossover(std::vector<int>& y,
          const std::vector<int>& x1, 
          const std::vector<int>& x2,Random*rnd) {
        assert(x1.size() == x2.size());
        ...
        size_t point = (size_t)(rnd->uniform.next() * n);
        for (size_t i = 0; i != n; ++i) {
          if (i < point) {
            y.push_back(x1[i]);
          }
      ...
      void TwoPointCrossover(std::vector<int>& y,
          const std::vector<int>& x1, const std::vector<int>& x2,Random*rnd) {
        assert(x1.size() == x2.size());
        using namespace std;
        size_t n = x1.size();
        ...
        twopoints.push_back((size_t)(rnd->uniform.next() * n));
        twopoints.push_back((size_t)(rnd->uniform.next() * n));
        while (twopoints[0] == twopoints[1]) {
          twopoints[1] = (size_t)(rnd->uniform.next() * n);
        }
      ...
      void set_crossover_operator(std::string c) {
        ...
        if (c == "UNIFORMCROSSOVER") {
          ...
          std::cerr << "invalid name for crossover operator" << std::endl;
          assert(false);
        }
      }
      ...
      std::vector<size_t> c_flipped_index; /// < recording indexes where bits flip.
