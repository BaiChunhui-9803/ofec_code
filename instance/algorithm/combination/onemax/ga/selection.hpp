...
namespace ofec {
    namespace modularGA
    {
        ...
        public:
            ...
            void DoSelection(std::vector<std::vector<int>>& parents,
                std::vector<double>& parents_fitness,
                const std::vector<std::vector<int>>& offspring,
                const std::vector<double>& offspring_fitness, ofec::Random* rnd)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    this->TournamentPlusStrategy(parents, parents_fitness, offspring, offspring_fitness, rnd);
                    break;
                case 4:
                    this->TournamentCommaStrategy(parents, parents_fitness, offspring, offspring_fitness, rnd);
                    break;
                case 5:
                    this->ProportionalPlusStrategy(parents, parents_fitness, offspring, offspring_fitness, rnd);
                    break;
                case 6:
                    this->ProportionalCommaStrategy(parents, parents_fitness, offspring, offspring_fitness, rnd);
                    break;
                default:
                    std::cout << "unknown selection operator" << std::endl;
                    assert(false);
                    break;
                }
            }

            void BestCommaStrategy(std::vector<std::vector<int>>& parents,
                std::vector<double>& parents_fitness,
                const std::vector<std::vector<int>>& offspring,
            ...
            void BestPlusStrategy(std::vector<std::vector<int>>& parents,
                std::vector<double>& parents_fitness,
                const std::vector<std::vector<int>>& offspring,
                const std::vector<double>& offspring_fitness)
            {
                ...
                const std::vector<double>& offspring_fitness, ofec::Random* rnd)
            {
                ...
                for (size_t i = 0; i != mu; ++i)
                {
                    OneMax_alg::sampleNFromM(sample_k, this->tournament_k_, lambda, rnd);
                    //  if (Opt == optimizationType::MAXIMIZATION)
                    {
            ...
            void TournamentPlusStrategy(std::vector<std::vector<int>>& parents,
                std::vector<double>& parents_fitness,
                const std::vector<std::vector<int>>& offspring,
                const std::vector<double>& offspring_fitness, ofec::Random* rnd)
            {
                ...
                for (int i = 0; i != mu; ++i)
                {
                    OneMax_alg::sampleNFromM(sample_k, this->tournament_k_, mu + lambda, rnd);
                    // if (Opt == optimizationType::MAXIMIZATION)
                    {
                        tmp_best = numeric_limits<double>::lowest();
                        for (size_t j = 0; j != this->tournament_k_; ++j)
            ...
            void ProportionalCommaStrategy(std::vector<std::vector<int>>& parents,
                std::vector<double>& parents_fitness,
                const std::vector<std::vector<int>>& offspring,
                const std::vector<double>& offspring_fitness, ofec::Random* rnd)
            {
            ...
            void ProportionalPlusStrategy(std::vector<std::vector<int>>& parents,
                std::vector<double>& parents_fitness,
                const std::vector<std::vector<int>>& offspring,
                const std::vector<double>& offspring_fitness, ofec::Random* rnd)
            {
            ...
            void set_selection_operator(std::string s)
            {
                ...
                {
                    std::cout << "invalid name for selection operator" << std::endl;
                    assert(false);
                }
            }
