...
C              is returned and MIDACO stops. The Solution IFLAG flags are listed below.
C
...
C               (See the user manual for a more detailed description of Solution parameters)
C
...
C           Solution 'KEY' determining the license owner and its affiliation.
C
C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
	...
	long int eval, double tnow, long int iflag, double *pf, FILE *iout)
{
	...
	else
	{
		...
		for (i = 0; i < o; i++)
		{
			if (fabs(g[m + i]) <= 1.0e+18) {
				if (g[m + o + i] <= 0.0) { fprintf(iout, " f[%4i] = %34.15f \n", i, g[m + i]); }
				else { fprintf(iout, " f[%4i] = %34.15f \n", i, -g[m + i]); }
			}
			else {
				if (g[m + o + i] <= 0.0) { fprintf(iout, " f[%4i] = %34.6e \n", i, g[m + i]); }
