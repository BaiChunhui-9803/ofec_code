...
namespace ofec {
	...
	void SPMOEA1_7::initiVarSpace(Problem *pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA1_7::initPop(Problem *pro, Algorithm *alg, Random *rnd) {
		//在各个子空间生成一个子种群
		...
		for (size_t i = 0; i < space_num; ++i) {
			...
			std::vector<size_t> sub_space;
			sub_space.push_back(i);
			m_region_front_space.emplace_back(sub_space);
			std::vector <std::vector<size_t>> temp_space;
			temp_space.push_back(sub_space);
			getRegionLinkSpace().emplace_back(temp_space);
		}
		//全局种群
		SPMOEA_pop temp_pop(pop_num, pro);
	...
	int SPMOEA1_7::evolve(Problem *pro, Algorithm *alg, Random *rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
	...
	void SPMOEA1_7::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA1_7::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA1_7::updateSubPopSpace(size_t pop_inx, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_7::PopResourceAssign(std::vector<size_t>& assign_pop_resource, Problem *pro) {
		...
		for (size_t i = 0; i < getMO_HLC().getClusters().size(); ++i) {
			...
			/*if (count > 0) {
				...
				assign_pop_resource.push_back(2);
			}*/
		}
		auto max_count = *std::max_element(front_space_num.begin(),front_space_num.end());
		for (size_t i = 0; i < front_space_num.size(); ++i) {
			assign_pop_resource.push_back(2+(size_t)std::ceil((Real)max_pop_size*front_space_num[i]/max_count));
		}
		if (front_link_spaces.size() == 1) {
			...
			assign_pop_resource.push_back(total_size+5*front_link_spaces.size());
		}
		updatePopResource(assign_pop_resource);
	}

	void SPMOEA1_7::generateOffspring(Problem *pro, Random *rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
	...
	void SPMOEA1_7::spaceSubdivision(Problem *pro, Random *rnd) {
	...
	void SPMOEA1_7::clusterSubspace() {
	...
	std::vector<std::vector<size_t>> SPMOEA1_7::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA1_7::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA1_7::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	void SPMOEA1_7::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_7::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA1_7::findSplitDim(int inx, Problem *pro) {
	...
	void SPMOEA1_7::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
	...
	void SPMOEA1_7::recordMetrics(Problem *pro, Algorithm *alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA1_7::initiObjSpace(Problem *pro) {
