...
// based on spmoea1_5, just use historical front sols to update space info
// 采用多种群搜索的方式，在子空间初始化一个子种群，以及一个全局子种群，允许子种群越界
// 防止子种群重合

#ifndef OFEC_SPMOEA1_8_H
#define OFEC_SPMOEA1_8_H

#include "spmoea.h"

namespace ofec {

	class SPMOEA1_8 :public SPMOEA {
	public:
		...
		void PopResourceAssign(std::vector<size_t>& assign_pop_resource, Problem *pro);
		void generateOffspring(Problem *pro, Random *rnd, const std::vector<size_t>& pop_resource, std::vector<int> type);
		...
		void updatePop(Population<Solution<>> &off_pop1, Population<Solution<>>& off_pop2,Problem *pro, Algorithm *alg, Random *rnd);
		void assignPops(Population<Solution<>>& off_pop,Problem *pro, Algorithm *alg, Random *rnd);
