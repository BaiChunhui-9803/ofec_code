...
namespace ofec {
	...
	void SPMOEA1_13::initiVarSpace(Problem* pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA1_13::initPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//在各个子空间生成一个子种群
	...
	int SPMOEA1_13::evolve(Problem* pro, Algorithm* alg, Random* rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
	...
	void SPMOEA1_13::assignPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//先得到非前沿子空间
	...
	void SPMOEA1_13::testCoverage(Problem* pro) {
		//检测子空间是否包含不同的PS片段
	...
	std::vector<std::vector<Real>> SPMOEA1_13::sampleInFrontNeighSpace(std::vector<size_t>& front_link_spaces, size_t ind_inx, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA1_13::samplePush(Solution<>& sol, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	std::vector<std::vector<Real>> SPMOEA1_13::sampleByIndPos(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		auto space = getMO_HLC().subspaceTree().getRegionIdx(sol.variable().vect());
		auto& box = getMO_HLC().subspaceTree().getBox(space);
		auto bound = CAST_CONOP(pro)->boundary();
	...
	void SPMOEA1_13::SolutionSelection(Problem* pro, Random* rnd) {
		//后续解的选择需要考虑离已选解的分布
	...
	void SPMOEA1_13::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA1_13::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA1_13::updateSubPopSpace(size_t pop_inx, Problem* pro, Random* rnd) {
	...
	void SPMOEA1_13::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem* pro) {
	...
	void SPMOEA1_13::generateOffspring(Problem* pro, Algorithm* alg, Random* rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
	...
	bool SPMOEA1_13::spaceSubdivision(Problem* pro, Random* rnd) {
	...
	void SPMOEA1_13::updateLinkSubspace(size_t inx, const std::vector<size_t>& front_spaces, Random* rnd) {
	...
	void SPMOEA1_13::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
		for (size_t i = 0; i < getMO_HLC().numSubspace(); ++i) {
	...
	std::vector<std::vector<size_t>> SPMOEA1_13::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA1_13::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA1_13::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	void SPMOEA1_13::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem* pro, Random* rnd) {
	...
	void SPMOEA1_13::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA1_13::findSplitDim(int inx, Problem* pro) {
	...
	void SPMOEA1_13::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
	...
	void SPMOEA1_13::recordMetrics(Problem* pro, Algorithm* alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA1_13::initiObjSpace(Problem* pro) {
