...
namespace ofec {
	...
	void SPMOEA4_2::initialize_() {
		...
		m_test_ratio = v.get<Real>("explore rate");
	...
	void SPMOEA4_2::initiVarSpace(Problem* pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA4_2::initPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//在各个子空间生成一个子种群
	...
	int SPMOEA4_2::evolve(Problem* pro, Algorithm* alg, Random* rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
		...
		********************************************************************************/
		...
		if (!m_evolve_by_predict) {
			...
			for (size_t i = 0; i < getPop().size(); ++i) {
				auto igd = getIGD();
				if (igd.size() > 1) {
					auto delta_igd = std::fabs(igd.back() - igd[igd.size() - 2]);
					if (delta_igd / igd[igd.size() - 2] < 0.01) {
						m_stagnate_times++;
					}
					else {
						m_stagnate_times = 0;
				...
				if (m_stagnate_times >= m_num_push) {
		...
		*********************************************************************************************/
		...
		for (size_t i = 0; i < getPop().size(); ++i) {
			if (m_stagnate_times >= m_num_push) {
				...
				for (size_t j = 0; j < getPop()[i].size()-m_test_num; ++j) {
	...
	void SPMOEA4_2::assignPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//先得到非前沿子空间
	...
	size_t SPMOEA4_2::selectSubspace(Problem* pro, Random* rnd) {
	...
	std::vector<std::pair<size_t,size_t>> SPMOEA4_2::selectSubspaceFromObj(Problem* pro, Random* rnd) {
		//将目标空间所有非支配解分两类
		...
		//求最近类类距离
		std::vector<std::pair<size_t, size_t>> output_pairs;
		for (size_t i = 0; i < ind2cluster.size()-1; ++i) {
			auto ind_inx1 = ind2cluster[i];
			std::map < Real, std::pair<size_t, size_t>> cluster_dists;
			for (size_t j = i+1; j < ind2cluster.size(); ++j) {
			...
			output_pairs.emplace_back(min_pair.second);
		}
		return output_pairs;
	}

	void SPMOEA4_2::testCoverage(Problem* pro) {
		//检测子空间是否包含不同的PS片段
	...
	std::vector<std::vector<Real>> SPMOEA4_2::sampleInFrontNeighSpace(std::vector<size_t>& front_link_spaces, size_t ind_inx, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA4_2::sampleLinearInFrontSpace(size_t space, size_t ind_inx, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		//子连通空间内采样
		auto& sol = getPop()[0][ind_inx].variable().vect();
		for (size_t i = 0; i < sample_num; ++i) {
	...
	std::vector<std::vector<Real>> SPMOEA4_2::samplePush(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	std::vector<std::vector<Real>> SPMOEA4_2::sampleByIndPos(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		auto space = getMO_HLC().subspaceTree().getRegionIdx(sol.variable().vect());
		auto& box = getMO_HLC().subspaceTree().getBox(space);
		auto bound = CAST_CONOP(pro)->boundary();
	...
	void SPMOEA4_2::SolutionSelection(Problem* pro, Random* rnd) {
		//后续解的选择需要考虑离已选解的分布
	...
	void SPMOEA4_2::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA4_2::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA4_2::updateSubPopSpace(size_t pop_inx, Problem* pro, Random* rnd) {
	...
	void SPMOEA4_2::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem* pro) {
	...
	void SPMOEA4_2::generateOffspring(Problem* pro, Algorithm* alg, Random* rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
		...
		for (size_t i = 0; i < getPop().size(); ++i) {
			...
			if (type[i] == 0) {
				...
				//all_off = sampleRandom(getPop()[i], exploit_num, pro, alg, rnd);
				//选择一个子空间
				auto sele_space = selectSubspace(pro, rnd);
				auto box = getMO_HLC().subspaceTree().getBox(sele_space);
				all_off = sampleRandom(getPop()[i], box, exploit_num, pro, alg, rnd);
			}
			else if (type[i] == 3) {
				auto point_pairs = selectSubspaceFromObj(pro, rnd);
				auto& his_fronts_sols = getHisFrontSols();
				size_t count = 0;
				size_t num_var = pro->numberVariables();
				size_t num = (size_t)std::floor(pop_resource[i] / point_pairs.size());
				for (size_t j = 0; j < point_pairs.size(); ++j) {
					//根据点对确定bound
					auto sol1 = his_fronts_sols[point_pairs[j].first]->variable().vect();
					auto sol2 = his_fronts_sols[point_pairs[j].second]->variable().vect();
					...
					if (j == point_pairs.size() - 1) {
						num=exploit_num-count;
					}
					auto off = sampleRandom(getPop()[i], box, num, pro, alg, rnd);
					for (auto jj : off) {
				...
				for (auto& sub : pop2space) {
					...
					if (residual_space_inx.size() > 0) {
						auto temp_num = sub.second.size() * (1 - m_test_ratio);
						if (temp_num - std::floor(temp_num) >= 0.5) {
							ind_num = (size_t)std::ceil(temp_num);
						}
						else {
							ind_num = (size_t)std::floor(temp_num);
						}
					}
					else {
						ind_num = sub.second.size();
					}
				...
				//测试子代
				size_t test_num = getPopsize() - count;
				m_test_num = test_num;
				for (size_t j = 0; j < test_num; ++j) {
					...
					getPop()[i].getOffspring()[count + j].variable().vect() = sol;
					getPop()[i].getOffspring()[count + j].objective() = obj1;
	...
	std::vector<std::vector<Real>> SPMOEA4_2::sampleDiverse(Population<Solution<>>& pop, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA4_2::sampleRandom(Population<Solution<>>& pop, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_put;
		auto bound = CAST_CONOP(pro)->boundary();
	...
	std::vector<std::vector<Real>> SPMOEA4_2::sampleRandom(Population<Solution<>>& pop, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_put;
		//auto bound = CAST_CONOP(pro)->boundary();
	...
	bool SPMOEA4_2::spaceSubdivision(Problem* pro, Random* rnd) {
	...
	void SPMOEA4_2::updateLinkSubspace(size_t inx, const std::vector<size_t>& front_spaces, Random* rnd) {
	...
	void SPMOEA4_2::updateNeighSpace() {

	}

	void SPMOEA4_2::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
		for (size_t i = 0; i < getMO_HLC().numSubspace(); ++i) {
	...
	std::vector<std::vector<size_t>> SPMOEA4_2::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA4_2::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA4_2::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	//统计子空间前沿解的交互成功率
	void SPMOEA4_2::adaptiveSplit(Problem* pro) {
		for (size_t i = 0; i < getPop().size(); ++i) {
	...
	//统计子空间前沿解的交互成功率
	void SPMOEA4_2::adaptiveSplitFrontSpace(Problem* pro, Random* rnd) {
	...
	void SPMOEA4_2::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem* pro, Random* rnd) {
	...
	void SPMOEA4_2::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA4_2::findSplitDim(int inx, Problem* pro) {
	...
	void SPMOEA4_2::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
		for (size_t i = 0; i < pop.size(); ++i) {
	...
	void SPMOEA4_2::recordMetrics(Problem* pro, Algorithm* alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA4_2::initiObjSpace(Problem* pro) {
