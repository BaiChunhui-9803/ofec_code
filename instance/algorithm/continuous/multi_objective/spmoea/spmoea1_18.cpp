...
namespace ofec {
	...
	void SPMOEA1_18::initiVarSpace(Problem* pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA1_18::initPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//在各个子空间生成一个子种群
	...
	int SPMOEA1_18::evolve(Problem* pro, Algorithm* alg, Random* rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
		...
		*********************************************************************************************/
		...
		std::cout << "parent succ : " << m_total_results[0] << std::endl;
		std::cout << "offspr succ : " << m_total_results[1] << std::endl;
		std::cout << "nondominated: " << m_total_results[2] << std::endl;
		std::cout << std::endl;
	...
	void SPMOEA1_18::assignPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//先得到非前沿子空间
	...
	std::vector<Real> SPMOEA1_18::fitLine(size_t space_inx,const std::vector<std::vector<Real>> &data,Problem *pro,Random *rnd) {
		/*********************************************************
			*                   根据样本构造训练数据
			*********************************************************/
		std::vector<Real> weighs;
		std::vector<std::vector<Real>> train_data;
		std::vector<std::vector<Real>> origin_data;
		for (size_t i = 0; i < data.size(); ++i) {
			origin_data.emplace_back(data[i]);
		}
		...
		if (data.size() <= sample_num) {
			...
			rand_seq.resize(data.size());
			for (size_t i = 0; i < data.size(); ++i) {
				rand_seq[i] = i;
			}
			rnd->uniform.shuffle(rand_seq.begin(), rand_seq.end());
			for (size_t i = 0; i < sample_num; ++i) {
				train_data.push_back(data[rand_seq[i]]);
			}
		}
		...
		if (transform_coordinate) {
			...
			for (size_t i = 0; i < num_var; ++i) {
				t_origin[i] = bound[i].first;
			}
			//然后根据参考坐标原点变换训练数据和原始数据
			for (size_t i = 0; i < train_data.size(); ++i) {
				for (size_t j = 0; j < num_var; ++j) {
					train_data[i][j] = (train_data[i][j]- bound[j].first)/(bound[j].second-bound[j].first);
				}
			}
			for (size_t i = 0; i < origin_data.size(); ++i) {
				for (size_t j = 0; j < num_var; ++j) {
					origin_data[i][j] = (origin_data[i][j] - bound[j].first) / (bound[j].second - bound[j].first);
				}
			}
		}
		for (size_t i = 0; i < train_data.size(); ++i) {
	...
	void SPMOEA1_18::testCoverage(Problem* pro) {
		//检测子空间是否包含不同的PS片段
		...
		//std::cout << "push times: "  << m_total_push_times<< std::endl;
		//std::cout << "push succ : "  << m_push_results[0]  << "    "<<  (Real)m_push_results[0] / m_total_push_times << std::endl;
		//std::cout << "push fail : "  << m_push_results[1]  << "    " << (Real)m_push_results[1] / m_total_push_times << std::endl;
		//std::cout << "push equal: "  << m_push_results[2]  << "    " << (Real)m_push_results[2] / m_total_push_times << std::endl;
		//std::cout << std::endl;

		std::cout << "Operator results: " << std::endl;
		std::cout << "             " << "FIP   " <<"FEP   "<<"FIE   "<<"FEE   "<< "BIP   " << "BEP   " << "BIE   " << "BEE   " << std::endl;
		std::cout << "Opera succ : ";
		for (size_t i = 0; i < m_operator_results.size(); ++i) {
			std::cout <<std::setprecision(3)<< (Real)m_operator_results[i][0]/(0.0001+m_ind_operator_times[i])<<" ";
		}
		std::cout << std::endl;
		std::cout << "Opera fail : ";
		for (size_t i = 0; i < m_operator_results.size(); ++i) {
			std::cout << std::setprecision(3) << (Real)m_operator_results[i][1] / (0.0001 + m_ind_operator_times[i]) << " " ;
		}
		std::cout << std::endl;
		std::cout << "Opera equal: ";
		for (size_t i = 0; i < m_operator_results.size(); ++i) {
			std::cout << std::setprecision(3) << (Real)m_operator_results[i][2] / (0.0001 + m_ind_operator_times[i]) << " " ;
		}
		std::cout << std::endl;
		std::cout << "total times: " ;
		for (size_t i = 0; i < m_ind_operator_times.size(); ++i) {
			std::cout << m_ind_operator_times[i] << "   " ;
		}
		std::cout << std::endl;

		std::cout << "Extend results: " << std::endl;
		std::cout << "extend times: " << m_total_extend_times << std::endl;
		std::cout << "extend succ : " << m_extend_results[0] << "    " << (Real)m_extend_results[0] / m_total_extend_times << std::endl;
		std::cout << "extend fail : " << m_extend_results[1] << "    " << (Real)m_extend_results[1] / m_total_extend_times << std::endl;
		std::cout << "extebd equal: " << m_extend_results[2] << "    " << (Real)m_extend_results[2] / m_total_extend_times << std::endl;
		std::cout << std::endl;
	...
	std::vector<std::vector<Real>> SPMOEA1_18::sampleInFrontNeighSpace(std::vector<size_t>& front_link_spaces, size_t ind_inx, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA1_18::samplePush(Solution<>& sol, size_t sample_num, Real push_prob,Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	std::vector<std::vector<Real>> SPMOEA1_18::sampleByIndPos(Solution<>& sol, size_t sample_num, Real push_prob,Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		auto space = getMO_HLC().subspaceTree().getRegionIdx(sol.variable().vect());
		auto& box = getMO_HLC().subspaceTree().getBox(space);
		auto bound = CAST_CONOP(pro)->boundary();
	...
	std::pair<Real, std::vector<std::vector<Real>>> SPMOEA1_18::evolveInSubspace(size_t inx, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::pair<Real, std::vector<std::vector<Real>>> out_off;
		auto& bound = getMO_HLC().subspaceTree().getBox(inx);
		auto& space_his_sols = getMO_HLC().getSubspaceInfo(inx).m_history_inds;
		auto& space_front_sols = getMO_HLC().getSubspaceInfo(inx).m_subspace_front_sol;
		auto& global_front_sols = getMO_HLC().getSubspaceInfo(inx).m_front_sol_in_subspace;
		auto front_spaces = getFrontSpace();
	...
	std::vector<std::vector<Real>> SPMOEA1_18::sampleDiverse(Population<Solution<>>& pop, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	void SPMOEA1_18::SolutionSelection(Problem* pro, Random* rnd) {
		//后续解的选择需要考虑离已选解的分布
	...
	void SPMOEA1_18::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA1_18::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA1_18::updateSubPopSpace(size_t pop_inx, Problem* pro, Random* rnd) {
	...
	void SPMOEA1_18::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem* pro) {
	...
	void SPMOEA1_18::generateOffspring(Problem* pro, Algorithm* alg, Random* rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
		...
		for (size_t i = 0; i < getPop().size(); ++i) {
			...
			//	auto space_prob = spaceLinkProbability(front_spaces[j]);
			...
			if (type[i] == 1) {
				...
				for (size_t j = 0; j < exploit_num; ++j) {
					...
					//	}
					//	temp_pop.selectInCandidates(3, candidate, ridx, rnd);
					//	temp_pop[sele_inx].mutate(temp_pop.scalingFactor(), &temp_pop[ridx[0]], &temp_pop[ridx[1]], &temp_pop[ridx[2]], pro);
				...
				for (auto& sub : pop2space) {
					...
					//和邻域子空间内的个体交互
					std::vector<size_t> act_space = {idx,max_pair.first};
					PopDE<> temp_pop;
					std::vector<size_t> candidate_inx;
	...
	bool SPMOEA1_18::spaceSubdivision(Problem* pro, Random* rnd) {
	...
	void SPMOEA1_18::updateLinkSubspace(size_t inx, const std::vector<size_t>& front_spaces, Random* rnd) {
	...
	void SPMOEA1_18::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
		for (size_t i = 0; i < getMO_HLC().numSubspace(); ++i) {
	...
	std::vector<std::vector<size_t>> SPMOEA1_18::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA1_18::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA1_18::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	void SPMOEA1_18::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem* pro, Random* rnd) {
	...
	void SPMOEA1_18::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA1_18::findSplitDim(int inx, Problem* pro) {
	...
	void SPMOEA1_18::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
		for (size_t i = 0; i < pop.size(); ++i) {
	...
	void SPMOEA1_18::recordMetrics(Problem* pro, Algorithm* alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA1_18::initiObjSpace(Problem* pro) {
