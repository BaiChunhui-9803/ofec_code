...
namespace ofec {
	...
	void SPMOEA1_12::initiVarSpace(Problem *pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA1_12::initPop(Problem *pro, Algorithm *alg, Random *rnd) {
		//在各个子空间生成一个子种群
	...
	int SPMOEA1_12::evolve(Problem *pro, Algorithm *alg, Random *rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
		...
		if (!m_evolve_by_predict) {
			...
			for (size_t i = 0; i < getPop().size(); ++i) {
				...
				for (size_t j = 0; j < getPop()[i].size(); ++j) {
					...
					if (std::find(pop_inx.begin(), pop_inx.end(), space_inx) != pop_inx.end()) {
						pop_inx.push_back(space_inx);
					}
				}
				Real front_ratio = (Real)front_size / getPop()[i].size();
				if (front_ratio >= 1.) {
					if (pop_inx.size() > 1) {
		...
		***********************************************************************************/
		...
		if (m_divide_iteration % 1 == 0) {
			...
			if (!divide_flag) {
				m_stage_last_time++;
				if (m_stage_last_time > 5 && m_divide_granularity<=10) {
		...
		*********************************************************************************************/
		...
		//localCrowdSelection3(pro, rnd);
		//localCrowdSelection4(pro, rnd);
		ensembleSelection(getPop()[0].size(), pro, rnd);
		//indicatorSelection(pro, rnd);
		//subspaceSelection(pro,rnd);
	...
	void SPMOEA1_12::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA1_12::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA1_12::updateSubPopSpace(size_t pop_inx, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_12::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem *pro) {
	...
	void SPMOEA1_12::generateOffspring(Problem *pro, Algorithm *alg, Random *rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
		...
		for (size_t i = 0; i < getPop().size(); ++i) {
			...
			if (type[i] == 1) {
			...
			else if (type[i] == 4) {//基于个体采样
				for (size_t j = 0; j < getPop()[i].size(); ++j) {
					auto& sol1 = getPop()[i][j].variable().vect();
					auto space_inx = getMO_HLC().subspaceTree().getRegionIdx(sol1);
					if (std::find(front_spaces.begin(), front_spaces.end(), space_inx) != front_spaces.end()) {
						//找个体所属的连通子空间，再基于个体采样
						size_t cluster_inx = 0;
						for (size_t k = 0; k < front_clusters.size(); ++k) {
							if (std::find(front_clusters[k].begin(), front_clusters[k].end(), space_inx) != front_clusters[k].end()) {
								cluster_inx = k;
								break;
							}
						}
						...
						auto off = sampleByImprove(getPop()[i][j], 1, pro, alg, rnd);
						for (size_t k = 0; k < off.size(); ++k) {
							all_off.emplace_back(off[k]);
						}
						operator_ratio[0] += 1.;
				...
				//	auto off = sampleByImprove(getPop()[i][no_front_inds[j]], 1, pro, alg, rnd);
				//	for (size_t k = 0; k < off.size(); ++k) {
				//		all_off.emplace_back(off[k]);
				//	}
				...
				//	//找个体所属的连通子空间，再基于个体采样
				//	auto& sol = getPop()[i][front_inds[j]].variable().vect();
				//	auto space_inx = getMO_HLC().subspaceTree().getRegionIdx(sol);
				//	size_t cluster_inx = 0;
				//	for (size_t k = 0; k < front_clusters.size(); ++k) {
				//		if (std::find(front_clusters[k].begin(), front_clusters[k].end(),space_inx) != front_clusters[k].end()) {
				...
				//		}
				//	}
				//	auto off = sampleInFrontNeighSpace(front_clusters[cluster_inx],sol, bound, 1, pro, alg, rnd);
				//	for (size_t k = 0; k < off.size(); ++k) {
				//		all_off.emplace_back(off[k]);
				//	}
	...
	bool SPMOEA1_12::spaceSubdivision(Problem *pro, Random *rnd) {
	...
	void SPMOEA1_12::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
	...
	std::vector<std::vector<size_t>> SPMOEA1_12::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA1_12::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA1_12::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	void SPMOEA1_12::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_12::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA1_12::findSplitDim(int inx, Problem *pro) {
	...
	void SPMOEA1_12::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
	...
	void SPMOEA1_12::recordMetrics(Problem *pro, Algorithm *alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA1_12::initiObjSpace(Problem *pro) {
