...
namespace ofec {
	...
	void SPMOEA4_7::initialize_() {
		...
		m_test_rate = v.get<Real>("explore rate");
		m_exploit_rate = v.get<Real>("exploit rate");
	...
	void SPMOEA4_7::initiVarSpace(Problem* pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA4_7::initPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//在各个子空间生成一个子种群
		...
		size_t test_pop_num = (size_t)std::floor(pop_num * m_test_rate);
		size_t exploit_pop_num = (size_t)std::floor(pop_num * m_exploit_rate);
		size_t main_pop_num = pop_num - test_pop_num-exploit_pop_num;
		...
		for (size_t i = main_pop_num; i < main_pop_num + test_pop_num; ++i) {
			inx2.push_back(i);
		}
		m_pop_index.emplace_back(inx2);
		std::vector<size_t> inx3;
		for (size_t i = main_pop_num+test_pop_num; i < main_pop_num + test_pop_num+exploit_pop_num; ++i) {
			inx3.push_back(i);
		}
		m_pop_index.emplace_back(inx3);
		getExploitSubspace().setBoxBound(bound);
		Population<Solution<>> exploit_pop;
		for (size_t i = 0; i < exploit_pop_num; ++i) {
			exploit_pop.append(temp_pop[inx3[i]]);
		}
		getExploitSubspace().updateSubspaceInfo(exploit_pop,pro);
	...
	int SPMOEA4_7::evolve(Problem* pro, Algorithm* alg, Random* rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
		...
		*********************************************************************************************/
		...
		for (size_t j = 0; j < m_pop_index[2].size(); ++j) {
			temp_pop2.append(getPop()[0][m_pop_index[2][j]]);
		}
		for (size_t j = 0; j < m_pop_index[2].size(); ++j) {
			temp_pop2.append(getPop()[0].getOffspring()[m_pop_index[2][j]]);
		}
		...
		if (front_sol_num > 0) {
			sele_inx = selectMaxMinFromFront(all_data, ranks, sele_num);
		}
		else {
			sele_inx = selectByNorm(all_data, sele_num,pro,rnd);
		}

		for (size_t j = 0; j < m_pop_index[0].size() - keep_inx.size(); ++j) {
			getPop()[0][j] = temp_pop1[sele_inx[j]];
		}
		for (size_t j = 0; j < keep_inx.size(); ++j) {
		...
		//更新精搜种群，子空间内排序，与历史解一起排
		if (keep_inx.size() >= m_pop_index[2].size() || m_exploit_fail_times >= 5 || m_exploit_succ_times >= 5) {
			m_explore_reset_flag = true;
			m_exploit_fail_times = 0;
			m_exploit_succ_times = 0;
			for (size_t j = 0; j < m_pop_index[2].size(); ++j) {
				getPop()[0][m_pop_index[2][j]] = getPop()[0].getOffspring()[m_pop_index[2][j]];
			}
		}
		else {
			if (m_explore_reset_flag) {
				...
				size_t sele_num2 = m_pop_index[2].size() - keep_inx.size();
				std::vector<std::vector<Real>> all_data2;
				std::vector<size_t> ranks2;
				std::vector<size_t> candidates;
				for (size_t j = 0; j < temp_pop2.size(); ++j) {
					if (std::find(keep_inx.begin(), keep_inx.end(), j) == keep_inx.end()) {
						all_data2.emplace_back(temp_pop2[j].objective());
						ranks2.push_back(temp_pop2[j].fitness());
						candidates.push_back(j);
					}
				}
				auto sele_inx2 = selectMaxMinFromFront(all_data2, ranks2, sele_num2);
				for (size_t j = 0; j < sele_num2; ++j) {
					getPop()[0][m_pop_index[2][j]] = temp_pop2[candidates[sele_inx2[j]]];
				}
				for (size_t j = 0; j < keep_inx.size(); ++j) {
					getPop()[0][m_pop_index[2][sele_num2 + j]] = temp_pop2[keep_inx[j]];
				}
				...
				for (size_t j = 0; j < temp_pop2.size(); ++j) {
					if (temp_pop2[j].fitness() == 0) {
						first_num++;
						if (j >= m_pop_index[2].size()) {
							better_off_num++;
						}
					}
				}
				if (keep_inx.size() < 1) {
					m_exploit_fail_times++;
				}
				else {
					m_exploit_fail_times = 0;
					m_exploit_succ_times++;
				}

				std::cout << "the number of front solutions: " << keep_inx.size() << std::endl;
				std::cout << "the  number   of  succ times : " << m_exploit_succ_times << std::endl;
				std::cout << "the  number   of  fail times : " << m_exploit_fail_times << std::endl;
			}
		}
	...
	void SPMOEA4_7::assignPop(Population<Solution<>>& pop, Problem* pro, Algorithm* alg, Random* rnd) {
		//先得到非前沿子空间
	...
	void SPMOEA4_7::testCoverage(Problem* pro) {
		//检测子空间是否包含不同的PS片段
	...
	std::vector<std::vector<Real>> SPMOEA4_7::sampleInFrontNeighSpace(std::vector<size_t>& front_link_spaces, size_t ind_inx, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA4_7::sampleLinearInFrontSpace(size_t space, size_t ind_inx, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		//子连通空间内采样
		auto& sol = getPop()[0][ind_inx].variable().vect();
		for (size_t i = 0; i < sample_num; ++i) {
	...
	std::vector<std::vector<Real>> SPMOEA4_7::samplePush(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	std::vector<std::vector<Real>> SPMOEA4_7::sampleByIndPos(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		auto space = getMO_HLC().subspaceTree().getRegionIdx(sol.variable().vect());
		auto& box = getMO_HLC().subspaceTree().getBox(space);
		auto bound = CAST_CONOP(pro)->boundary();
	...
	void SPMOEA4_7::SolutionSelection(Problem* pro, Random* rnd) {
		//后续解的选择需要考虑离已选解的分布
	...
	void SPMOEA4_7::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA4_7::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA4_7::updateSubPopSpace(size_t pop_inx, Problem* pro, Random* rnd) {
	...
	void SPMOEA4_7::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem* pro) {
	...
	void SPMOEA4_7::generateOffspring(Problem* pro, Algorithm* alg, Random* rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
		...
		for (size_t i = 0; i < getPop().size(); ++i) {
			...
			std::vector<size_t> residual_space_inx;//剩余可细分的前沿子空间
			...
			if (type[i] == 0) {
				...
				auto main_pop = m_pop_index[0];
				auto test_pop = m_pop_index[1];
				auto exploit_pop = m_pop_index[2];

				for (size_t j = 0; j < main_pop.size(); ++j) {
					//auto off = samplePush(getPop()[i][j], 1, 0.2, pro, alg, rnd);
					//auto off = sampleByIndPos(getPop()[i][j], 1,0.5, pro, alg, rnd);
					auto space = getMO_HLC().subspaceTree().getRegionIdx(getPop()[i][main_pop[j]].variable().vect());
		            //auto& box = getMO_HLC().subspaceTree().getBox(space);
		            auto& his_sols = getMO_HLC().getSubspaceInfo(space).m_history_inds;
					auto off = sampleByPush(getPop()[i][main_pop[j]], his_sols,1, 0.5, pro, alg, rnd);
					for (size_t k = 0; k < off.size(); ++k) {
						all_off.emplace_back(off[k]);
					}

					getPop()[i].getOffspring()[main_pop[j]].variable() = getInteractiveSols().back().back()->variable();
					getPop()[i].getOffspring()[main_pop[j]].objective() = getInteractiveSols().back().back()->objective();
				...
				if (residual_space_inx.size() > 0) {
					for (size_t j = 0; j < test_pop.size(); ++j) {
						...
						getPop()[i].getOffspring()[test_pop[j]].variable() = getInteractiveSols().back().back()->variable();
						getPop()[i].getOffspring()[test_pop[j]].objective() = getInteractiveSols().back().back()->objective();
				...
				if (m_explore_reset_flag) {
					...
					for (size_t k = 0; k < getPop()[i].size(); ++k) {
						if (std::find(exploit_pop.begin(), exploit_pop.end(), k) == exploit_pop.end()) {
							residual_inx.push_back(k);
						}
					}
					//根据点对确定bound
					auto point_pair = max_pair.second;
					size_t num_var = pro->numberVariables();
					auto sol1 = getPop()[i][residual_inx[point_pair.first]].variable().vect();
					auto sol2 = getPop()[i][residual_inx[point_pair.second]].variable().vect();
					...
					*/
					...
					size_t ind_inx = 0;//稀疏点个体索引
					std::vector<Real> center;
					if (rr < 0.5) {
						center = sol1;
						ind_inx = point_pair.first;
					}
					else {
						center = sol2;
						ind_inx = point_pair.second;
					}
					...
					for (size_t k = 0; k < num_var; ++k) {
						...
						std::pair<Real, Real> temp;
						Real lower = center[k] - span / 50;
						temp.first = lower > search_bound[k].first ? lower : search_bound[k].first;
						Real upper = center[k] + span / 50;
						temp.second = upper < search_bound[k].second ? upper : search_bound[k].second;
						box.emplace_back(temp);
					}
					//m_reset_box = box;
					getExploitSubspace().setBoxBound(box);
					getExploitSubspace().clearInfos();
					Population<Solution<>> update_pop;
					int method2 = 1;
					Real rand2 = rnd->uniform.next();
					if (rand2 < 1) {
						method2 = 1;
					...
					if (method2 == 1) {
						//邻域空间产生子代
						//随机生成
						for (size_t j = 0; j < exploit_pop.size(); ++j) {
							std::vector<Real> sol;
							for (size_t j = 0; j < box.size(); ++j) {
								sol.push_back(box[j].first + rnd->uniform.next() * (box[j].second - box[j].first));
							}
							Solution<> ind4(getPop()[i][exploit_pop[j]]);
							ind4.variable().vect() = sol;
							ind4.evaluate(pro, alg);
							std::vector<std::shared_ptr<Solution<>>> temp_pair;
							temp_pair.emplace_back(std::make_shared<Solution<>>(getPop()[i][exploit_pop[j]]));
							temp_pair.emplace_back(std::make_shared<Solution<>>(ind4));
							getInteractiveSols().emplace_back(temp_pair);

							getPop()[i].getOffspring()[exploit_pop[j]].variable() = ind4.variable();
							getPop()[i].getOffspring()[exploit_pop[j]].objective() = ind4.objective();

							update_pop.append(getPop()[i].getOffspring()[exploit_pop[j]]);
						}
					}
					else {
						//近邻交互产生一定数量子代
						...
						for (size_t j = 0; j < residual_inx.size(); ++j) {
							main_pop.append(getPop()[i][residual_inx[j]]);
						}
						auto ind_pos = std::distance(residual_inx.begin(), std::find(residual_inx.begin(), residual_inx.end(), ind_inx));
						auto neighs = main_pop.nearestNeighbour(ind_inx, pro, m_neighs - 1);
						PopDE<> exp_pop;
						exp_pop.append(main_pop[ind_inx]);
						std::vector<size_t> candidate, ridx;
						candidate.push_back(0);
						for (auto jj : neighs) {
							exp_pop.append(main_pop[jj.second]);
							candidate.push_back(candidate.size());
						}

						for (size_t j = 0; j < exploit_pop.size(); ++j) {
							exp_pop.selectInCandidates(3, candidate, ridx, rnd);
							exp_pop[ridx[0]].mutate(exp_pop.scalingFactor(), &exp_pop[ridx[0]], &exp_pop[ridx[1]], &exp_pop[ridx[2]], pro);
							exp_pop.recombine(ridx[0], rnd, pro);

							//越界处理
							//SPMOEA::repairSol(exploit_pop[j].trial().variable().vect(), m_reset_box, rnd);

							exp_pop[ridx[0]].trial().evaluate(pro, this);

							Solution<> ind4(exp_pop[ridx[0]]);
							ind4.variable() = exp_pop[ridx[0]].trial().variable();
							ind4.objective() = exp_pop[ridx[0]].trial().objective();
							std::vector<std::shared_ptr<Solution<>>> temp_pair;
							temp_pair.emplace_back(std::make_shared<Solution<>>(exp_pop[ridx[0]]));
							temp_pair.emplace_back(std::make_shared<Solution<>>(ind4));
							getInteractiveSols().emplace_back(temp_pair);
					...
					getExploitSubspace().updateSubspaceInfo(update_pop, pro);
				}
				else {
					auto& his_sols = getExploitSubspace().getSubspaceHisSols();
					for (size_t j = 0; j < exploit_pop.size(); ++j) {
						//auto off = samplePush(getPop()[i][j], 1, 0.2, pro, alg, rnd);
						//auto off = sampleByIndPos(getPop()[i][j], 1,0.5, pro, alg, rnd);
						auto off = sampleByPush(getPop()[i][exploit_pop[j]], his_sols, 1, 0.5, pro, alg, rnd);
						for (size_t k = 0; k < off.size(); ++k) {
							all_off.emplace_back(off[k]);
						}

						getPop()[i].getOffspring()[exploit_pop[j]].variable() = getInteractiveSols().back().back()->variable();
						getPop()[i].getOffspring()[exploit_pop[j]].objective() = getInteractiveSols().back().back()->objective();
					}
				}
			}
				...
				auto exploit_pop = m_pop_index[2];
				for (size_t j = 0; j < getPop()[i].size(); ++j) {
					if (std::find(main_pop.begin(), main_pop.end(), j) != main_pop.end()) {
				...
				for (auto& sub : pop2space) {
					...
					if (method == 1) {//有个体的子空间
						...
						for (size_t j = 0; j < main_pop.size(); ++j) {
							if (std::find(sub.second.begin(), sub.second.end(),main_pop[j]) == sub.second.end()) {
								temp_pop.append(getPop()[i][main_pop[j]]);
							}
						}
					}
					for (size_t j = 0; j < sub.second.size(); ++j) {
						...
						getPop()[i].getOffspring()[count].variable() = ind4.variable();
						getPop()[i].getOffspring()[count].objective() = ind4.objective();
				...
				if (m_explore_reset_flag) {
					...
					*/
					...
					std::vector<size_t> candidate, ridx;
					for (size_t j = 0; j < exploit_pop.size(); ++j) {
						temp_pop.append(getPop()[i][exploit_pop[j]]);
						candidate.push_back(j);
					}
					auto exploit_box = getExploitSubspace().getBox();
					for (size_t j = 0; j < temp_pop.size(); ++j) {
						temp_pop.selectInCandidates(3, candidate, ridx, rnd);
						temp_pop[j].mutate(temp_pop.scalingFactor(), &temp_pop[ridx[0]], &temp_pop[ridx[1]], &temp_pop[ridx[2]], pro);
						temp_pop.recombine(j, rnd, pro);

						//越界处理
						SPMOEA::repairSol(temp_pop[j].trial().variable().vect(),exploit_box , rnd);
	...
	//基于子空间个体采样，向个体所在子空间前沿解推进，提升个体
	std::vector<std::vector<Real>> SPMOEA4_7::sampleByPush(Solution<>& sol, const std::vector<std::shared_ptr<Solution<>>>& his_sols, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		//auto space = getMO_HLC().subspaceTree().getRegionIdx(sol.variable().vect());
		//auto& box = getMO_HLC().subspaceTree().getBox(space);
		//auto bound = CAST_CONOP(pro)->boundary();
	...
	size_t SPMOEA4_7::selectSubspace(Problem* pro, Random* rnd) {
	...
	std::map<Real, std::pair<size_t, size_t>> SPMOEA4_7::selectSubspaceFromObj(Problem* pro, Random* rnd) {
		//将目标空间所有非支配解分两类
	...
	std::vector<std::vector<Real>> SPMOEA4_7::sampleDiverse(Population<Solution<>>& pop, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA4_7::sampleRandom(Population<Solution<>>& pop, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_put;
		//auto bound = CAST_CONOP(pro)->boundary();
	...
	bool SPMOEA4_7::spaceSubdivision(Problem* pro, Random* rnd) {
	...
	void SPMOEA4_7::updateLinkSubspace(size_t inx, const std::vector<size_t>& front_spaces, Random* rnd) {
	...
	void SPMOEA4_7::updateNeighSpace() {

	}

	void SPMOEA4_7::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
		for (size_t i = 0; i < getMO_HLC().numSubspace(); ++i) {
	...
	std::vector<std::vector<size_t>> SPMOEA4_7::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA4_7::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA4_7::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	//统计子空间前沿解的交互成功率
	void SPMOEA4_7::adaptiveSplit(Problem* pro) {
		for (size_t i = 0; i < getPop().size(); ++i) {
	...
	//统计子空间前沿解的交互成功率
	void SPMOEA4_7::adaptiveSplitFrontSpace(Problem* pro, Random* rnd) {
	...
	void SPMOEA4_7::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem* pro, Random* rnd) {
	...
	void SPMOEA4_7::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA4_7::findSplitDim(int inx, Problem* pro) {
	...
	void SPMOEA4_7::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
		for (size_t i = 0; i < pop.size(); ++i) {
	...
	void SPMOEA4_7::recordMetrics(Problem* pro, Algorithm* alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA4_7::initiObjSpace(Problem* pro) {
