...
namespace ofec {
	...
	void SPMOEA4_5::initiVarSpace(Problem* pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA4_5::initPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//在各个子空间生成一个子种群
	...
	int SPMOEA4_5::evolve(Problem* pro, Algorithm* alg, Random* rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
	...
	void SPMOEA4_5::assignPop(Population<Solution<>>& pop, Problem* pro, Algorithm* alg, Random* rnd) {
		//先得到非前沿子空间
	...
	void SPMOEA4_5::testCoverage(Problem* pro) {
		//检测子空间是否包含不同的PS片段
	...
	std::vector<std::vector<Real>> SPMOEA4_5::sampleInFrontNeighSpace(std::vector<size_t>& front_link_spaces, size_t ind_inx, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA4_5::sampleLinearInFrontSpace(size_t space, size_t ind_inx, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		//子连通空间内采样
		auto& sol = getPop()[0][ind_inx].variable().vect();
		for (size_t i = 0; i < sample_num; ++i) {
	...
	std::vector<std::vector<Real>> SPMOEA4_5::samplePush(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	std::vector<std::vector<Real>> SPMOEA4_5::sampleByIndPos(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		auto space = getMO_HLC().subspaceTree().getRegionIdx(sol.variable().vect());
		auto& box = getMO_HLC().subspaceTree().getBox(space);
		auto bound = CAST_CONOP(pro)->boundary();
	...
	void SPMOEA4_5::SolutionSelection(Problem* pro, Random* rnd) {
		//后续解的选择需要考虑离已选解的分布
	...
	void SPMOEA4_5::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA4_5::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA4_5::updateSubPopSpace(size_t pop_inx, Problem* pro, Random* rnd) {
	...
	void SPMOEA4_5::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem* pro) {
	...
	void SPMOEA4_5::generateOffspring(Problem* pro, Algorithm* alg, Random* rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
					...
					*/
					...
					if (method2 == 1) {
						...
						for (size_t j = 0; j < first_pop.size(); ++j) {
							auto& sol3 = getPop()[i][first_pop[j]].variable().vect();
							bool flag = true;
							for (size_t k = 0; k < box.size(); ++k) {
								if (sol3[k]<box[k].first || sol3[k] > box[k].second) {
									flag = false;
									break;
								}
							}
							if (flag) {
								exploit_pop.append(getPop()[i][first_pop[j]]);
							}
						}
						if (exploit_pop.size() < 500) {
							//随机生成
							...
							for (size_t k = 0; k < exploit_pop.size();++k) {
								candidate.push_back(k);
							}
							for (size_t j = 0; j < second_pop.size(); ++j) {
								exploit_pop.selectInCandidates(3, candidate, ridx, rnd);
								exploit_pop[ridx[0]].mutate(exploit_pop.scalingFactor() * rnd->uniform.next(), &exploit_pop[ridx[0]], &exploit_pop[ridx[1]], &exploit_pop[ridx[2]], pro);
								exploit_pop.recombine(ridx[0], rnd, pro);

								//越界处理
								SPMOEA::repairSol(exploit_pop[ridx[0]].trial().variable().vect(), m_reset_box, rnd);

								exploit_pop[ridx[0]].trial().evaluate(pro, this);

								Solution<> ind4(exploit_pop[ridx[0]]);
								ind4.variable() = exploit_pop[ridx[0]].trial().variable();
								ind4.objective() = exploit_pop[ridx[0]].trial().objective();
								std::vector<std::shared_ptr<Solution<>>> temp_pair;
								temp_pair.emplace_back(std::make_shared<Solution<>>(exploit_pop[ridx[0]]));
								temp_pair.emplace_back(std::make_shared<Solution<>>(ind4));
								getInteractiveSols().emplace_back(temp_pair);
						...
						PopDE<> main_pop;
						for (size_t j = 0; j < first_pop.size(); ++j) {
							main_pop.append(getPop()[i][first_pop[j]]);
						}
						auto neighs = main_pop.nearestNeighbour(ind_inx, pro, m_neighs);
						PopDE<> exploit_pop;
						exploit_pop.append(getPop()[i][ind_inx]);
						std::vector<size_t> candidate, ridx;
						candidate.push_back(0);
						for (auto jj : neighs) {
							exploit_pop.append(main_pop[jj.second]);
	...
	//基于子空间个体采样，向个体所在子空间前沿解推进，提升个体
	std::vector<std::vector<Real>> SPMOEA4_5::sampleByPush(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		auto space = getMO_HLC().subspaceTree().getRegionIdx(sol.variable().vect());
		auto& box = getMO_HLC().subspaceTree().getBox(space);
		auto bound = CAST_CONOP(pro)->boundary();
	...
	size_t SPMOEA4_5::selectSubspace(Problem* pro, Random* rnd) {
	...
	std::map<Real, std::pair<size_t, size_t>> SPMOEA4_5::selectSubspaceFromObj(Problem* pro, Random* rnd) {
		//将目标空间所有非支配解分两类
	...
	std::vector<std::vector<Real>> SPMOEA4_5::sampleDiverse(Population<Solution<>>& pop, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA4_5::sampleRandom(Population<Solution<>>& pop, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_put;
		//auto bound = CAST_CONOP(pro)->boundary();
	...
	bool SPMOEA4_5::spaceSubdivision(Problem* pro, Random* rnd) {
	...
	void SPMOEA4_5::updateLinkSubspace(size_t inx, const std::vector<size_t>& front_spaces, Random* rnd) {
	...
	void SPMOEA4_5::updateNeighSpace() {

	}

	void SPMOEA4_5::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
		for (size_t i = 0; i < getMO_HLC().numSubspace(); ++i) {
	...
	std::vector<std::vector<size_t>> SPMOEA4_5::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA4_5::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA4_5::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	//统计子空间前沿解的交互成功率
	void SPMOEA4_5::adaptiveSplit(Problem* pro) {
		for (size_t i = 0; i < getPop().size(); ++i) {
	...
	//统计子空间前沿解的交互成功率
	void SPMOEA4_5::adaptiveSplitFrontSpace(Problem* pro, Random* rnd) {
	...
	void SPMOEA4_5::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem* pro, Random* rnd) {
	...
	void SPMOEA4_5::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA4_5::findSplitDim(int inx, Problem* pro) {
	...
	void SPMOEA4_5::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
		for (size_t i = 0; i < pop.size(); ++i) {
	...
	void SPMOEA4_5::recordMetrics(Problem* pro, Algorithm* alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA4_5::initiObjSpace(Problem* pro) {
