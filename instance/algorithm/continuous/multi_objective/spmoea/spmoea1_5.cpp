...
namespace ofec {
	...
	void SPMOEA1_5::initiVarSpace(Problem *pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA1_5::initPop(Problem *pro, Algorithm *alg, Random *rnd) {
		//在各个子空间生成一个子种群
	...
	int SPMOEA1_5::evolve(Problem *pro, Algorithm *alg, Random *rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
	...
	void SPMOEA1_5::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA1_5::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA1_5::updateSubPopSpace(size_t pop_inx, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_5::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t eval_num, Problem *pro) {
		//根据种群的历史前沿解在边界内的比例分配计算资源
		size_t max_pop_size = getPopsize();
		updateFrontRegionLinkSpace();
		if (m_divide_iteration % 2 == 0) {
			for (size_t i = 0; i < getPop().size(); ++i) {
				assign_pop_resource.push_back(max_pop_size/2);
			}
		}
		else {
			...
			if (front_link_spaces.size() == 1) {
				...
				assign_pop_resource.push_back(total_size+15 * front_link_spaces.size());
			}
		}
		updatePopResource(assign_pop_resource);
	}

	void SPMOEA1_5::generateOffspring(Problem *pro, Random *rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
		...
		for (size_t i = 0; i < getPop().size(); ++i) {
			...
			if (type[i] == 1) {
				...
				if (front_link_spaces.size() == 1) {//子空间连通
					...
					auto all_off = sampleByManifoldSpaces(front_link_spaces[0], manifold_dist, pop_resource.back(), pro, rnd);
					...
					for (size_t j = 0; j < front_link_spaces.size(); ++j) {
						...
						auto off = sampleByLinkSpaces(front_link_spaces[j], manifold_dist,10, pro, rnd);
						//子连通集内部交互
						auto off2 = sampleInLinkSpaces(front_link_spaces[j],bound, CAST_CONOP(pro)->numberObjectives() * front_link_spaces[j].size(), pro, rnd);
						//为子代赋值
	...
	void SPMOEA1_5::spaceSubdivision(Problem *pro, Random *rnd) {
	...
	void SPMOEA1_5::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
	...
	std::vector<std::vector<size_t>> SPMOEA1_5::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA1_5::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA1_5::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	void SPMOEA1_5::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_5::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA1_5::findSplitDim(int inx, Problem *pro) {
	...
	void SPMOEA1_5::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
	...
	void SPMOEA1_5::recordMetrics(Problem *pro, Algorithm *alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA1_5::initiObjSpace(Problem *pro) {
