...
// 在spmoea1_17的基础上，控制迁移个体的位置选择，控制前沿子空间交互

#ifndef OFEC_SPMOEA1_18_H
#define OFEC_SPMOEA1_18_H

#include "spmoea.h"

namespace ofec {

	class SPMOEA1_18 :public SPMOEA {
	public:
		...
		void assignPop(Problem* pro, Algorithm* alg, Random* rnd);

		std::vector<Real> fitLine(size_t space_inx, const std::vector<std::vector<Real>>& data, Problem* pro, Random* rnd);
		...
		std::pair<Real,std::vector<std::vector<Real>>> evolveInSubspace(size_t inx, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd);
	...
	private:
		...
		enum class m_operator_action { push, extend, diverse };
	};

}

#endif  // !OFEC_SPMOEA1_18_H