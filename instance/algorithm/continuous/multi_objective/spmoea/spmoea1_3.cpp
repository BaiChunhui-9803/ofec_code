...
namespace ofec {
	...
	void SPMOEA1_3::initiVarSpace(Problem* pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA1_3::initPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//在各个子空间生成一个子种群
	...
	int SPMOEA1_3::evolve(Problem* pro, Algorithm* alg, Random* rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
	...
	void SPMOEA1_3::assignPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//先得到非前沿子空间
	...
	void SPMOEA1_3::testCoverage(Problem* pro) {
		//检测子空间是否包含不同的PS片段
	...
	void SPMOEA1_3::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA1_3::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA1_3::updateSubPopSpace(size_t pop_inx, Problem* pro, Random* rnd) {
	...
	void SPMOEA1_3::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem* pro) {
	...
	void SPMOEA1_3::generateOffspring(Problem* pro, Algorithm* alg, Random* rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
	...
	bool SPMOEA1_3::spaceSubdivision(Problem* pro, Random* rnd) {
	...
	void SPMOEA1_3::updateLinkSubspace(size_t inx, const std::vector<size_t>& front_spaces, Random* rnd) {
	...
	void SPMOEA1_3::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
		for (size_t i = 0; i < getMO_HLC().numSubspace(); ++i) {
	...
	std::vector<std::vector<size_t>> SPMOEA1_3::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA1_3::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA1_3::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	void SPMOEA1_3::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem* pro, Random* rnd) {
	...
	void SPMOEA1_3::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA1_3::findSplitDim(int inx, Problem* pro) {
	...
	void SPMOEA1_3::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
	...
	void SPMOEA1_3::recordMetrics(Problem* pro, Algorithm* alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA1_3::initiObjSpace(Problem* pro) {
