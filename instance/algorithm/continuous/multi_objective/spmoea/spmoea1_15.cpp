...
namespace ofec {
	...
	void SPMOEA1_15::initiVarSpace(Problem *pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA1_15::initPop(Problem *pro, Algorithm *alg, Random *rnd) {
		//在各个子空间生成一个子种群
	...
	int SPMOEA1_15::evolve(Problem *pro, Algorithm *alg, Random *rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
	...
	void SPMOEA1_15::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA1_15::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA1_15::updateSubPopSpace(size_t pop_inx, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_15::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem *pro) {
	...
	void SPMOEA1_15::generateOffspring(Problem *pro, Algorithm *alg, Random *rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
			...
			if (type[i] == 1) {
				...
				for (size_t j = 0; j < getPop()[i].size(); ++j) {
					...
					if (std::find(front_spaces.begin(), front_spaces.end(), space_inx) != front_spaces.end()) {
						...
						auto off = sampleInFrontSubspace(space_inx, j, front_clusters[cluster_inx], 1, pro, alg, rnd);
						...
						//非前沿子空间采样
						auto off = sampleInSubspace(space_inx, j, 1, pro, alg, rnd);
	...
	bool SPMOEA1_15::spaceSubdivision(Problem *pro, Random *rnd) {
	...
	void SPMOEA1_15::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
	...
	std::vector<std::vector<size_t>> SPMOEA1_15::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA1_15::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA1_15::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	void SPMOEA1_15::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_15::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA1_15::findSplitDim(int inx, Problem *pro) {
	...
	void SPMOEA1_15::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
	...
	void SPMOEA1_15::recordMetrics(Problem *pro, Algorithm *alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA1_15::initiObjSpace(Problem *pro) {
