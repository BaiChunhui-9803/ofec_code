...
namespace ofec {
	...
	void SPMOEA4_3::initiVarSpace(Problem* pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA4_3::initPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//在各个子空间生成一个子种群
	...
	int SPMOEA4_3::evolve(Problem* pro, Algorithm* alg, Random* rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
		...
		*********************************************************************************************/
		...
		if (m_divide_iteration % m_switch_period < m_num_push) {
			...
			auto sele_inx = selectMaxMinFromFront(all_data, ranks, m_pop_index[0].size());
			for (size_t j = 0; j < m_pop_index[0].size(); ++j) {
				getPop()[0][m_pop_index[0][j]] = temp_pop1[sele_inx[j]];
			}

			/*size_t first_num = 0;
			SPMOEA_pop temp_pop3(0, pro);
			for (size_t j = 0; j < temp_pop1.size(); ++j) {
				if (temp_pop1[j].fitness() == 0) {
					first_num++;
					temp_pop3.append(temp_pop1[j]);
				}
			}
			if (first_num < m_pop_index[0].size()) {
				SPMOEA_pop temp_pop4(0, pro);
				for (size_t j = 0; j < m_pop_index[0].size(); ++j) {
					temp_pop4.append(getPop()[0][m_pop_index[0][j]]);
				}
				temp_pop4.evalDense(temp_pop4, temp_pop1);
				for (size_t j = 0; j < m_pop_index[0].size(); ++j) {
					getPop()[0][m_pop_index[0][j]] = temp_pop4[j];
				}
			}
			else {
				auto sele_inx = selectMaxMinFromFront(temp_pop3, m_pop_index[0].size());
				for (size_t j = 0; j < m_pop_index[0].size(); ++j) {
					getPop()[0][m_pop_index[0][j]] = temp_pop3[sele_inx[j]];
				}
			}*/

			//更新探索种群，子空间内排序，与历史解一起排
			if (keep_inx.size() >= 0.5 * m_pop_index[1].size() || m_explore_fail_times >= 70) {
				...
				/*if (keep_inx.size() < 0.2 * m_pop_index[1].size()) {
				...
				if (m_explore_reset_flag) {
					...
					/*SPMOEA_pop temp_pop6(0, pro);
					for (size_t j = 0; j < m_pop_index[1].size(); ++j) {
						temp_pop6.append(getPop()[0][m_pop_index[1][j]]);
					}
					auto sele_inx = temp_pop6.denseSelect(temp_pop6, temp_pop2);
					size_t better_off_num = 0;

					for (size_t j = 0; j < sele_inx.size(); ++j) {
						getPop()[0][m_pop_index[1][j]] = temp_pop2[sele_inx[j]];
					}
					for (size_t j = m_pop_index[1].size(); j < temp_pop2.size(); ++j) {
					...
					std::cout << "the number of better solutions: " << keep_inx.size() << std::endl;
				}
			}
	...
	void SPMOEA4_3::assignPop(Population<Solution<>>& pop, Problem* pro, Algorithm* alg, Random* rnd) {
		//先得到非前沿子空间
	...
	void SPMOEA4_3::testCoverage(Problem* pro) {
		//检测子空间是否包含不同的PS片段
	...
	std::vector<std::vector<Real>> SPMOEA4_3::sampleInFrontNeighSpace(std::vector<size_t>& front_link_spaces, size_t ind_inx, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA4_3::sampleLinearInFrontSpace(size_t space, size_t ind_inx, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		//子连通空间内采样
		auto& sol = getPop()[0][ind_inx].variable().vect();
		for (size_t i = 0; i < sample_num; ++i) {
	...
	std::vector<std::vector<Real>> SPMOEA4_3::samplePush(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	std::vector<std::vector<Real>> SPMOEA4_3::sampleByIndPos(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		auto space = getMO_HLC().subspaceTree().getRegionIdx(sol.variable().vect());
		auto& box = getMO_HLC().subspaceTree().getBox(space);
		auto bound = CAST_CONOP(pro)->boundary();
	...
	void SPMOEA4_3::SolutionSelection(Problem* pro, Random* rnd) {
		//后续解的选择需要考虑离已选解的分布
	...
	void SPMOEA4_3::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA4_3::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA4_3::updateSubPopSpace(size_t pop_inx, Problem* pro, Random* rnd) {
	...
	void SPMOEA4_3::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem* pro) {
	...
	void SPMOEA4_3::generateOffspring(Problem* pro, Algorithm* alg, Random* rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
		...
		for (size_t i = 0; i < getPop().size(); ++i) {
			...
			if (type[i] == 0) {
				...
				if (m_explore_reset_flag) {
					...
					size_t space = selectSubspace(pro,rnd);
					auto box = getMO_HLC().subspaceTree().getBox(space);
					...
					auto sol = getPop()[i][second_pop[0]].variable().vect();
					auto space_inx = getMO_HLC().subspaceTree().getRegionIdx(sol);
					...
					for (size_t j = 0; j < temp_pop.size(); ++j) {
						...
						//越界处理
						SPMOEA::repairSol(temp_pop[j].trial().variable().vect(), box, rnd);
	...
	size_t SPMOEA4_3::selectSubspace(Problem* pro, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA4_3::sampleDiverse(Population<Solution<>>& pop, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA4_3::sampleRandom(Population<Solution<>>& pop, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_put;
		//auto bound = CAST_CONOP(pro)->boundary();
	...
	bool SPMOEA4_3::spaceSubdivision(Problem* pro, Random* rnd) {
	...
	void SPMOEA4_3::updateLinkSubspace(size_t inx, const std::vector<size_t>& front_spaces, Random* rnd) {
	...
	void SPMOEA4_3::updateNeighSpace() {

	}

	void SPMOEA4_3::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
		for (size_t i = 0; i < getMO_HLC().numSubspace(); ++i) {
	...
	std::vector<std::vector<size_t>> SPMOEA4_3::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA4_3::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA4_3::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	//统计子空间前沿解的交互成功率
	void SPMOEA4_3::adaptiveSplit(Problem* pro) {
		for (size_t i = 0; i < getPop().size(); ++i) {
	...
	//统计子空间前沿解的交互成功率
	void SPMOEA4_3::adaptiveSplitFrontSpace(Problem* pro, Random* rnd) {
	...
	void SPMOEA4_3::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem* pro, Random* rnd) {
	...
	void SPMOEA4_3::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA4_3::findSplitDim(int inx, Problem* pro) {
	...
	void SPMOEA4_3::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
		for (size_t i = 0; i < pop.size(); ++i) {
	...
	void SPMOEA4_3::recordMetrics(Problem* pro, Algorithm* alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA4_3::initiObjSpace(Problem* pro) {
