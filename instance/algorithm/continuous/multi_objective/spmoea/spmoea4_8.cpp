...
namespace ofec {
	...
	void SPMOEA4_8::initialize_() {
		...
		m_global_interactive_rate = v.get<Real>("global interactive rate");
		m_neigh_interactive_rate = v.get<Real>("neigh interactive rate");
		m_subspace_interactive_rate = 1- m_global_interactive_rate- m_neigh_interactive_rate;
	...
	void SPMOEA4_8::initiVarSpace(Problem* pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA4_8::initPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//在各个子空间生成一个子种群
		...
		size_t main_pop_num = pop_num - exploit_pop_num;
		...
		for (size_t i = main_pop_num; i < main_pop_num + exploit_pop_num; ++i) {
			inx2.push_back(i);
		}
		...
		for (size_t i = 0; i < exploit_pop_num; ++i) {
			exploit_pop.append(temp_pop[inx2[i]]);
		}
	...
	int SPMOEA4_8::evolve(Problem* pro, Algorithm* alg, Random* rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
		...
		*********************************************************************************************/
		...
		if (keep_inx.size() >= m_pop_index[1].size() || m_exploit_fail_times >= 5 || m_exploit_succ_times >= 5) {
			...
			if (m_explore_reset_flag) {
				...
				size_t sele_num2 = m_pop_index[1].size() - keep_inx.size();
				...
				for (size_t j = 0; j < sele_num2; ++j) {
					getPop()[0][m_pop_index[1][j]] = temp_pop2[candidates[sele_inx2[j]]];
				}
				for (size_t j = 0; j < keep_inx.size(); ++j) {
					getPop()[0][m_pop_index[1][sele_num2 + j]] = temp_pop2[keep_inx[j]];
				}
	...
	void SPMOEA4_8::assignPop(Population<Solution<>>& pop, Problem* pro, Algorithm* alg, Random* rnd) {
		//先得到非前沿子空间
	...
	void SPMOEA4_8::testCoverage(Problem* pro) {
		//检测子空间是否包含不同的PS片段
	...
	std::vector<std::vector<Real>> SPMOEA4_8::sampleInFrontNeighSpace(std::vector<size_t>& front_link_spaces, size_t ind_inx, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA4_8::sampleLinearInFrontSpace(size_t space, size_t ind_inx, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		//子连通空间内采样
		auto& sol = getPop()[0][ind_inx].variable().vect();
		for (size_t i = 0; i < sample_num; ++i) {
	...
	std::vector<std::vector<Real>> SPMOEA4_8::samplePush(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	std::vector<std::vector<Real>> SPMOEA4_8::sampleByIndPos(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		auto space = getMO_HLC().subspaceTree().getRegionIdx(sol.variable().vect());
		auto& box = getMO_HLC().subspaceTree().getBox(space);
		auto bound = CAST_CONOP(pro)->boundary();
	...
	void SPMOEA4_8::SolutionSelection(Problem* pro, Random* rnd) {
		//后续解的选择需要考虑离已选解的分布
	...
	void SPMOEA4_8::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA4_8::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA4_8::updateSubPopSpace(size_t pop_inx, Problem* pro, Random* rnd) {
	...
	void SPMOEA4_8::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem* pro) {
	...
	void SPMOEA4_8::generateOffspring(Problem* pro, Algorithm* alg, Random* rnd, const std::vector<size_t>& pop_resource) {
		...
		for (size_t i = 0; i < getPop().size(); ++i) {
			...
			auto exploit_pop = m_pop_index[1];
			...
			//主种群演化
			if (rand < m_subspace_interactive_rate) {
				//子空间支配关系交互
				for (size_t j = 0; j < main_pop.size(); ++j) {
					auto space = getMO_HLC().subspaceTree().getRegionIdx(getPop()[i][main_pop[j]].variable().vect());
					auto& his_sols = getMO_HLC().getSubspaceInfo(space).m_history_inds;
					sampleByPush(getPop()[i][main_pop[j]], his_sols, 1, 0.5, pro, alg, rnd);
				}
			}
			else if (rand >= m_subspace_interactive_rate && rand < m_subspace_interactive_rate + m_neigh_interactive_rate) {
				//个体之间邻域交互
				sampleAmongNeighs(pop2space, search_bound, main_pop.size(), pro, alg, rnd);
				/*Population<Solution<>> temp_pop;
				for (size_t j = 0; j < main_pop.size(); ++j) {
					temp_pop.append(getPop()[i][main_pop[j]]);
				}
				sampleAmongNeighs(temp_pop, search_bound, pro, alg, rnd);*/
			}
			else {
				//个体之间全局交互
				for (size_t j = 0; j < main_pop.size(); ++j) {
					...
					size_t sele_inx2 = (size_t)std::floor(main_pop.size() * rnd->uniform.next());
					auto& sol2 = getPop()[i][main_pop[sele_inx2]].variable().vect();
					temp_pop.append(getPop()[i][main_pop[sele_inx2]]);
					size_t sele_inx3 = 0;
					std::vector<Real> sol3;
					do {
						sele_inx3 = (size_t)std::floor(main_pop.size() * rnd->uniform.next());
						sol3 = getPop()[i][main_pop[sele_inx3]].variable().vect();
					} while (ifSame(sol2, sol3));
					temp_pop.append(getPop()[i][main_pop[sele_inx3]]);
			...
			if (m_explore_reset_flag) {
				//在选择的子空间重新初始化
				auto sol_info = selectSubspaceFromObj(pro, rnd);//根据主种群目标空间的分布选择一个子空间初始化
				//选择最大的一对点或者随机选择一对
				...
				size_t num_var = pro->numberVariables();
				auto sol1 = getPop()[i][main_pop[point_pair.first]].variable().vect();
				auto sol2 = getPop()[i][main_pop[point_pair.second]].variable().vect();
				/*auto& his_front_sols = getHisFrontSols();
				...
				*/
			    ...
			    //随机生成子代
				sampleRandom(getPop()[i][exploit_pop[0]],box,exploit_pop.size(),pro,alg,rnd);
				...
				for (size_t j = interactive_sols.size() - 1; j >= interactive_sols.size() - exploit_pop.size(); --j) {
					update_pop.append(*interactive_sols[j].back());
				}
				...
				if (rand > m_subspace_interactive_rate) {
					...
					for (size_t j = 0; j < exploit_pop.size(); ++j) {
						sampleByPush(getPop()[i][exploit_pop[j]], his_sols, 1, 0.5, pro, alg, rnd);
					}
				}
				else {
					//个体之间全局交互
					for (size_t j = 0; j < exploit_pop.size(); ++j) {
						...
						size_t sele_inx2 = (size_t)std::floor(exploit_pop.size() * rnd->uniform.next());
						auto& sol2 = getPop()[i][exploit_pop[sele_inx2]].variable().vect();
						temp_pop.append(getPop()[i][exploit_pop[sele_inx2]]);
						size_t sele_inx3 = 0;
						std::vector<Real> sol3;
						do {
							sele_inx3 = (size_t)std::floor(exploit_pop.size() * rnd->uniform.next());
							sol3 = getPop()[i][exploit_pop[sele_inx3]].variable().vect();
						} while (ifSame(sol2, sol3));
						temp_pop.append(getPop()[i][exploit_pop[sele_inx3]]);
	...
	//基于子空间个体采样，向个体所在子空间前沿解推进，提升个体
	std::vector<std::vector<Real>> SPMOEA4_8::sampleByPush(Solution<>& sol, const std::vector<std::shared_ptr<Solution<>>>& his_sols, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		//auto space = getMO_HLC().subspaceTree().getRegionIdx(sol.variable().vect());
		//auto& box = getMO_HLC().subspaceTree().getBox(space);
		//auto bound = CAST_CONOP(pro)->boundary();
	...
	std::vector<std::vector<Real>> SPMOEA4_8::sampleAmongGlobal(Population<Solution<>>& pop, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		PopDE<> temp_pop;
	...
	std::vector<std::vector<Real>> SPMOEA4_8::sampleAmongNeighs(std::map<size_t, std::vector<size_t>>& pop2space, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		for (auto& sub : pop2space) {
			...
			for (size_t j = 0; j < act_space.size(); ++j) {
				for (size_t k = 0; k < pop2space[act_space[j]].size(); ++k) {
					temp_pop.append(getPop()[0][pop2space[act_space[j]][k]]);
				}
			}
	...
	std::vector<std::vector<Real>> SPMOEA4_8::sampleAmongNeighs(Population<Solution<>>& pop, std::vector<std::pair<Real, Real>>& bound, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	std::vector<std::vector<Real>> SPMOEA4_8::sampleAmongSubspace(Population<Solution<>>& pop, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	size_t SPMOEA4_8::selectSubspace(Problem* pro, Random* rnd) {
	...
	std::map<Real, std::pair<size_t, size_t>> SPMOEA4_8::selectSubspaceFromObj(Problem* pro, Random* rnd) {
		//将目标空间所有非支配解分两类
	...
	std::vector<std::vector<Real>> SPMOEA4_8::sampleRandom(Solution<>& ind, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		for (size_t i = 0; i < sample_num; ++i) {
			std::vector<Real> sol;
	...
	bool SPMOEA4_8::spaceSubdivision(Problem* pro, Random* rnd) {
	...
	void SPMOEA4_8::updateLinkSubspace(size_t inx, const std::vector<size_t>& front_spaces, Random* rnd) {
	...
	void SPMOEA4_8::updateNeighSpace() {

	}

	void SPMOEA4_8::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
		for (size_t i = 0; i < getMO_HLC().numSubspace(); ++i) {
	...
	std::vector<std::vector<size_t>> SPMOEA4_8::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA4_8::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA4_8::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	//统计子空间前沿解的交互成功率
	void SPMOEA4_8::adaptiveSplit(Problem* pro) {
		for (size_t i = 0; i < getPop().size(); ++i) {
	...
	//统计子空间前沿解的交互成功率
	void SPMOEA4_8::adaptiveSplitFrontSpace(Problem* pro, Random* rnd) {
	...
	//统计子空间前沿解的交互成功率
	void SPMOEA4_8::adaptiveSplitSpace(Problem* pro, Random* rnd) {
	...
	void SPMOEA4_8::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem* pro, Random* rnd) {
	...
	void SPMOEA4_8::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA4_8::findSplitDim(int inx, Problem* pro) {
	...
	void SPMOEA4_8::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
		for (size_t i = 0; i < pop.size(); ++i) {
	...
	void SPMOEA4_8::recordMetrics(Problem* pro, Algorithm* alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA4_8::initiObjSpace(Problem* pro) {
