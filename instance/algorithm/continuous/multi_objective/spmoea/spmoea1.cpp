...
namespace ofec {
	...
	void SPMOEA1::initiVarSpace(Problem* pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA1::initPop(Problem* pro, Algorithm* alg, Random* rnd) {
		//在各个子空间生成一个子种群
	...
	int SPMOEA1::evolve(Problem* pro, Algorithm* alg, Random* rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
	...
	void SPMOEA1::ensembleSelect(size_t select_num, Problem* pro, Random* rnd) {
		auto& front_clusters = getFrontRegionLinkSpace();
	...
	void SPMOEA1::selectInHisRange(size_t select_num, Problem* pro, Random* rnd) {
	...
	void SPMOEA1::subspaceClusterSelection(size_t select_num, Problem* pro, Random* rnd) {
		auto front_spaces = getFrontSpace();
		auto bound = CAST_CONOP(pro)->boundary();
		for (size_t i = 0; i < getPop().size(); ++i) {
			for (size_t j = 0; j < getPop()[i].size(); ++j) {
	...
	void SPMOEA1::objspaceClusterSelection(size_t select_num, Problem* pro, Random* rnd, bool add_his) {
		auto front_spaces = getFrontSpace();
	...
	void SPMOEA1::clusterSelection(size_t select_num, Problem* pro, Random* rnd, size_t m_num_neigh) {
		auto front_spaces = getFrontSpace();
		auto bound = CAST_CONOP(pro)->boundary();
		for (size_t i = 0; i < getPop().size(); ++i) {
			for (size_t j = 0; j < getPop()[i].size(); ++j) {
	...
	void SPMOEA1::clusterInVarspace(Problem* pro) {
	...
	void SPMOEA1::doubleClusterSelection(size_t select_num, Problem* pro, Random* rnd) {
		//搜索空间聚类
	...
	void SPMOEA1::assignPop(Population<Solution<>>& pop, Problem* pro, Algorithm* alg, Random* rnd) {
		//先得到非前沿子空间
		std::vector<size_t> behind_spaces;
	...
	void SPMOEA1::testCoverage(Problem* pro) {
		//检测子空间是否包含不同的PS片段
	...
	std::vector<std::vector<Real>> SPMOEA1::sampleInFrontNeighSpace(std::vector<size_t>& front_link_spaces, size_t ind_inx, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
	...
	std::vector<std::vector<Real>> SPMOEA1::sampleLinearInFrontSpace(size_t space, size_t ind_inx, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		//子连通空间内采样
		auto& sol = getPop()[0][ind_inx].variable().vect();
		for (size_t i = 0; i < sample_num; ++i) {
	...
	std::vector<std::vector<Real>> SPMOEA1::samplePush(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	std::vector<std::vector<Real>> SPMOEA1::sampleByIndPos(Solution<>& sol, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		auto space = getMO_HLC().subspaceTree().getRegionIdx(sol.variable().vect());
		auto& box = getMO_HLC().subspaceTree().getBox(space);
		auto bound = CAST_CONOP(pro)->boundary();
	...
	void SPMOEA1::SolutionSelection(Problem* pro, Random* rnd) {
		//后续解的选择需要考虑离已选解的分布
	...
	void SPMOEA1::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA1::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA1::updateSubPopSpace(size_t pop_inx, Problem* pro, Random* rnd) {
	...
	void SPMOEA1::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem* pro) {
	...
	void SPMOEA1::generateOffspring(Problem* pro, Algorithm* alg, Random* rnd, const std::vector<size_t>& pop_resource) {
		for (size_t i = 0; i < getPop().size(); ++i) {
			size_t first_num = 0;
			for (size_t j = 0; j < getPop()[i].size(); ++j) {
	...
	void SPMOEA1::uniformSample(size_t pop_inx, Problem* pro, Algorithm* alg, Random* rnd) {
		//均匀分配资源
	...
	void SPMOEA1::biasSample(size_t pop_inx, Problem* pro, Algorithm* alg, Random* rnd, Real prob) {
	...
	//基于子空间个体采样，向个体所在子空间前沿解推进，提升个体
	std::vector<std::vector<Real>> SPMOEA1::sampleByPush(Solution<>& sol, const std::vector<std::shared_ptr<Solution<>>>& his_sols, size_t sample_num, Real push_prob, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		for (size_t i = 0; i < sample_num; ++i) {
	...
	std::vector<std::vector<Real>> SPMOEA1::sampleAmongGlobal(Population<Solution<>>& pop, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		PopDE<> temp_pop;
	...
	std::vector<std::vector<Real>> SPMOEA1::sampleAmongNeighs(size_t method, std::map<size_t, std::vector<size_t>>& pop2space, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		auto front_spaces = getFrontSpace();
	...
	std::vector<std::vector<Real>> SPMOEA1::sampleAmongNeighs(Population<Solution<>>& pop, size_t ind_inx, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	std::vector<std::vector<Real>> SPMOEA1::sampleAmongNeighs(Population<Solution<>>& pop, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		for (size_t j = 0; j < sample_num; ++j) {
	...
	std::vector<std::vector<Real>> SPMOEA1::sampleAmongSubspace(Population<Solution<>>& pop, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
	...
	size_t SPMOEA1::selectSubspace(Problem* pro, Random* rnd) {
	...
	std::map<Real, std::pair<size_t, size_t>> SPMOEA1::selectSubspaceFromObj(Problem* pro, Random* rnd, int source) {
		//将目标空间所有非支配解分两类
		size_t num_obj = pro->numberObjectives();
		std::vector<std::vector<Real>> all_data;
		if (source == 1) {
	...
	std::vector<size_t> SPMOEA1::selectSubspaceFromVar(Problem* pro, Random* rnd) {
		//综合子空间覆盖率和rank值排序，选出前排子空间
		/*std::vector<Real> volumn;
		for (size_t j = 0; j < getMO_HLC().numSubspace(); ++j) {
			Real v = getMO_HLC().subspaceTree().getBoxVolume(j);
	...
	std::vector<std::vector<Real>> SPMOEA1::sampleRandom(Solution<>& ind, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd) {
		std::vector<std::vector<Real>> out_off;
		for (size_t i = 0; i < sample_num; ++i) {
			std::vector<Real> sol;
	...
	bool SPMOEA1::spaceSubdivision(Problem* pro, Random* rnd) {
	...
	void SPMOEA1::updateLinkSubspace(size_t inx, const std::vector<size_t>& front_spaces, Random* rnd) {
	...
	void SPMOEA1::updateNeighSpace() {

	}

	void SPMOEA1::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
		for (size_t i = 0; i < getMO_HLC().numSubspace(); ++i) {
	...
	std::vector<std::vector<size_t>> SPMOEA1::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA1::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA1::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	//统计子空间前沿解的交互成功率
	void SPMOEA1::adaptiveSplit(Problem* pro) {
		for (size_t i = 0; i < getPop().size(); ++i) {
	...
	//统计子空间前沿解的交互成功率
	void SPMOEA1::adaptiveSplitFrontSpace(Problem* pro, Random* rnd) {
	...
	void SPMOEA1::periodSplitSpace(Problem* pro, Random* rnd) {

	}

	//统计子空间前沿解的交互成功率
	void SPMOEA1::adaptiveSplitSpace(Problem* pro, Random* rnd) {
		////test个体生成
	...
	void SPMOEA1::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem* pro, Random* rnd) {
	...
	void SPMOEA1::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA1::findSplitDim(int inx, Problem* pro) {
	...
	void SPMOEA1::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
		for (size_t i = 0; i < pop.size(); ++i) {
	...
	void SPMOEA1::recordMetrics(Problem* pro, Algorithm* alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA1::initiObjSpace(Problem* pro) {
