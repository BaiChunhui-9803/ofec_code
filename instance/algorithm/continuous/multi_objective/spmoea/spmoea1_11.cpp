...
//#endif
	...
	void SPMOEA1_11::initiVarSpace(Problem *pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA1_11::initPop(Problem *pro, Algorithm *alg, Random *rnd) {
		//在各个子空间生成一个子种群
		...
		//算子比例
		std::vector<Real> operator_ratio(3, 0.);
		operator_ratio[0] = 1.;
	...
	int SPMOEA1_11::evolve(Problem *pro, Algorithm *alg, Random *rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
	...
	void SPMOEA1_11::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA1_11::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA1_11::updateSubPopSpace(size_t pop_inx, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_11::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem *pro) {
	...
	void SPMOEA1_11::generateOffspring(Problem *pro, Algorithm *alg, Random *rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
			...
			if (type[i] == 1) {
				...
				for (size_t j = 0; j < getPop()[i].size(); ++j) {
					...
					for (size_t k = 0; k < front_sols.size(); ++k) {
						...
						if (ifSame(sol1, sol2)) {
							...
							if (std::find(front_spaces.begin(), front_spaces.end(), space_inx) != front_spaces.end()) {
								...
								auto off = sampleByFrontNeighSpace(space_inx, j, front_clusters[cluster_inx], bound, 1, pro, alg, rnd);
								...
								//邻域子空间形成子种群
								auto off = sampleByNeighSpaceFront(space_inx, j, bound, 1, pro, alg, rnd);
								//auto off = sampleByDE(getPop()[i], j, bound, 1, kk, pro, alg, rnd);
								for (size_t k = 0; k < off.size(); ++k) {
									all_off.emplace_back(off[k]);
								}
					...
					if (!flag) {
						...
						//个体所在子空间演化
						auto off = sampleInSubspace(getPop()[i][j], 1, pro, alg, rnd);
				...
				operator_ratio[2] /= getPop()[i].size();
			}
			else if (type[i]==3) {
				//连通子空间采样
	...
	bool SPMOEA1_11::spaceSubdivision(Problem *pro, Random *rnd) {
	...
	void SPMOEA1_11::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
	...
	std::vector<std::vector<size_t>> SPMOEA1_11::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA1_11::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA1_11::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	void SPMOEA1_11::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_11::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA1_11::findSplitDim(int inx, Problem *pro) {
	...
	void SPMOEA1_11::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
	...
	void SPMOEA1_11::recordMetrics(Problem *pro, Algorithm *alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA1_11::initiObjSpace(Problem *pro) {
