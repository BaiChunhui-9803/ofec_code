...
*************************************************************************/
// Created: 28 June 2023 by Qingshan Tan (email:qingshan.t@cug.edu.cn)
// partition subspace + simple MOEA for nonlinear PS
// 初次划分子空间，并在每个子空间分配子种群，允许子种群越过初始的子空间

#ifndef OFEC_SPMOEA1_0_H
#define OFEC_SPMOEA1_0_H

#include "spmoea.h"

namespace ofec {

	class SPMOEA1_0 :public SPMOEA {
	public:
		...
		//void updatePop(Problem *pro, Algorithm *alg, Random *rnd);
		void calFrontBoundRatio(std::vector<Real>& front_bound_ratio);
		void PopResourceAssign(std::vector<Real>& pop_exploit_ratio, std::vector<size_t>& assign_pop_resource, std::vector<Real>& front_bound_ratio, Problem *pro);
		void generateOffspring(Problem *pro, Random *rnd, const std::vector<size_t>& pop_resource, const std::vector<Real>& pop_exploit_ratio, int type);
		//void repairSol(std::vector<Real>& sol, Problem *pro, Random *rnd);
		void spaceSubdivision(Problem *pro, Random *rnd);
		...
		void updateVarSpace(Problem *pro, Random *rnd);
