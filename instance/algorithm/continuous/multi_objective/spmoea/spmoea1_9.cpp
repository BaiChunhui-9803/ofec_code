...
namespace ofec {
...
#ifdef OFEC_DEMO
	void SPMOEA1_9::updateBuffer() {
		if (ofec_demo::g_buffer->algorithm().get() == this) {
			m_solution.clear();
			m_solution.resize(getPop().size() + 1);//第二个为子代
			for (size_t i = 0; i < getPop().size(); ++i) {
				...
				/*for (size_t j = 0; j < getPop()[i].getOffspring().size(); ++j) {
					  m_solution[getPop().size()].push_back(&getPop()[i].getOffspring()[j]);
				}*/
			}
	...
	void SPMOEA1_9::initiVarSpace(Problem *pro) {
		/*************************************/
		/*      MO_HLC搜索空间划分初始化     */
		/*************************************/
	...
	void SPMOEA1_9::initPop(Problem *pro, Algorithm *alg, Random *rnd) {
		//在各个子空间生成一个子种群
		...
		std::vector<Real> ratios(3, 0.);
		getOperatorRatio().emplace_back(ratios);
		//record();
	}

	int SPMOEA1_9::evolve(Problem *pro, Algorithm *alg, Random *rnd) {
		/********************************************************************************
							   根据子种群前沿子空间的体积分配搜索资源
		********************************************************************************/
		...
		********************************************************************************/
		...
		if (!m_evolve_by_predict) {
			...
			for (size_t i = 0; i < getPop().size(); ++i) {
				...
				//	for (size_t j = 0; j < parent_spaces.size(); ++j) {
				//		if (std::find(front_spaces.begin(), front_spaces.end(), parent_spaces[j]) == front_spaces.end()) {
				//			break;
				//		}
				//		if (j == parent_spaces.size() - 1) {
				//			all_in_front = true;
				//		}
				//	}
				//	if (all_in_front) {
				//		interactive_type.push_back(3);//连通子空间采样
				//	}
				//	else {
				//		interactive_type.push_back(3);
				//	}
	...
	void SPMOEA1_9::calFrontBoundRatio(std::vector<Real>& front_bound_ratio) {
	...
	bool SPMOEA1_9::indInRange(std::vector<std::pair<Real, Real>>& bound, std::vector<Real>& sol) {
		bool flag = true;
	...
	void SPMOEA1_9::updateSubPopSpace(size_t pop_inx, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_9::PopResourceAssign(std::vector<size_t>& assign_pop_resource, size_t switch_period, Problem *pro) {
		...
		/*if (m_divide_iteration % switch_period == 0) {
			...
			if (front_link_spaces.size() == 1) {
				...
				for (size_t i = 0; i < front_link_spaces.size(); ++i) {
					total_size += (M * front_link_spaces[i].size());
				}
				assign_pop_resource.push_back(total_size);
			}
		}*/
		if (m_stage_last_time > 0) {
			...
			total_size = M * front_link_spaces.size();
			assign_pop_resource.push_back(total_size);
		}
		else {
			assign_pop_resource.push_back(max_pop_size);
	...
	void SPMOEA1_9::generateOffspring(Problem *pro, Algorithm *alg, Random *rnd, const std::vector<size_t>& pop_resource, std::vector<int> type) {
		...
		for (size_t i = 0; i < getPop().size(); ++i) {
			...
			if (type[i] == 1) {
				...
				/*std::vector<size_t> assign_pop(3);
				assign_pop[0] = (size_t)std::floor(pop_resource[i] / 3.);
				assign_pop[1] = (size_t)std::floor(pop_resource[i] / 3.);
				assign_pop[2] = pop_resource[i] - assign_pop[0] - assign_pop[1];*/
				auto front_spaces = getFrontSpace();
				//亲代所在子空间采样
				std::vector<size_t> parent_spaces;
				for (size_t j = 0; j < getPop()[i].size(); ++j) {
				...
				for (auto& jj : not_front_inds) {
					for (size_t k = 0; k < jj.second.size(); ++k) {
						//auto sol = getPop()[i][jj.second[k]].variable().vect();
						auto off = sampleBySolution(getPop()[i][jj.second[k]], 1, pro, alg, rnd);
						assign1++;
						for (auto& kk : off) {
							all_off.emplace_back(kk);
						}
					}
				}
				...
				//前沿子空间采样
				size_t assign2 = (size_t)std::ceil((getPop()[i].size() - (Real)assign1) / 2.);
				...
				for (size_t j = 0; j < assign2; ++j) {
					...
					auto off1 = sampleInFrontNeighSpace(front_clusters[cluster_inx], bound, 1, pro, alg, rnd);*/
					for (size_t k = 0; k < off1.size(); ++k) {
						all_off.emplace_back(off1[k]);
					}
				}
				//亲代交互
				size_t assign3 = getPop()[i].size() - assign1 - assign2;
				size_t kk = 2;
				...
				std::vector<Real> operator_ratio;
				operator_ratio.push_back((Real)assign1 / getPop()[i].size());
				operator_ratio.push_back((Real)assign2 / getPop()[i].size());
				operator_ratio.push_back((Real)assign3 / getPop()[i].size());
				getOperatorRatio().emplace_back(operator_ratio);
			}
			else if (type[i] == 3) {//连通子空间采样
				for (size_t j = 0; j < front_clusters.size(); ++j) {
					auto off = sampleInFrontNeighSpace(front_clusters[j], bound, 2, pro, alg, rnd);
					for (size_t k = 0; k < off.size(); ++k) {
						all_off.emplace_back(off[k]);
					}
				}
				std::vector<Real> operator_ratio;
				operator_ratio.push_back(0.);
				operator_ratio.push_back(1.);
				operator_ratio.push_back(0.);
				getOperatorRatio().emplace_back(operator_ratio);
			}
			else if (type[i] == 4) {
				//基于每个个体采样
				for (size_t j = 0; j < getPop()[i].size(); ++j) {
					auto off = sampleBySolution(getPop()[i][j], 1, pro, alg, rnd);
					for (size_t k = 0; k < off.size(); ++k) {
						all_off.emplace_back(off[k]);
					}
				}
	...
	bool SPMOEA1_9::spaceSubdivision(Problem *pro, Random *rnd) {
	...
	void SPMOEA1_9::clusterSubspace() {
		//每次聚类只加入一层，允许类间有重叠
	...
	std::vector<std::vector<size_t>> SPMOEA1_9::clusterFrontSpace(const std::vector<size_t>& frontspace) {
	...
	bool SPMOEA1_9::subspaceLink(size_t inx1, size_t inx2) {
		bool flag = false;
		//两个子空间中的前沿解的分布
	...
	void SPMOEA1_9::findClusterCenterSsp() {
		getMO_HLC().findClusterCenterSsp();
	}

	void SPMOEA1_9::splitSpace(size_t inx, size_t num, int dim, Real pos, bool flag, Problem *pro, Random *rnd) {
	...
	void SPMOEA1_9::splitSubspace(size_t inx, size_t num, int dim, Real pos, bool flag) {
		if (flag) {
	...
	int SPMOEA1_9::findSplitDim(int inx, Problem *pro) {
	...
	void SPMOEA1_9::NDSort(std::vector<std::shared_ptr<Solution<>>>& pop) {
		std::vector<std::vector<Real>*> objs;
	...
	void SPMOEA1_9::recordMetrics(Problem *pro, Algorithm *alg) {
		/************************************/
		/*            性能指标计算          */
		/************************************/
		//使用archive计算性能指标
	...
	void SPMOEA1_9::initiObjSpace(Problem *pro) {
