...
// 在spmoea4_5和spmoea4_2的基础上，设置两个参数控制测试和精搜的比例，
// 在环境选择阶段，精搜部分单独演化，其余个体统一排序选择
// 设置一部分自由个体进行内部演化，为主种群找出新的前沿区域
// 并且当自由种群到前排或者演化一定代数后，分析目标空间点分布，找出稀疏点位置附近
// 重新部署自由种群的个体位置进行搜索

// 优化自适应分割子空间的方式

#ifndef OFEC_SPMOEA4_8_H
#define OFEC_SPMOEA4_8_H
...
namespace ofec {
	...
	public:
		...
		std::vector<std::vector<Real>> sampleAmongNeighs(std::map<size_t, std::vector<size_t>> &pop2space, std::vector<std::pair<Real, Real>>& bound, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd);
		std::vector<std::vector<Real>> sampleAmongNeighs(Population<Solution<>>& pop, std::vector<std::pair<Real, Real>>& bound, Problem* pro, Algorithm* alg, Random* rnd);
	...
	private:
		...
		Real m_test_rate;
		Real m_exploit_rate;
		Real m_global_interactive_rate;
