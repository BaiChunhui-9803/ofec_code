...
// 在spmoea4_1的基础上，每一代分出部分个体进行测试，通过IGD的变化，重新分布种群

#ifndef OFEC_SPMOEA4_2_H
#define OFEC_SPMOEA4_2_H
...
namespace ofec {
	...
	public:
		...
		std::vector<std::pair<size_t, size_t>> selectSubspaceFromObj(Problem* pro, Random* rnd);
		...
		std::vector<std::vector<Real>> sampleRandom(Population<Solution<>>& pop, size_t sample_num, Problem* pro, Algorithm* alg, Random* rnd);
