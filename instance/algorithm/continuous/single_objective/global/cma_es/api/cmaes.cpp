...
double *
cmaes_init_final(cmaes_t *t, ofec::Random *rnd)
/*
 * */
{
	int i, j, N;
	...
	if (t->sp.typicalXcase) {
		...
		for (i = 0; i < N; ++i)
			t->rgxmean[i] += t->sigma * t->rgD[i] * rnd->normal.next();//cmaes_random_Gauss(&t->rand);
	}
...
/* --------------------------------------------------------- */
double *const *
cmaes_SamplePopulation(cmaes_t *t, ofec::Random *rnd)
{
	...
	{ /* generate scaled cmaes_random vector (D * z)    */
		for (i = 0; i < N; ++i) {
			using namespace ofec;
			if (flgdiag)
				t->rgrgx[iNk][i] = xmean[i] + t->sigma * t->rgD[i] * rnd->normal.next();// cmaes_random_Gauss(&t->rand);
			else
				t->rgdTmp[i] = t->rgD[i] * rnd->normal.next();// cmaes_random_Gauss(&t->rand);
		}
		if (!flgdiag) {
			/* addSubspace mutation (sigma * B * (D*z)) */
			for (i = 0; i < N; ++i) {
				for (j = 0, sum = 0.; j < N; ++j)
...
/* --------------------------------------------------------- */
double const *
cmaes_ReSampleSingle_old(cmaes_t *t, double *rgx, ofec::Random *rnd)
{
	int i, j, N = t->sp.N;
	double sum;
...
/* --------------------------------------------------------- */
double *const *
cmaes_ReSampleSingle(cmaes_t *t, int iindex, ofec::Random *rnd)
{
	int i, j, N = t->sp.N;
	double *rgx;
	double sum;
...
/* --------------------------------------------------------- */
double *
cmaes_SampleSingleInto(cmaes_t *t, double *rgx, ofec::Random *rnd)
{
	int i, j, N = t->sp.N;
	double sum;
...
/* --------------------------------------------------------- */
double *
cmaes_PerturbSolutionInto(cmaes_t *t, double *rgx, double const *xmean, double eps, ofec::Random *rnd)
{
	int i, j, N = t->sp.N;
	double sum;
...
/* --------------------------------------------------------- */
const double *
cmaes_Optimize(cmaes_t *evo, double(*pFun)(double const *, int dim), long iterations, ofec::Random *rnd)
/* TODO: make signals.par another argument or, even better, part of cmaes_t */
{
		...
		(evo->gen < startiter + iterations || !iterations))
	{
		...
		pop = cmaes_SamplePopulation(evo, rnd); /* do not change content of pop */
...
#if 0
	/* removeSubspace momentum in ps, if ps is large and fitness is getting worse */
...
cmaes_random_init(cmaes_random_t *t, long unsigned inseed)
{
	...
	if (inseed < 1) {
		while ((long)(cloc - clock()) == 0)
			; /* TODO: removeSubspace this for time critical applications? */
