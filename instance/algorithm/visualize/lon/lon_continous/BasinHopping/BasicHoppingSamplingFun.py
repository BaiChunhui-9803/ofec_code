...
def BasicHoppingSamplingFun(*argv):
    # HELP DISPLAY
    if len(argv) == 2:
        if argv[1] == '--help':
            ...
            print(
                '    --prec  : <integer> number of decimal used in the candidate solution and fitness value, default: all')
    ...
    file = open(ofile, "w")
    file.write(
        "Run\tValue_Current_Best\tCurrent_Best " + str(nvar) + "\tValue_Solution_At_Iteration\tSolution_At_Iteration\n")
    my_method = 'L-BFGS-B'
...
if __name__ == "__main__":
    BasicHoppingSamplingFun(sys.argv)
