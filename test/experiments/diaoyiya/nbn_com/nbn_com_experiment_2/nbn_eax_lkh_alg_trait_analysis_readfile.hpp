...
void runEAX_LKHbyFile(const std::string& readDir,
	const std::string& saveDir,
	const std::string& tspfilename,
...

	{
		...
		auto& nbnInfo = eaxNBNinfo.nbnInfo;
		std::string taskName = tspname + "_eax";
		nbnInfo.inputNBNinfo(saveDir, taskName);

		analyseNBN(eaxNBNinfo, 10);
	}

	{
		auto& nbnInfo = lkhNBNinfo.nbnInfo;
		std::string taskName = tspname + "_lkh";
		nbnInfo.inputNBNinfo(saveDir, taskName);

		analyseNBN(lkhNBNinfo, 0);
...
void runTask() {
	...
	for (auto& it : filenames) {
		...
		if (it.m_tspname == "u574.tsp") {
			runEAX_LKHbyFile(dir2, saveDir, it.m_tspname, lkhNBNinfo, eaxNBNinfo);
		}
		else {
			runEAX_LKHbyFile(dir1, saveDir, it.m_tspname, lkhNBNinfo, eaxNBNinfo);
		}
