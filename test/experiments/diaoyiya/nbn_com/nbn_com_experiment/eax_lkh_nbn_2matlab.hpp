...
void calculateNBNnetwork(const std::string& filepath, NBNinfo &nbnInfo) {
...
void runTask() {
	...
	saveDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/eax_lkh_total_sortedX/";

	auto saveDir2 = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/eax_lkh_nbnData2/";
...
namespace ofec {
	...
	void run() {
		...
		ofec::Random rnd(0.5);
		std::vector<int> vidx(100);
		for (auto& it : vidx) {
			it = rnd.uniform.nextNonStd<int>(0, 1e9);
		}
		std::vector<double> vdouble(30);
		for (auto& it : vdouble) {
			it = rnd.uniform.next();
		...
		std::ofstream out("testMatlabInput.txt");
		out << 1 << "\t" << vidx.size() << std::endl;
		for (auto& it : vidx) out << it << "\t";
		out << std::endl;
