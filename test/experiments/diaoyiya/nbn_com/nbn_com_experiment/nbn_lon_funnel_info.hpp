...
void calTask(const std::string& proReadDir,
...

	{
		...
		{
			std::vector<int> nodeFunnelId(lon.m_node_fit.size(), -2);
...
void transferData(const std::string& proReadDir,
	const std::string& lonRealDir,
	const std::string& saveDir,
	const std::string& tspfilename) {
	using namespace ofec;
	...
	auto tspname = tspfilename.substr(0, tspfilename.find_first_of("."));
	{
		...
		ofec::ParameterVariantStream paramsStream;
		inputFromFile(paramsStream, saveDir + tspname + "_lonFunnelIds.txt");
		paramsStream >> nodeFunnelId;


		{
			std::ofstream out2(saveDir + tspname + "_lonFunnelIdsMatlab.txt");
			out2 << 1 << "\t" << nodeFunnelId.size() << std::endl;
			for (auto& it : nodeFunnelId) {
				out2 << it << std::endl;
			}
			out2.close();
		}
...
//	ofec::g_working_directory = "/mnt/Data/Student/2018/YiyaDiao/code_total/data";
	...
	auto saveDir2 = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/nbn_data_lonSample2/";
	...
	for (auto& it : filenames) {
		if (it == "u574.tsp") {
			transferData(dir2, readDir, saveDir2, it);
		}
		else {
			transferData(dir1, readDir, saveDir2, it);
		}
	}
