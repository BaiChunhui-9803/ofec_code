...
void readEAX_LKH(const std::string& readDir,
	...
	//double maxFit(0);
	//ofec::calMax(nbnInfo.vFitness, maxFit);
	//for (auto& it : nbnInfo.vFitness) it = maxFit;
	ofec::dataNormalize(nbnInfo.vFitness);


	std::vector<std::array<double, 2>> maxDis;

	for (int idx(0); idx < nbnInfo.vFitness.size(); ++idx) {
		if (nbnInfo.vFitness[idx] >= 0.99) {
			//	std::array<double, 2> cur = {};
			maxDis.push_back({ nbnInfo.vFitness[idx],nbnInfo.dis2parent[idx] });
		}
	}

	std::sort(maxDis.begin(), maxDis.end(), [](std::array<double, 2> a, std::array<double, 2> b) {
		return a.back() > b.back();
		});


	{
		std::string savepath = saveDri2 + tspname + "_fitnessDis.txt";
	...
	std::vector<std::vector<int>> m_traits(4);
	std::vector<std::string> task_names = { "lkh", "eax" };
	std::vector<int> successRate;

	//{
	//	ofec::ParameterVariantStream paramsStream;
	//	inputFromFile(paramsStream, saveDir + tspname + "_trait_sucessRate.txt");
	//	size_t inputSize(0);
...
void runTask() {
	...
	auto saveDir2 = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/eax_lkh_nbnData_fitness_distance2/";
	...
	for (auto& it : filenames) {
		if (it == "u574.tsp") {
			readEAX_LKH(dir2, saveDir, saveDir2, it);
		}
		else {
			readEAX_LKH(dir1, saveDir, saveDir2, it);
		}
	}
