...
//	cmd = LDEE_path + "TSNEEmbedding.exe " + "E:/Diao_Yiya/paper/fitness_landscape/IDEE/idee_data_nearest_neighbor/" + filename + subfix[1];
	...
	cmd = LDEE_path + "VacuumEmbedding.exe --skip-rows=" + std::to_string(skipData) + " " + path + "/" + filename + subfix[2];
	std::system(cmd.c_str());
...
void runIDEE(const std::string& filename) {
	...
	data_dir = "F:/code/ofec_data/idee_data_lon_eax_lkh";
...
void runTask() {
	...
	filenames =
	{ "2202.tsp", "5955.tsp","u574.tsp" ,   "1281.tsp" };
