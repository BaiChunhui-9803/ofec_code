function drawNBNfunnelsAndBridge( readdir,readdir2, savedir, filename)
...
formatSpec="%f";
bridgeIdsName= readdir2 + filename + "_optSolIds.txt";
sizeA= [1 2]; 
...
bridgeIdsName= readdir2 + filename + "_furtherSolIds.txt";
sizeA= [1 2]; 
fileID = fopen(bridgeIdsName,'r');
sizeM= fscanf(fileID,formatSpec,sizeA);
...
bridgeIds=bridgeIds+1;
sizeM= sizeM(1,2);
furtherPos= zeros(sizeM,5);
for idx=1:sizeM
    furtherPos(idx,:)=  network_mat(bridgeIds(idx),:);
end




bridgeIdsName= readdir2 + filename + "_bridgeSolIds.txt";
sizeA= [1 2]; 
fileID = fopen(bridgeIdsName,'r');
sizeM= fscanf(fileID,formatSpec,sizeA);
...
bridgeIds=bridgeIds+1;
sizeM= sizeM(1,2);
pathPos= zeros(sizeM,5);
for idx=1:sizeM
    pathPos(idx,:)=  network_mat(bridgeIds(idx),:);
end
...
best_points.MarkerEdgeColor= 'g';
best_points.Marker= 'square';
best_points.SizeData= 50;


hold on;
further_points=  scatter3(furtherPos(:,2),furtherPos(:,3),furtherPos(:,4),'filled');
further_points.MarkerFaceColor= 'none';
further_points.MarkerEdgeColor= 'b';
further_points.Marker= 'square';
further_points.SizeData= 50;




hold on;
path_points=  scatter3(pathPos(:,2),pathPos(:,3),pathPos(:,4),'filled');
path_points.MarkerFaceColor= 'none';
path_points.MarkerEdgeColor= 'k';
path_points.Marker= 'square';
path_points.SizeData= 50;
