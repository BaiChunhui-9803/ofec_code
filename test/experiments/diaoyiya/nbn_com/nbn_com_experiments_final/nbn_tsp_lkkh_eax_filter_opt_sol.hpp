...
void analyseEAX_LKH_NBN(const std::string& readDir,
...

	{
...

		{
			...
			auto bestSol = optNBN.m_solbases[bestSolId];
			std::vector<double> dis2opt(optNBN.m_dis2parent.size());
			std::vector<int> filterSolIds;

			for (int idx(0); idx < dis2opt.size(); ++idx) {
				dis2opt[idx] = nbnInfo.m_solbases[idx]->variableDistance(*bestSol, env.get());

			}
			std::ofstream out(saveDri2 + tspname + "_filterOptSols.txt");
...
void runTask_visualization() {
	...
	saveDir2 = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/eax_lkh_nbn_rnd_lkhmore5_filterSol/";
