...
void runLKH(const std::string& readDir,
	...
	while (true) {
		...
		LKH::LKHAlg lkh_alg;
		//	std::cout << "curSeed\t" << Seed << std::endl;
		lkh_alg.SRandom(Seed);
		std::vector<double> costs;
		lkh_alg.run(readDir, tspfilename, totalSols, curInfo.rnds[curRunId].get());
...
void runEAX_LKH(const std::string& readDir,
	...
	for (int idx(0); idx < lkh_solx.size(); ++idx) {
		...
		for (int idy(0); idy < lkh_solx[idx].size(); ++idy) {
			lkh_ids[idx][idy].resize(lkh_solx[idx][idy].size());
			for (int idz(0); idz < lkh_solx[idx][idy].size(); ++idz) {
				...
				tspSol.variable().vect() = lkh_solx[idx][idy][idz];
				lkh_sols.push_back(cursol);
				lkh_ids[idx][idy][idz] = totalSols.size();
...

	{
...

		{
			...
			int runId(0);
			for (auto& it : lkh_ids) {
				outputTrait(saveDir, tspname + "_lkh_trait_" + std::to_string(runId), it);
			...
			for (auto& it : eax_ids) {
				outputTrait(saveDir, tspname + "_eax_trait_" + std::to_string(runId), it);
				runId++;
			}
...
void filterMatlabfile(const std::string& filedir) {


	std::filesystem::path directory_path = filedir;  // 替换为你的目录路径
	namespace fs = std::filesystem;
	std::string target_extension = ".txt_matlab.txt";
	std::string new_extension = "_matlab.txt";
		...
		if (fs::exists(directory_path) && fs::is_directory(directory_path)) {
			...
			for (const auto& entry : fs::directory_iterator(directory_path)) {
				...
				if (entry.is_regular_file()) {
					auto subtr = entry.path().string();
					auto subfix = subtr.substr(subtr.size() - target_extension.size(), target_extension.size());
					if (subfix == target_extension) {
						std::cout << "Found and renaming file: " << entry.path() << std::endl;
						std::cout << subfix << std::endl;
						auto newname = entry.path().string().substr(0, subtr.size() - target_extension.size());
						newname = newname + new_extension;
					...
					//新文件名
					//fs::path new_path = entry.path().replace_extension(new_extension);
...
namespace ofec {
	...
	void run(int argc, char* argv[]) {
		...
		runTask();
		//filterMatlabfile("//172.24.24.151/e/DiaoYiya/code/data/ofec-data/paper_com_experiment_data/totalTsp/eax_lkh_more_hnsw_rnd_2/");
