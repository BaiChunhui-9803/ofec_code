...
void getNBN_bridgeData(const std::string& filedir, const std::string& saveDir, const std::string& filename) {
	...
	ofec::nbn::tsp::getNarrowGap(info.nbnInfo, bridgeIds, 0.9, 0.00);
...
void getNBNinfo_bridgeData(const std::string& filedir, const std::string& saveDir, const std::string& filename, NBNanalyseInfo& info, std::vector<int>& bridgeIds) {
...

	{
		...
		subNBNinfo.subset(info.nbnInfo, bridgeIds);
		{
...
void getNBN_bridgeData() {
	...
	nbnInfoDir = "//172.24.24.151/f/DiaoYiya/nbn_com_exp_final/nbn_data_eax_sampling_1000000_final_nbnNetworks_remote/";
	nbnInfoDir = "//172.24.24.151/f/DiaoYiya/nbn_com_exp_final/nbn_subregion_centers/merge_network/";
	...
	savedir = "//172.24.242.8/share/Student/2018/YiyaDiao/code_total/data/paper_com_experiment_data/totalTsp/nbn_sub_region_solVec_hnswModelSingleThread_equalBetter_bridgeData/";
	savedir = "//172.24.24.151/f/DiaoYiya/nbn_com_exp_final/nbn_data_eax_sampling_1000000_final_nbnNetworks_remote_bridgeInfo/";
	savedir = "//172.24.24.151/f/DiaoYiya/nbn_com_exp_final/nbn_subregion_centers/merge_network_bridgeInfo3/";
	...
	for (auto& curname : filenames) {
		...
		out << "randomSamples" << "\t" << gapInfo.m_maxlength << "\t" << gapInfo.m_numNodes << "\t" << gapInfo.m_avgBasinSize << "\t";
		...
		for (auto& k : subRegions) {
			...
			ofec::nbn::tsp::calculateNarrowGapInfo(info.nbnInfo, bridgeData1, gapInfo);
			//	getNBNinfo_bridgeData(nbnInfoDir, curname + randomSamples + neighborK + "_" + std::to_string(k), bridgeData1);
			out << k << "\t" << gapInfo.m_maxlength << "\t" << gapInfo.m_numNodes << "\t" << gapInfo.m_avgBasinSize << "\t" << gapInfo.m_minDirectionSons << "\t" << gapInfo.maxNBD << "\t";
			//getNBN_bridgeData(filedir, savedir, curname + randomSamples + neighborK + "_" + std::to_string(k));
		}
		out << std::endl;
	}
