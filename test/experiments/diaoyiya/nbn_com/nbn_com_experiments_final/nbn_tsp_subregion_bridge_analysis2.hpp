...
void bridgeInfo(const std::string& saveDir, const std::string& filename, ofec::nbn::NBNinfo& nbnInfo) {
	std::vector<int> bridgeIds;
	ofec::nbn::tsp::getNarrowGap(nbnInfo, bridgeIds, 0.95, 0.0);

	{
		ofec::nbn::NBNinfo subNBNinfo;
		subNBNinfo.subset(nbnInfo, bridgeIds);
		{

			std::vector<ofec::TreeGraphSimple::NBNdata> nbn_data;
			subNBNinfo.transferToNBNdata(nbn_data);
			ouputNBNdata(saveDir + filename + "_bridgeIds_nbn.txt", nbn_data);
		}
...
void calTask(const std::string& readDir,
	...
	for (int id(0); id < centerSols.size(); ++id) {
		...
		calculateNBNnetwork(nbnInfo, networkFilenames[id], filename, env.get(), rnd.get());

		bridgeInfo(networkFilenames[id], filename, nbnInfo);
	...
	NBNinfo mergeNBNinfo;
	{
		...
		calculateNBNnetwork(mergeNBNinfo, networkFilenames.back(), filename, env.get(), rnd.get());

		bridgeInfo(networkFilenames.back(), filename, mergeNBNinfo);
...
void runTask() {
	...
	saveDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp3/nbn_subregion_centers2/";
...
int getHeadNumber(const std::string& line) {
	std::stringstream str;
	str << line;
	std::string name;
	str >> name >> name >> name >> name >> name;
	auto name_strs = UTILITY::split(name, "_");
	return std::stoi(name_strs[1]);
}

int getLineTime(const std::string& line) {
	std::stringstream str;
	str << line;
	std::string name;
	...
	//str >> name >> name >> name >> name >> name >> name >> name;
	return std::stoi(names.back());
}


void readAlgTime(const std::string& filepath, const std::string& saveDir, const std::string& filename) {

	std::string line;
	std::ifstream in(filepath);

	std::vector<int> heads;
	std::vector<int> nbnIterTimes;
	std::vector<int> nbnDivTimes;
	getline(in, line);
	...
	while (true) {
		if (line.size() > 3) {
			heads.push_back(getHeadNumber(line));
		}
		else break;

		if (heads.back() == 575000) {
			int stop = -1;
		}

		while (getline(in, line)) {
			if (std::isdigit(line.front())) break;
		}

		while (getline(in, line)) {
			if (!std::isdigit(line.front())) break;
			nbnTime = getLineTime(line);
		}

		nbnIterTimes.push_back(nbnTime);
	...
	infos.push_back(heads);
	infos.push_back(nbnIterTimes);
	infos.push_back(nbnDivTimes);

	{
		std::ofstream out(saveDir + filename + "_algInfo.txt");
		for (auto& it : infos) {
			for (auto& it2 : it) {
				out << it2 << "\t";
			}
	...
	{
		std::ofstream out(saveDir + filename + "_algTimeMatlab.txt");
		ofec::matlab::outputMatrix(out, infos);
		out.close();
...
namespace ofec {
	...
	void run(int argc, char* argv[]) {
		...
		registerInstance();
		std::string saveDir = "//172.24.24.151/f/DiaoYiya/nbn_com_exp_final/alg_time/";
		readAlgTime("//172.24.24.151/f/DiaoYiya/nbn_com_exp_final/alg_time/nb.txt", saveDir, "2202_tsp");
