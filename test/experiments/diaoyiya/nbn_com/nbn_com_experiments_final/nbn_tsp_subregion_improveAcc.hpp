...
void calTask(const std::string& readDir,
	...
	std::vector<int> neighborKs = { 50, 30,10 };
...
void calTaskImprove(const std::string& readDir,
	const std::string& traitDir,
	const std::string& saveDir,
	const std::string& tspfilename) {
	...
	for (auto& neighborK : neighborKs) {
		...
		ofec::nbn::NBNinfo optNBNinfo;
		{
			...
			udpateNBNinfoIteration(optNBNinfo,
				1e5, env.get(), rnd.get());

			optNBNinfo.outputVecSolutions<int>(saveDir, filename, env.get());
			optNBNinfo.outputNBNinfo(saveDir, filename);
...
void runTask() {
	...
	auto readdir3 = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp3/nbn_subregion_aournd_center_hnswRnd/optima_data/";
	...
	saveDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp3/nbn_subregion_aournd_center_hnswRnd_improveAccur/";
...
//	std::reverse(filenames.begin(), filenames.end());
	...
	for (auto& it : filenames) {
		if (it == "u574.tsp") {
			calTaskImprove(dir2, readdir3, saveDir, it);
			//calTaskOptBelong(dir2,)
		}
		else {
			calTaskImprove(dir1, readdir3, saveDir, it);
