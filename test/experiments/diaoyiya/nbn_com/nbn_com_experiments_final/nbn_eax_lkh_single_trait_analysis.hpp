	...
	ofec::Environment* env) {
		{
			...
			/*for (int idx(0); idx < nearestSolId.size(); ++idx)*/ {
				...
				{
					nearestNBNinfo.subset(nbnInfo, nearestSolId[idx]);
				}
			...
			std::vector<std::vector<int>> curTrait;
			{
...

				{
					...
					calculateNearestBest(
						...
						curTrait,
						env.get());
					ofec::outputNBNdata(out, nearestNBNinfo.m_solIds, nearestNBNinfo.m_belong, nearestNBNinfo.m_dis2parent, nearestNBNinfo.m_vFitness);
				}
			}
...
void runTask_visualization() {
	...
	saveDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/eax_lkh_more_hnsw_rnd_2/";
