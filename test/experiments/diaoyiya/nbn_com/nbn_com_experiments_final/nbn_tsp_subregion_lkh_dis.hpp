...
void runLKHmultiThread(
	const std::string& readDir,
	const std::string& tspfilename,
	LKH_RunInfo& totalInfo, int numRuns = 30) {
	using namespace ofec;
	//int numRuns(30);
	//numRuns = 1;
...
void runEAX_LKH(const std::string& readDir,
	...
	std::ofstream out(saveDir + tspname + "_lkh_searchDis.txt");
	for (auto& it : heads) {
		out << it << "\t";
	}
	out << std::endl;
...
void runTask() {
	...
	saveDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/nbn_lkh_analysis/";
