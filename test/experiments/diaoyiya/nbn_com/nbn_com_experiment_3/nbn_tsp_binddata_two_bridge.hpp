...
void bindDataTraitSubRegionOneTask(
...
void bindDataTraitSubRegionTask(
	...
	if (std::filesystem::exists(filepath)) {
		...
		{
			// bestSolInfo
			auto curfilename = filename + "_bestSol";
			std::vector<int> solIds;
			nbn::getBestSolIds(traitNBNinfo, solIds);
			std::cout << "best solIds size\t" << solIds.size() << std::endl;
			ofec::nbn::NBNinfo curNBNinfo;
			curNBNinfo.subset(traitNBNinfo, solIds);
			bindDataTraitSubRegionOneTask(curNBNinfo, subRegionNBNinfo, saveDir, curfilename, env);
		}


		{
			// bestSolInfo
			auto curfilename = filename + "_secondFarSol";
			std::vector<int> solIds;
			nbn::getSecondFurtherSolIds(traitNBNinfo, solIds);

			std::cout << "_secondFarSol solIds size\t" << solIds.size() << std::endl;
			ofec::nbn::NBNinfo curNBNinfo;
			curNBNinfo.subset(traitNBNinfo, solIds);
			bindDataTraitSubRegionOneTask(curNBNinfo, subRegionNBNinfo, saveDir, curfilename, env);
		}
...
void bindDataTraitSubRegion(
	...
	std::string filepath;
	filepath = NBNinfo::getSolutionsFileName(traitDir, tspname + "_bestSolutions");
	if (!std::filesystem::exists(filepath)) {
		return;
	}
	ofec::nbn::NBNinfo traitNBNinfo;
	traitNBNinfo.inputVecSolutions<int>(traitDir, tspname + "_bestSolutions", env.get());
	traitNBNinfo.inputNBNinfo(traitDir, tspname + "_bestSolutions");
...
void runTask() {
	...
	const std::string traitDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp3/eax_lkh_lon_nbndata_rnd_analysis/";
	const std::string& subRegionSolDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp3/nbn_sub_region_solVec_hnswModelSingleThreadRnd/";
	const std::string& subRegionNbnInfoDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp3/nbn_sub_region_solVec_hnswModelSingleThreadRnd/";

	const std::string& saveDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp3/nbn_sub_region_solVec_hnswModelSingleThreadRnd_bindData/";
