...
struct NBNAlgInfo {
	...
	static void outputHead(std::ostream& out) {
		out << "iteration\tloop\tmax\tdev\terrorRatio\tmaxErr\ttseconds" << std::endl;
	}
	void output(std::ostream& out) {
		out << m_iteration << "\t" << m_numLoop << "\t" << m_maxChange << "\t" << m_dev << "\t" << m_errRatio << "\t" << m_maxErr << "\t" << m_millisecond << std::endl;
	}
	...
	double errorRatio(const std::vector<double>& dis1, const std::vector<double>& dis2) {
		int numRatio = 0;
		m_maxErr = 0;
		...
		for (int idx(0); idx < dis1.size(); ++idx) {
			...
			m_maxErr = std::max<double>(m_maxErr, abs(dis1[idx] - dis2[idx]));

		}
		m_errRatio = double(numRatio) / double(dis1.size());
		return m_errRatio;
	}
};
...
void solDataToNBN(
	...
	const std::vector<double>& accuDis2Parent,
		...
		};
	{
		...
		std::vector<double> beforeDis2parent(sols.size(), std::numeric_limits<double>::max());
	...
	//	double milliseconds = difference.count();
		...
		edge_division.setNumberLoop(20);
		bool accur_flag = false;
		while (true) {
		...
		//	double milliseconds = difference.count();
			...
			std::chrono::duration<double> diffSeconds = end - start; // 单位为秒
			auto differenceInSeconds = std::chrono::duration_cast<std::chrono::seconds>(diffSeconds).count();
			if (differenceInSeconds - algInfo.m_millisecond > 60) {


				algInfo.updateInfo(beforeDis2parent, dis2parent);
				double erroRatio = algInfo.errorRatio(dis2parent, accuDis2Parent);
				algInfo.m_millisecond = differenceInSeconds;
				algInfo.output(algInfoOut);
				algInfo.output(std::cout);

			}
			if (algInfo.m_errRatio < 0.01 && !accur_flag)
				/*if (algInfo.m_dev < 0.01&&algInfo.m_maxChange<=2)*/ {
				accur_flag = true;
				algInfo.updateInfo(beforeDis2parent, dis2parent);
				double erroRatio = algInfo.errorRatio(dis2parent, accuDis2Parent);
...
void solDataToNBN_subRegion(
	...
	//	double milliseconds = difference.count();
		...
		while (true) {
			...
			//algInfo.m_millisecond = differenceInSeconds;
			edge_division.updateDivisionSubRegionOnCenteredEdgeMultiThread();
			edge_division.getResult(belong, dis2parent);
			algInfo.m_iteration++;
			auto end = std::chrono::system_clock::now();
			//std::chrono::duration<float> difference = end - start;
...
void solDataToNBN_itererationMultithread(
	...
	{
		std::ofstream timeOut(outputDir + filename + "_currentTime.txt");
		timeOut << "time begin\t" << std::endl;
		timeOut.close();
...
void calTaskOneTask(
	...
	if (!samples.empty()) {
		...
		solDataToNBN_itererationMultithread(saveDir, sols, centerSol, filename + "_iteration", dis2parent, env, rnd);
		if (neighborK < 0) {
			solDataToNBN(saveDir, sols, filename + "_random", dis2parent, env, rnd);

		}
		else {
			solDataToNBN_subRegion(saveDir, sols, centerSol, filename + "_subRegion", dis2parent, env, rnd);
...
void calTaskTotal(const std::string& readDir,
	...
	{
		int curSample = 1e5;
		int addedSample = curSample / 2;

		while (curSample <= 1e6) {
			curSample += addedSample;
...
void runTask2() {
	...
	saveDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/nbn_tsp_algorithmTime_basedCenter/";
