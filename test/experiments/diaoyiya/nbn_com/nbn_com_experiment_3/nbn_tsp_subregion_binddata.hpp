...
void bindDataTraitSubRegionTask(
	...
	const std::vector<int>& lonFunnelIds,
	...
	if (std::filesystem::exists(filepath)) {
		...
		std::cout << "input subRegionNBNinfo begin" << std::endl;
		...
		std::vector<int> subsetFunnel;
		for (int idx(0); idx < lonFunnelIds.size(); ++idx) {
			if (lonFunnelIds[idx] != -2) {
				subsetIds.push_back(idx);
				subsetFunnel.push_back(lonFunnelIds[idx]);
			}
		}

		std::cout << "subset lon\t" << subsetIds.size() << std::endl;


		lonNBNinfo.resize(lonNBNinfo.m_solbases.size());

		ofec::nbn::NBNinfo sublonNBNinfo;
		sublonNBNinfo.subset(lonNBNinfo, subsetIds);

		std::cout << "calculate  getNearestBestSolutions begin" << std::endl;
		ofec::nbn::NBNinfo curNBNinfo;
		getNearestBestSolutions(curNBNinfo, sublonNBNinfo, subRegionNBNinfo, env);

		std::cout << "calculate  getNearestBestSolutions end" << std::endl;
...

		{
			...
			std::vector<int> funnelInfo(subRegionNBNinfo.m_belong.size(), -2);
			for (int idx(0); idx < sublonNBNinfo.m_solbases.size(); ++idx) {
				auto curId = curNBNinfo.m_belong[idx];
				if (funnelInfo[curId] == -2) {
					funnelInfo[curId] = subsetFunnel[idx];
				}
				else if (funnelInfo[curId] != subsetFunnel[idx]) {
					funnelInfo[curId] = -1;
				}
				//
				//if (funnelInfo[curId] != ) {
...

			{
				...
				std::cout << "numFunnelIds\t" << numFunnelIds << "\tcommom ids\t" << commomIds << std::endl;


				std::vector<int> funnelPath(sublonNBNinfo.m_solbases.size(), -2);
				std::vector<int> subRegionFunnelIds;
				for (int idx(0); idx < funnelInfo.size(); ++idx) {
					if (funnelInfo[idx] >= 0) {
						subRegionFunnelIds.push_back(idx);
					}
				}
			...
			{
				std::ofstream out(saveDir + filename + "_lonFunnelIdx.txt");
				ofec::matlab::outputVector(out, funnelInfo);
				out.close();
...
void outputTspSolutions(const std::string& saveDir, const std::string& filename,
	...
	ofec::variants_stream::outputToFile(paramsStream, saveDir + filename + "_solVariables.txt");
}
void inputTspSolutions(const std::string& saveDir, const std::string& filename, ofec::nbn::NBNinfo& info, ofec::Environment* env) {
	ofec::ParameterVariantStream paramsStream;
	ofec::variants_stream::inputFromFile(paramsStream, saveDir + filename + "_solVariables.txt");
	size_t solSize(0);

	auto& m_sols = info.m_sols;
...
void readLONdata(const std::string& traitDir,
	const std::string& tspname,
	ofec::nbn::NBNinfo& info, ofec::Environment* env) {
...
void bindDataTraitSubRegion(
	...
	std::vector<int> nodeFunnelId;
	{
		{
			ofec::ParameterVariantStream paramsStream;
			ofec::variants_stream::inputFromFile(paramsStream, traitDir + tspname + "_lonFunnelIds.txt");
	...
	inputTspSolutions(traitDir, tspname + "_lonSamples", lonNBNinfo, env.get());

	//traitNBNinfo.inputVecSolutions<int>(traitDir, tspname + "_optSols", env.get());
	std::cout << "traitNBNinfo size" << std::endl;
	std::cout << lonNBNinfo.m_solbases.size() << std::endl;
	std::string filename;
	filename = tspname + "_randomSamples";

	bindDataTraitSubRegionTask(
		lonNBNinfo, nodeFunnelId, subRegionSolDir, subRegionNBNinfoDir, saveDir, filename, env.get());
...
void runTask() {
	...
	const std::string traitDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/nbn_data_lonSample3/";
	...
	const std::string& saveDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/nbn_sub_region_solVec_hnswModelSingleThread_lonData/";
	...
	std::cout << "calculating nbn subresion all task nbn_hnswEqualBetter_BonData" << std::endl;
