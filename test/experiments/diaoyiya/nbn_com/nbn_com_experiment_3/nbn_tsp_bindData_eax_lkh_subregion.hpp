...
void bindDataTraitSubRegionTask(
	...
	std::string filepath = { nbn::NBNinfo::getSolutionsFileName(subRegionDir,filename) };
	if (std::filesystem::exists(filepath)) {



		ofec::nbn::NBNinfo subRegionNBNinfo;
		subRegionNBNinfo.inputVecSolutions<int>(subRegionDir, filename, env);
...
void bindDataTraitSubRegion(
	...
	bindDataTraitSubRegionTask(
		traitNBNinfo, subRegionDir, saveDir, filename, env.get());
...
void runTask() {
	...
	const std::string& subRegionDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/nbn_sub_region_solVec_hnswModelEqualBetter/";
	const std::string& saveDir = ofec::g_working_directory + "/paper_com_experiment_data/totalTsp/nbn_hnswEqualBetter_bindData/";
...
//	std::reverse(filenames.begin(), filenames.end());
	...
	for (auto& it : filenames) {
		if (it == "u574.tsp") {
			bindDataTraitSubRegion(dir2, traitDir, subRegionDir, saveDir, it);
		}
		else {
			bindDataTraitSubRegion(dir1, traitDir, subRegionDir, saveDir, it);
