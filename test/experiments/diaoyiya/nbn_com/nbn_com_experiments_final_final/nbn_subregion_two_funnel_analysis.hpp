...
void calTask(const std::string& readDir,
	...
	std::vector<double> numSamplesSize = { 1e4,1e5,1e6 };
		...
		for (auto& curSample : numSamplesSize) {
			...
			for (auto& neighborK : neiKs) {
...

				{
					...
					//std::vector<int> connectedBridgeIds;
					...
					for (int nodeId(0); nodeId < mergeNBNinfo.m_solIds.size(); ++nodeId) {
						if (mergeNBNinfo.m_belong[nodeId] != -1 && mergeNBNinfo.m_belong[nodeId] != nodeId) {
							if (funnelIds[mergeNBNinfo.m_belong[nodeId]] != funnelIds[nodeId]) {
								std::cout << "tspname\t" << tspname << "\tSolId\t" << nodeId << "\tBridgeDis\t" << mergeNBNinfo.m_dis2parent[nodeId] << std::endl;
							}
						}
					}
...

					{
						...
						std::vector<double> bridgeDis;
						for (auto& it : bridgeIds) {
							if (mergeNBNinfo.m_belong[it] != it && mergeNBNinfo.m_belong[it] != -1)
								bridgeDis.push_back(mergeNBNinfo.m_dis2parent[it]);
						}

						double maxNBD(0);
						calMax(bridgeDis, maxNBD);
						std::cout << "max NBD of bridge \t" << maxNBD << std::endl;
						//std::vector<int> solIds;
...

						{
							double maxNBD(0);
							int maxNBDid = 0;
							maxNBD = ofec::nbn::getMaxNBD(mergeNBNinfo, maxNBDid);
							std::cout << "max NBD of network \tId\t" << maxNBDid << "\tmaxNBD\t" << maxNBD << std::endl;
						}


						totalOut << tspname << "\t" << filterSolId << "\t" << numSamples << "\t" << means.front() - means.back() << "\t" << maxNBD << "\t";
					}
...
void runTask() {
	...
	readdir2 = "//172.24.24.151/f/DiaoYiya/nbn_com_exp_final/eax_lkh_nbn_rnd_lkhmore5_filterSol_final2/";
	...
	saveDir = "//172.24.24.151/f/DiaoYiya/nbn_com_exp_final/nbn_subregion_two_funnel2_linux/";
...
//	std::reverse(filenames.begin(), filenames.end());
	//std::reverse(filenames.begin(), filenames.end());
	std::ofstream totalOut("//172.24.24.151/f/DiaoYiya/nbn_com_exp_final/nbn_subregion_two_funnel2_analysis_result/bridgeDataOut.txt");
	for (auto& it : filenames) {
		if (it == "u574.tsp") {
			calTask(dir2, totalOut, readdir2, saveDir, it);
		}
		else {
			calTask(dir1, totalOut, readdir2, saveDir, it);
		}
...
namespace ofec {
	...
	void run(int argc, char* argv[]) {
		...
		dis.push_back(7);
		double mean(0), stdvalue(0);
		ofec::calMeanAndStd(dis, mean, stdvalue);
