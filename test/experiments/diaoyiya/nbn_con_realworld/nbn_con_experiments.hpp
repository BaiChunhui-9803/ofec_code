...
void generateSolutionsConsRadiusSub(
	...
	for (int idx(from); idx < to; ++idx) {
		...
		for (int idDim(0); idDim < conPro->numberVariables(); ++idDim) {
			cursol.variable()[idx] = rnd->uniform.nextNonStd<double>(initboudnary[idx].first, initboudnary[idx].second);
		}
...
void runTotalTasks() {
	...
	std::vector<std::string> proNames = { "GearTrain", "SoundWaves", "GTOP_Cassini2" };
